        ...
        with mock_tmux_fixture() as mock_tmux, \
            ...
            with patch('subprocess.run') as mock_git:
                ...
                self.assertIsNotNone(orchestration.message_broker, 
                                   "Redis message broker should be available")
                
                # 2. Agent created and registered
                agent_keys = [k for k in mock_redis.hashes.keys() if k.startswith('agent:')]
                self.assertGreater(len(agent_keys), 0, "Agent should be registered in Redis")
                
                # 3. Tmux session created
                self.assertGreater(len(mock_tmux.sessions), 0, "Tmux session should be created")
                ...
                worktree_calls = [call for call in git_calls 
                                if 'worktree' in call and 'main' in call]
                self.assertGreater(len(worktree_calls), 0, 
                                 "Git worktree should be created from main")
        ...
        with mock_tmux_fixture() as mock_tmux, \
            ...
            with patch('subprocess.run') as mock_git:
                ...
                self.assertGreater(len(mock_tmux.sessions), 0,
                                 "Agent should be created even without Redis")
                
                # 3. File-based coordination used
                self.assertTrue(os.path.exists('/tmp/orchestration_results'),
                              "Result directory should exist for file-based coordination")
        ...
        with mock_tmux_fixture() as mock_tmux, \
            ...
            try:
                with patch('subprocess.run') as mock_subprocess:
                    ...
                    self.assertTrue(continue_flag_found, 
                                  "Should use --continue flag for existing conversations")
    ...
    def test_multiple_agents_coordination(self):
        ...
        with mock_tmux_fixture() as mock_tmux, \
            ...
            with patch('subprocess.run') as mock_git:
                ...
                self.assertEqual(len(session_names), len(unique_sessions),
                               "All tmux sessions should have unique names")
                
                # If multiple agents were created, verify Redis coordination
                agent_keys = [k for k in mock_redis.hashes.keys() if k.startswith('agent:')]
                self.assertEqual(len(agent_keys), len(mock_tmux.sessions),
                               "Number of Redis registrations should match tmux sessions")
            ...
            with patch('subprocess.run') as mock_git:
                ...
                def git_side_effect(cmd, **kwargs):
                    ...
                    if call_count == 1 and 'git' in cmd[0]:
                        return Mock(returncode=1, stderr='git error')  # First git call fails
                ...
                self.assertIsNotNone(orchestration.task_dispatcher,
                                   "Task dispatcher should be initialized")
    ...
    def test_dependency_integration(self):
        ...
        with mock_tmux_fixture(), mock_claude_fixture(), mock_redis_fixture():
            ...
            with patch('subprocess.run') as mock_deps:
                ...
                self.assertTrue(has_dependencies, "All dependencies should be available")
                
                # Verify dependency checks were made
                dep_calls = [call.args[0] for call in mock_deps.call_args_list
                ...
                self.assertTrue(expected_tools.issubset(checked_tools),
                              f"Should check all required tools: {expected_tools}")
