...
class TestA2AIntegration(unittest.TestCase):
    ...
    def test_agent_registration_with_a2a(self):
        ...
        with mock_tmux_fixture(), mock_claude_fixture(), \
            ...
            with patch('subprocess.run') as mock_git:
                ...
                self.assertGreater(len(registered_agents), 0, 
                                 "Agent should be registered with A2A system")
                
                # 2. Verify agent has correct A2A capabilities
                agent_id = list(registered_agents.keys())[0]
                agent_info = registered_agents[agent_id]
                
                expected_capabilities = ["task_execution", "development", "git_operations", 
                                       "server_management", "testing", "full_stack"]
                self.assertEqual(agent_info['capabilities'], expected_capabilities)
                
                # 3. Verify agent type is set correctly
                self.assertEqual(agent_info['type'], 'development')
    ...
    def test_redis_message_broker_initialization(self):
        ...
        with patch('redis.Redis') as mock_redis_class:
            ...
            self.assertIsNotNone(broker.redis_client)
            self.assertEqual(broker.redis_client, mock_redis_instance)
    ...
    def test_agent_capability_registration(self):
        ...
        with mock_message_broker_fixture() as mock_broker:
            ...
            registered_agents = mock_broker.get_registered_agents()
            self.assertEqual(len(registered_agents), 3, "All agents should be registered")
            
            for agent_spec in test_cases:
                agent_name = agent_spec['name']
                self.assertIn(agent_name, registered_agents)
                
                agent_info = registered_agents[agent_name]
                self.assertEqual(agent_info['type'], agent_spec['type'])
                self.assertEqual(agent_info['capabilities'], agent_spec['capabilities'])
    ...
    def test_inter_agent_messaging_flow(self):
        ...
        with mock_message_broker_fixture() as mock_broker:
            ...
            self.assertEqual(len(sent_tasks), 1, "Task should be sent")
            
            sent_task = sent_tasks[0]
            self.assertEqual(sent_task['from'], agent1_id)
            self.assertEqual(sent_task['to'], agent2_id)
            self.assertEqual(sent_task['data'], task_data)
        ...
        with mock_tmux_fixture(), mock_claude_fixture():
            ...
            with patch('subprocess.run') as mock_git:
                ...
                self.assertIsNone(orchestration.message_broker,
                                "Should handle A2A unavailability gracefully")
    ...
    def test_task_result_reporting_through_a2a(self):
        ...
        with mock_message_broker_fixture() as mock_broker:
            ...
            mock_broker.register_agent(coordinator_id, "coordinator", ["task_management"])
            ...
            mock_broker.send_task(agent_id, coordinator_id, {
                'type': 'task_result',
                'data': result_data
            })
            ...
            sent_tasks = mock_broker.sent_tasks
            result_task = next((task for task in sent_tasks 
                              if task['data'].get('type') == 'task_result'), None)
            
            self.assertIsNotNone(result_task, "Task result should be sent")
            self.assertEqual(result_task['from'], agent_id)
            self.assertEqual(result_task['to'], coordinator_id)
            self.assertEqual(result_task['data']['data'], result_data)
    ...
    def test_agent_discovery_through_a2a(self):
        ...
        with mock_message_broker_fixture() as mock_broker:
            ...
            agents = [
                {'id': 'ui-expert', 'type': 'frontend', 'capabilities': ['react', 'css']},
                {'id': 'api-expert', 'type': 'backend', 'capabilities': ['fastapi', 'postgresql']},
                {'id': 'test-expert', 'type': 'testing', 'capabilities': ['pytest', 'selenium']}
            ]
            
            # When: Agents register with A2A
            for agent in agents:
                mock_broker.register_agent(agent['id'], agent['type'], agent['capabilities'])
            
            # Then: Any agent should be able to discover others
            registered_agents = mock_broker.get_registered_agents()
            
            # Verify all agents are discoverable
            self.assertEqual(len(registered_agents), 3, "All agents should be discoverable")
            
            # Verify agent capabilities are accessible
            ui_expert = registered_agents['ui-expert']
            self.assertIn('react', ui_expert['capabilities'])
            
            api_expert = registered_agents['api-expert']
            self.assertIn('fastapi', api_expert['capabilities'])
    ...
    def test_a2a_message_persistence_in_redis(self):
        ...
        with patch('redis.Redis') as mock_redis_class:
            ...
            self.assertEqual(call_args[0][0], f"agent:{agent_id}")  # Redis key
            
            # Verify mapping contains expected fields
            mapping = call_args[1]['mapping']
            self.assertEqual(mapping['id'], agent_id)
            self.assertEqual(mapping['type'], agent_type) 
            ...
            self.assertEqual(stored_capabilities, capabilities)
