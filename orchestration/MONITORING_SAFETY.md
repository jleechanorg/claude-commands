...
The root cause of agent ta[REDACTED_OPENAI_KEY]'s failure was sending keyboard input ('q') via tmux during monitoring.
...
```bash
# SAFE - Read only
tmux capture-pane -t ta[REDACTED_OPENAI_KEY] -p | tail -50

# SAFE - Use safe monitor script
python3 orchestration/safe_agent_monitor.py ta[REDACTED_OPENAI_KEY]
...
tmux send-keys -t ta[REDACTED_OPENAI_KEY] q     # Causes SIGINT!
tmux send-keys -t ta[REDACTED_OPENAI_KEY] " "   # Can interfere!
tmux send-keys -t ta[REDACTED_OPENAI_KEY] Enter # Unpredictable!
```
...
python3 orchestration/safe_agent_monitor.py ta[REDACTED_OPENAI_KEY] -c

# Monitor all agents
python3 orchestration/safe_agent_monitor.py -a

# Custom interval
python3 orchestration/safe_agent_monitor.py ta[REDACTED_OPENAI_KEY] -c -i 10
```
