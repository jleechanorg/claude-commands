...
# Constraint examples
/orch "update $PROJECT_ROOT/readme.md with latest info. only update that file"
...
**File Constraints:**
- `allowed_files`: Specific patterns like `['*.py', '$PROJECT_ROOT/*']`
...
// /tmp/orchestration/a2a/registry.json
{
  "task-agent-80914589": {
    ...
    "current_task": null,
    "constraints": {
      "allowed_files": ["*.py", "$PROJECT_ROOT/*", "src/*"],
      "max_file_changes": 10,
...
// /tmp/orchestration/a2a/agents/task-agent-80914589/heartbeat.json
{
  ...
  "progress": {
    "files_analyzed": ["$PROJECT_ROOT/main.py", "$PROJECT_ROOT/game_state.py"],
    "files_modified": [],
...
// Agent completion message
{
  ...
  "payload": {
    "task_id": "debug-mode-investigation",
    "status": "completed",
    "files_modified": ["$PROJECT_ROOT/main.py", "$PROJECT_ROOT/debug_mode_parser.py"],
...
5. **Constraint Application** → Agent restricted to `['*.py', '$PROJECT_ROOT/*']`
