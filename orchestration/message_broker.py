...
class MessageBroker:
    ...
    def get_task(self, agent_id: str) -> Optional[TaskMessage]:
    ...
    def heartbeat(self, agent_id: str):
        ...
        self.redis_client.hset(f"agent:{agent_id}", "last_heartbeat", datetime.now().isoformat())
    ...
    def cleanup_stale_agents(self, timeout_seconds: int = 300):
        ...
        for agent_id in list(self.agent_registry.keys()):
            last_heartbeat = self.redis_client.hget(f"agent:{agent_id}", "last_heartbeat")
            if last_heartbeat:
