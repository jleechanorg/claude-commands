...
class TaskDispatcher:
    ...
    def __init__(self, orchestration_dir: str = None):
        ...
        self.agent_capabilities = {
            "frontend-agent": {
                ...
                "keywords": ["ui", "react", "css", "html", "component", "style", "interface", "form"],
                "max_concurrent": 2,
                "current_load": 0
            },
            "backend-agent": {
                "types": [TaskType.BACKEND, TaskType.INFRASTRUCTURE],
                "keywords": ["api", "database", "server", "flask", "firestore", "auth", "endpoint"],
                "max_concurrent": 3,
                "current_load": 0
            },
            "testing-agent": {
                "types": [TaskType.TESTING, TaskType.BUGFIX],
                "keywords": ["test", "bug", "fix", "validate", "verify", "quality", "coverage"],
                "max_concurrent": 2,
                "current_load": 0
            }
        }
    ...
    def _parse_task_description(self, task_id: str, description: str, suggested_agent: str = None) -> Task:
    ...
    def _infer_task_type(self, description: str) -> TaskType:
        ...
        if any(keyword in desc_lower for keyword in ["ui", "react", "css", "html", "component", "form", "style"]):
            return TaskType.FRONTEND
        elif any(keyword in desc_lower for keyword in ["api", "database", "server", "backend", "auth", "endpoint"]):
            return TaskType.BACKEND
        elif any(keyword in desc_lower for keyword in ["test", "testing", "validate", "verify", "quality"]):
            return TaskType.TESTING
        elif any(keyword in desc_lower for keyword in ["bug", "fix", "error", "issue"]):
            return TaskType.BUGFIX
        elif any(keyword in desc_lower for keyword in ["doc", "documentation", "readme", "guide"]):
            return TaskType.DOCUMENTATION
        elif any(keyword in desc_lower for keyword in ["deploy", "ci", "infrastructure", "setup"]):
    ...
    def _infer_priority(self, description: str) -> TaskPriority:
        ...
        if any(keyword in desc_lower for keyword in ["critical", "urgent", "emergency", "production"]):
            return TaskPriority.CRITICAL
        elif any(keyword in desc_lower for keyword in ["important", "high", "priority"]):
            return TaskPriority.HIGH
        elif any(keyword in desc_lower for keyword in ["low", "minor", "nice to have"]):
    ...
    def _estimate_duration(self, description: str) -> int:
        ...
        desc_lower = description.lower()
        if any(keyword in desc_lower for keyword in ["complex", "refactor", "redesign", "architecture"]):
            base_duration *= 2
    ...
    def get_best_agent_for_task(self, task: Task) -> Optional[str]:
    ...
    def assign_task(self, task: Task, agent: str = None) -> bool:
        ...
        if self.agent_capabilities[agent]["current_load"] >= self.agent_capabilities[agent]["max_concurrent"]:
    ...
    def get_agent_workload(self) -> Dict[str, Any]:
        ...
        for agent_name, capabilities in self.agent_capabilities.items():
            agent_tasks = [task for task in self.tasks.values() 
                         if task.assigned_agent == agent_name and task.status in ["assigned", "in_progress"]]
            
            workload[agent_name] = {
                "current_load": capabilities["current_load"],
                "max_concurrent": capabilities["max_concurrent"],
                "utilization": capabilities["current_load"] / capabilities["max_concurrent"],
    ...
    def generate_task_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "task_distribution": {
                task_type.value: sum(1 for task in self.tasks.values() 
    ...
    def save_task_report(self):
        ...
        print(f"📊 Task report saved: {report['total_tasks']} tasks, "
