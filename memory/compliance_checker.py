...
from mock_mcp_reader import MockMemoryMCPReader, ComplianceMemoryReader, LearningMemoryReader
...
class HeaderComplianceChecker:
    ...
    def check_header_compliance(self, verbose: bool = False) -> Dict[str, Any]:
        ...
        results = {
            ...
            "recommendations": self._generate_recommendations(header_violations, header_learnings),
    ...
    def _generate_recommendations(self, violations: List[Dict], learnings: List[Dict]) -> List[str]:
...
class LearningRetriever:
    ...
    def get_context_learnings(self, context: str, category: str = None) -> Dict[str, Any]:
        """Get learnings relevant to current context"""
        related_learnings = self.learning_reader.get_related_learnings(context)
        
        if category:
            category_learnings = self.learning_reader.get_learnings_by_category(category)
            # Combine and deduplicate
            all_learnings = {l["name"]: l for l in related_learnings + category_learnings}
        ...
        for learning in learnings[:3]:  # Top 3
            ...
            if learning["observations"]:
                ...
                if "Classification:" in first_obs:
                    classification = first_obs.split("Classification:")[1].split("Category:")[0].strip()
                    content = first_obs.split("Content:")[1].split("Context:")[0].strip() if "Content:" in first_obs else first_obs
