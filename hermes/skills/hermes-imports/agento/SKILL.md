...
`ao` reads `agent-orchestrator.yaml` from the current directory. In this setup, the canonical config is `~/.hermes/agent-orchestrator.yaml` (includes project `jleechanclaw`). Running from unrelated directories can fail with:
