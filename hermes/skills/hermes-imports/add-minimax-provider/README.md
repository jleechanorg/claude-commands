...
> **GitHub**: https://github.com/jooey/hermes-skill-add-minimax-provider

为 Hermes 配置 MiniMax 大模型的 AgentSkill。

## 功能

一键引导 Hermes 接入 MiniMax 模型，支持两种接入方式：
...
git clone https://github.com/jooey/hermes-skill-add-minimax-provider.git \
  ~/.hermes/workspace/skills/add-minimax-provider
```
...
```bash
# 下载并解压到 skills 目录
cd ~/.hermes/workspace/skills
curl -L https://github.com/jooey/hermes-skill-add-minimax-provider/archive/refs/heads/main.tar.gz | tar xz
mv hermes-skill-add-minimax-provider-main add-minimax-provider
```
...
1. 先配好 `qwen-portal` provider（免费，OAuth 登录即可，用hermes config命令配置）
2. 用 Qwen Coder 作为 primary 模型启动 Hermes
3. 在聊天中说："帮我配置 MiniMax 模型"
4. Agent 自动加载本 skill，按步骤完成配置
5. `/model Minimax` 切换到 MiniMax 作为主力

> 这就是 Hermes 的"自举"——用免费模型启动系统，再让它帮你配更强的模型。

## Skill 覆盖的流程

1. **测试模型可用性** — curl 验证 API 连通性
2. **添加 Provider** — 写入 `config.yaml`
3. **配置别名** — `/model Minimax` 快速切换
4. **接入 Fallback 链** — 自动容灾
5. **验证** — JSON 校验 → `hermes doctor` → 重启 → 功能测试
6. **排障** — 常见问题诊断指南
