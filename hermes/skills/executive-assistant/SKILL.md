...
|---|---|---|
| `{{OWNER_NAME}}` / operator | Jeffrey Lee-Chan (`$USER`, Slack `U09GH5BR3QU`) | user identity |
...
| `#jleechanclaw` | `C0AJ3SD5C79` | Harness / SOUL.md / skill work |
...
- **P39 — Skill placeholders are never substituted in cron env.** The body uses `{{OWNER_NAME}}`, `{{ASSISTANT_EMAIL}}`, `{{PERSONAL_EMAIL}}`, `{{PRIMARY_WORK_EMAIL}}`, `{{SECONDARY_CALENDAR_EMAIL_*}}`. Cron runtimes do NOT resolve them. Hardcode the resolved values:
  - Owner: `Jeffrey Lee-Chan` (handle: `$USER`); DM channel: `D0AFTLEJGJU`; Slack user: `U09GH5BR3QU`; bot identity: `U0AEZC7RX1Q` (`hermes`).
...
- **P50 — Slack MCP `mcp__slack__conversations_add_message` "text must be a string" bug is payload-size + multi-line driven, not content-driven.** Verified 2026-07-07 08:01 PDT sweep: a ~5.1 KB / ~5,200-char top-level brief (with Slack mrkdwn emoji, newlines, and `*bold*` segments) returned `{"error":"text must be a string"}` from the MCP wrapper, while the *same* payload posted via raw `curl` to `chat.postMessage` with `HERMES_SLACK_BOT_TOKEN` succeeded with `{"ok":true,"ts":"..."}`. Pattern from the canonical `clawchief:ea-sweep-hourly` history: 12:01 PDT brief also fell back to curl with the same error; 16:01 PDT and 20:01 PDT briefs posted via MCP successfully (shorter, fewer newlines). **Concrete decision rule**: if the brief payload is ≥ ~3,000 chars OR contains > ~20 newlines OR > ~10 emoji shortcodes, post via curl from the start. Don't try MCP first and "see if it works" — the failure wastes a turn and the curl fallback is identical. **Curl recipe** (the working one):
  ```bash
  cat > /tmp/ea_brief.txt <<'EOF'
  <brief body, slack mrkdwn, emoji shortcodes fine>
  EOF
  PAYLOAD=$(python3 -c "import json; t=open('/tmp/ea_brief.txt').read(); print(json.dumps({'channel':'D0AFTLEJGJU','text':t}))")
  curl -fsS -X POST "https://slack.com/api/chat.postMessage" \
    -H "Authorization: Bearer $HERMES_SLACK_BOT_TOKEN" \
    -H "Content-Type: application/json; charset=utf-8" \
    -d "$PAYLOAD"
  ```
  The `cat > heredoc` + `python3 json.dumps` indirection is required: passing the raw body via `-d "$VAR"` corrupts when the body contains unescaped newlines/quotes; the heredoc + json.dumps step escapes correctly. **Token source**: `$HERMES_SLACK_BOT_TOKEN` is loaded by `launchd-env-wrapper.sh` from `~/.bashrc` per `slack-messaging` skill; do NOT source from `~/.bashrc` directly in the cron shell.
- **P51 — Top-level DM brief is a top-level POST, not a thread reply.** The DM channel `D0AFTLEJGJU` receives the executive assistant brief as a fresh top-level message (no `thread_ts`). Earlier SOUL.md drafts said "threaded to the most recent user message" — that's the `slack-reply-inherit-thread-ts` rule for *operator asks*, not for the EA sweep output. Concrete: pass `{"channel":"D0AFTLEJGJU","text":...}` and *omit* `thread_ts`. If a brief is posted with a `thread_ts` to a prior user message, it hides under the user's own message and is easy to miss. Verified 2026-07-07 08:01 PDT — brief landed at `ts=1783436623.430879` as a top-level post, visible in the channel sidebar.
- **P52 — Operator self-message in DM: when scanning `conversations_history(D0AFTLEJGJU, limit=5)` for the last *bot* brief, filter on `BotName=hermes` (not `UserID` prefix).** On 2026-07-07 the most recent 5 messages in the DM were 2 operator posts + 2 prior bot briefs + 1 bot self-reply. The dedup check needs to find the last `BotName=hermes` row, NOT the last `UserID=U0AEZC7RX1Q` row (the bot's self-replies in operator threads also match the UserID). Verified 2026-07-07 08:01 PDT: last bot brief was `ts=1783393463.370289` (Mon 20:01 evening brief, `BotName=hermes`, `UserID=U0AEZC7RX1Q`); the next-to-last message was a bot self-reply with same `UserID` but empty `BotName` — filtering on `BotName=hermes` correctly skipped it. Pair with the P40 in the canonical skill: "prior cron-prompt self-narration is not a Slack artifact" — same lesson, the fix is to trust `BotName` not `UserID`.
- **P53 — `mcp__slack__conversations_replies` accepts a top-level `thread_ts` and returns the thread INCLUDING the top-level message itself.** Verified 2026-07-07 08:01 PDT: passing `thread_ts=1783409528.821869` (which is itself a top-level operator message in `C09GRLXF9GR`) returned the thread anchored at that message with all bot replies. This is fine and expected — but if you want *only the replies*, you'd need to skip the first row or use a sub-thread marker. For the EA sweep (where you want the full context of the operator ask + the bot's diagnosis), passing the top-level ts as `thread_ts` is exactly right. Pitfall: do NOT pass a `thread_ts` that points to a bot's reply within an existing thread — that nests a sub-thread and creates a confusing second-layer view.
- **P54 — `uptime` on macOS 15.5: format is `HH:MM  up  D+HH:MM, N users, load averages: X Y Z`.** Three space-separated floats after `load averages:`. The cron-job history sometimes shows older format with commas (Linux) — when grepping for load, anchor on `load averages:` then take the next 3 tokens, not on `,` delimiters. Verified 2026-07-07 08:01 PDT: `8:02  up  8:49, 14 users, load averages: 8.55 9.72 12.81` → load1=8.55, load5=9.72, load15=12.81. Skill P44 (df data-volume row) + the launchd/probe recipes in `references/system-probes-recipes.md` already cover the other macOS-vs-Linux quirks; uptime-format is the only one not yet captured.
- **P55 — Duplicate `~/.hermes/skills/hermes-imports/executive-assistant/SKILL.md` STILL EXISTS as of 2026-07-07.** Verified by `skill_view(name='executive-assistant')` returning "Ambiguous skill name" — both copies still register. The canonical (`~/.hermes/skills/executive-assistant/SKILL.md`) has resolved `{{OWNER_NAME}}` → `Jeffrey`, real channel IDs, all P40–P49 pitfalls. The stale hermes-imports copy still has raw `{{...}}` placeholders and the obsolete `--days=1` flag. Workaround until curator fixes: always patch the canonical by name; the ambiguous-name error means `skill_manage(action='patch', name='executive-assistant')` ALSO fails to disambiguate — use the full path `hermes-imports/executive-assistant` only to *read* the stale copy for diff purposes, never to patch. (Curator action item — see `references/curator-action-items.md` if it exists, else log here.)
