...
This calls `_extract_bashrc_var` for known tokens (canonical; matches what launchd cron jobs see). Verified working: `HERMES_SLACK_BOT_TOKEN` (58 chars `[REDACTED_SLACK_TOKEN]`) + `SLACK_USER_TOKEN` (80 chars `[REDACTED_SLACK_TOKEN]`) both surface correctly.
