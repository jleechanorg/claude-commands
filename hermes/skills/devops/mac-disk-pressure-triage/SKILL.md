---
name: mac-di[REDACTED_OPENAI_KEY]
...
python3 ~/.hermes/skills/devops/mac-di[REDACTED_OPENAI_KEY]/scripts/parse_worktrees.py ~/projects/<repo>
...
1. **Unlocked + not on open PR** (gold-standard Tier 1 — git registry's own state, cross-checked against live PRs):
   ```bash
   python3 ~/.hermes/skills/devops/mac-di[REDACTED_OPENAI_KEY]/scripts/parse_worktrees.py ~/projects/<repo> --delete-safe --open-pr-file /tmp/open_prs_<repo>.txt
...
The canonical macOS di[REDACTED_OPENAI_KEY] watchdog (in `~/Library/Application Support/user-scope/bin/`):
- `disk_usage_alert.sh` — checks `/` by default, posts to Slack
- `disk_usage_alert.sh --silence` — writes `$HOME/.di[REDACTED_OPENAI_KEY]`, suppresses future alerts
...
- `references/2026-07-14-15-apfs-lazy-purge-di[REDACTED_OPENAI_KEY]` — first documented instance of the APFS drift trap (Trace #3).
