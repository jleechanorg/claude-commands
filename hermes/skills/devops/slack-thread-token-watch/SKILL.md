...
changelog:
  - '0.1.0 (2026-07-14): Initial authoring. Triggered by the 2026-07-14 babysit-ao-pr-loop v1.9.1 carve-out. Every prior literal-token watch loop was wired through babysit-ao-pr-loop, which assumes a PR + AO worker pair and exits immediately when neither exists. Drafted directly from the disk-pressure babysit prompt body (job 18bd680865d9), which is the reference implementation. Verified contract: tick equals conversations.replies substring scan + optional destructive callback + hermes cron remove on terminal state.'
---
...
- references/bug-ref-2026-07-14.md - full transcript of the disk-pressure babysit 18bd680865d9 failure mode and the cron-output dump that diagnosed it.
