...
- **DO NOT** use `launchctl print gui/$(id -u)/<plist>` to extract the token — `HERMES_SLACK_BOT_TOKEN` is REDACTED in launchctl's display (shows `xoxb-9...AjyN` not the full value). Always read directly from `~/.bashrc` via `grep -m1 '^export HERMES_SLACK_BOT_TOKEN=' ~/.bashrc | sed ...`.
...
- Token may produce `invalid_auth` if extracted incorrectly — `len(token) >= 56` is a quick sanity check (real xoxb tokens are ~56 chars). A `xoxb-9...AjyN` truncated value WILL produce `invalid_auth`.
