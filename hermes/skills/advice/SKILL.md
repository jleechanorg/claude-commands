...
1. **`/advice` is NOT a PR comment bot.** Posting `/advice` as a PR comment does nothing — the skill is processed locally by the calling LLM, not by a GitHub workflow. Confirm this with `grep -rln "/advice" /Users/$USER/projects_other/your-project.com/.github/workflows/` (returns empty).
