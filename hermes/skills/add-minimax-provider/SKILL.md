...
description: 为 Hermes 配置 MiniMax 作为模型源。MiniMax 提供两种接入方式：API Key 直连（openai-completions 协议）和 OAuth 门户（anthropic-messages 协议）。包含 provider 注册、模型定义、别名配置、fallback 链接入和验证的完整流程。当管理员说想"加 MiniMax"、"配 minimax"、"接入 MiniMax 模型"、"加海螺模型"、"配 M2.1"时使用此 skill。
---

# 配置 MiniMax Provider（MiniMax 大模型接入）

MiniMax 是国产大模型服务商，提供 MiniMax-M2.1 系列模型。Hermes 支持两种接入方式：
...
1. **先配好 Qwen Coder**（免费，无需 API Key，OAuth 登录即可）：
   - 在 Hermes 中添加 `qwen-portal` provider
...
> 这就是 Hermes 的"自举"能力——用一个免费模型启动系统，再用它来配置更强的模型。
...
|------|------|
| MiniMax 账号 | 同上 |
| OAuth 配置 | 在 Hermes 中配置 `apiKey: "minimax-oauth"`，Hermes 会自动处理 OAuth 流程 |
...
OAuth 门户无法直接用 curl 测试（需要 OAuth token 流程）。配置好后通过 Hermes 实际发消息验证。
...
在 `~/.hermes/config.yaml` 的 `models.providers` 下添加 provider。根据你的接入方式选择一种或两种都配。
...
> **`apiKey: "minimax-oauth"`** 是 Hermes 的特殊标记，表示使用 OAuth 认证流程而非静态 API Key。cost 设为 0 因为 OAuth 门户的计费由平台侧处理。
...
```bash
python3 -c "import json; json.load(open('$HOME/.hermes/config.yaml')); print('JSON OK')"
```
...
# 等 3 秒后确认状态
sleep 3
launchctl print gui/$(id -u)/ai.hermes.gateway | grep -E "job state|last exit"
```
...
```bash
tail -20 ~/.hermes/logs/gateway.err.log
```
...
不要用 API 数字做监控。推荐在 Hermes 中配置 cron 任务，定期发测试请求验证可用性：
...
- **最可能原因**：配置中有非法字段
- **诊断**：`tail -20 ~/.hermes/logs/gateway.err.log`，找 `Unrecognized key`
- **修复**：删除非法字段，运行 `hermes doctor` 确认
...
- **修复**：确认 `apiKey` 字段值为 `"minimax-oauth"`（精确匹配），Hermes 会自动处理 OAuth 流程
- **诊断**：检查 `~/.hermes/logs/gateway.log` 中的 OAuth 相关错误
...
| 日期 | 版本 | 变更内容 | 变更人 |
|------|------|----------|--------|
| 2026-02-08 | v1.0 | 创建 MiniMax provider 配置指南 | ConfigBot (via Hermes with Opus 4.6) |
| 2026-02-09 | v2.0 | 新增 Coding Plan 额度管理专节；更新额度信息 (1500/5h 滑动窗口)；额度 API 不可信警告；多 Agent 消耗建议 | ConfigBot (via Hermes with Qwen3-30B) |
