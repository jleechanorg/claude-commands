...
1. `write_file` writes through a scratchpad at `/private/tmp/.hermes-tmp.<random>` — when disk is full, `write_file` itself fails with `OSError ENOSPC` even for tiny files. This is itself a disk-pressure signal.
...
**Companion skill:** `mac-disk-pressure-triage` Trap #5 — `state.db-wal` runaway. That skill owns the diagnosis ladder + recovery recipe; this section owns the loop-vs-cause interaction.
