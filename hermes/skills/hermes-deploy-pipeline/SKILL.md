...
- ⚠️ **`workspace/` is in `.gitignore` but `workspace/SOUL.md` IS tracked on
  ...
  - Fix: `git add -f workspace/SOUL.md` (force-add overrides gitignore for
    tracked files). Confirmed at commit `3f5adb80f5` ($ORG, 2026-06-20).

- ⚠️ **`$ORG` self-merge flow uses feature-branch push to `main`**
  (different from your-project.com's AO worker PR flow). On $ORG,
...
- `references/$ORG-slash-command-rollout.md` — **read this before adding a slash command or trigger phrase to $ORG**. End-to-end recipe for the three wiring points (`.claude/commands/` + RESOLVER.md + SOUL.md `## COMMIT:`), the $ORG self-merge flow (feat-branch push to main, no human-review PR), the `auto/commit-pending` rebase trap, the 10-item pre-deploy audit, the post-push local-drift sequence, and the canary false-negatives. Verified 2026-06-20 against PR `3f5adb80f5` (`/finish` + `/auto` + "finish the job" trigger).
