...
**Test portability (CodeRabbit MAJOR, 2026-06-19):** the test file `tests/test_finish_the_job_contract.py` uses `HERMES_PROD_SKILLS` (env var, defaults to `$HERMES_HOME/skills`) instead of a hardcoded `/Users/$USER/...` path. Run the tests with:

```bash
# Default (Hermes dev machine: /Users/$USER/.hermes_prod/skills)
...
- `/Users/$USER/llm_wiki/wiki/concepts/vm-within-vm-isolation.md` — canonical page defining VM-within-VM as **container-in-VM-in-host 3-layer stack**.
- `/Users/$USER/roadmap/learnings-2026-07.md` line 267 — "Rust ezgha daemon, JIT registration, VM-within-VM isolation".
