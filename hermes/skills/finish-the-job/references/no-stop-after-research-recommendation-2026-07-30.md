...
- `task-ack-and-execute` SOUL.md commit — "Long task: same turn as first tool call, post: 'On it — [≤8 word summary]. Back shortly.'" The research-then-do trap is when the agent finishes the research half, posts the summary, and never starts the do half.
