...
ls ~/.hermes/logs/ao-lifecycle-*.log 2>/dev/null | tail -5
...
```bash
tail -50 ~/.hermes/logs/ao-lifecycle-<project>.log
```

## Step 3 — Find claim_failed errors

```bash
grep -n "claim_failed\|refusing to fetch\|Session not found" ~/.hermes/logs/ao-lifecycle-<project>.log | tail -20
```
...
```bash
grep -n "project:" ~/.hermes/logs/ao-lifecycle-<project>.log | tail -5
```
...
python3 -c "
import yaml
cfg = yaml.safe_load(open('$HOME/.hermes/agent-orchestrator.yaml'))
...
tail -f ~/.hermes/logs/ao-lifecycle-<project>.log
...
```bash
# Requires: HERMES_SLACK_BOT_TOKEN (from ~/.bashrc) and JLEECHAN_DM_CHANNEL (your DM channel ID)
# Bot token posts as hermes bot; user token ($SLACK_USER_TOKEN from ~/.profile) posts as $USER
HERMES_SLACK_BOT_TOKEN="${HERMES_SLACK_BOT_TOKEN:-}"  # set in ~/.bashrc
JLEECHAN_DM_CHANNEL="${JLEECHAN_DM_CHANNEL:-}"              # set in ~/.bashrc
curl -s -X POST "https://slack.com/api/chat.postMessage" \
  -H "Authorization: Bearer ${HERMES_SLACK_BOT_TOKEN:-$SLACK_USER_TOKEN}" \
