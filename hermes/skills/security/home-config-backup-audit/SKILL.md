...
```bash
launchctl list | grep -iE "backup|home-config|user-scope|conversation-backup|di[REDACTED_OPENAI_KEY]|cron-backup|qdrant-backup|snapshot|home\\.sh"
```
...
| `git commit "backup: ..." && git push` in cron | **High** — automated pushes with no human review | backup-home.sh, di[REDACTED_OPENAI_KEY] snapshot |
...
| Di[REDACTED_OPENAI_KEY] style: script with caller-output vs default-push branch | **Moderate** — depends on which branch is taken | disk_magician.sh line 285 |
