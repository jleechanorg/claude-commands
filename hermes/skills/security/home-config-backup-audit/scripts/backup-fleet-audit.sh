...
echo
echo '```'
launchctl list 2>/dev/null | grep -iE "backup|home-config|user-scope|conversation-backup|disk-snapshot|cron-backup|qdrant-backup|snapshot|home\.sh|openclaw-backup" | head -40
echo '```'
echo
