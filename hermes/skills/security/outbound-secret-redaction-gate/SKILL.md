...
- **Reports name paths and token fingerprints only** (e.g. `[REDACTED_GITHUB_TOKEN]…`), never the token itself. Fingerprints must be at least 12 hex chars.
...
6) Audit disk_magician + swarm + di[REDACTED_OPENAI_KEY] skills for raw `git remote -v`,
...
7. **The agent that opened the leak is often not the agent that should fix it.** A di[REDACTED_OPENAI_KEY] worker opened `disk_magician#25`; the fix lives in `jleechanclaw`'s harness. Don't try to patch `disk_magician` - that's the application code path. Patch the harness.
...
9a. **Don't place a literal `*` adjacent to any URL or identifier in outbound Slack/Discord messages — Slack's mrkdwn renderer interprets `*foo*` as italic and silently breaks the clickable link.** Verified 2026-07-26, Slack `C0AH3RY3DK6/p1785122814.849319` (`github.com/$GITHUB_REPOSITORY/pull/8629*` rendered unclickable, agent had appended `*` after the URL). Defense in depth: `~/.claude/hooks/slack-trailing-asteri[REDACTED_OPENAI_KEY]` is registered as PreToolUse matcher `mcp__slack__conversations_add_message` in `~/.claude/settings.json` — strips adjacent `*` from any `https://github.com/...` substring before the message lands. Companion: SOUL.md `## COMMIT: no-trailing-asteri[REDACTED_OPENAI_KEY]` and the matching line in `~/.claude/CLAUDE.md` line 225. **Always verify your outbound `text` parameter contains no `*` adjacent to any `https://...` substring before posting, even with the hook in place.** The hook catches the most common case but does NOT strip leading bullet markers (`* foo https://...`) since those are legitimate lists.
