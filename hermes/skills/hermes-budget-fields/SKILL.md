...
```bash
# 1. Chat-agent turns
grep -n "agent:" /Users/$USER/.hermes_prod/config.yaml | head -3
grep -n "max_turns:" /Users/$USER/.hermes_prod/config.yaml | head -3

# 2. Delegation/subagent iterations
grep -n "delegation:" /Users/$USER/.hermes_prod/config.yaml | head -3
grep -n "max_iterations:" /Users/$USER/.hermes_prod/config.yaml | head -3

# 3. AO worker — should print NOTHING
grep -inE "iter|iteration" /Users/$USER/.openclaw/agent-orchestrator.yaml
```
