...
for repo in $GITHUB_REPOSITORY jleechanorg/agent-orchestrator jleechanorg/$ORG; do
...
1. **For each MERGEABLE PR surfaced in § B**, spawn an AO worker via `agento`:
   ...
   cd ~/.openclaw && env -i HOME="$HOME" \
     PATH="/Users/$USER/.nvm/versions/node/v22.22.0/bin:/Users/$USER/.local/bin:/Users/$USER/.bun/bin:/opt/homebrew/bin:/usr/bin:/bin" \
...
**Fix commit:** `8d6e888a4a` on `jleechanorg/$ORG` `origin/main`
...
- **Slack gateway enforces ~4000-char split** — even though `chat.postMessage` accepts 40k, the gateway/MCP layer splits at ~4000 chars into 2 messages. Trim the reply or post as 2 deliberately-partitioned messages. Use plain `[text](url)` syntax (Slack native), NOT `<url|label>` (gets mangled into `___URL_PLACEHOLDER____`). `~` in file paths gets stripped (use `/Users/$USER/.hermes` full path).
...
- **9am + 5pm PT** (launchd `StartCalendarInterval` is in LOCAL time, so use `Hour: 9` and `Hour: 17`). Mon-Fri cadence confirmed running as of 2026-06-26 (commit `77f4fe6` on `jleechanorg/$ORG`).
- **`~/.hermes/scripts/deploy.sh` Stage 4.6 syncs skills/, but `.claude/commands/` requires manual `cp`** — per `$ORG-slash-command-rollout` recipe.
...
- **`ao spawn` enforces single-spawn-per-project serialization.** Verified 2026-07-05 02:50Z — when 4 parallel `ao spawn -p $ORG --claim-pr <N>` calls are issued simultaneously, ALL but one return immediately with `✗ Another ao spawn is in progress for project "$ORG" (PID <X>, started <ts>). Wait for it to finish.` and exit code 1. The daemon has a single in-flight spawn slot per project. Implication for Step 8.5 #1: do NOT fan out `ao spawn` calls in parallel within a subagent — sequence them with a brief wait + `ao session ls` verification between each. Verified spawn cadence: ~5–15 s for the child `ao` process to fork, create the session row, and claim the PR. Full recipe + worktree collision recovery: `references/ao-spawn-serialization-2026-07-05.md`.
...
- **Skill verification:** `/roadmap` thin pointer (1847 bytes) + `skills/roadmap/SKILL.md` + `RESOLVER.md` merged at `df209445` on `jleechanorg/$ORG` `origin/main` (replaces 11,388-byte Phase-0 stub).
...
- `~/.hermes_prod/skills/$ORG-slash-command-rollout/references/$ORG-slash-command-rollout.md` — wiring recipe
