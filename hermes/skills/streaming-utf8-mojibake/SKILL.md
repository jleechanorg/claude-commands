...
In any file under `$PROJECT_ROOT/llm_providers/`, search for:
...
- `$PROJECT_ROOT/llm_providers/openrouter_provider.py:390`
- `$PROJECT_ROOT/llm_providers/openai_proxy_provider.py:391`

(As of 2026-06-04, both are still buggy.)

## How to fix

Replace `iter_lines(decode_unicode=True)` with `iter_lines()`. The downstream consumer in `$PROJECT_ROOT/llm_service.py:8029-8030` already does `chunk.decode("utf-8", errors="replace")` for bytes:
...
The safe pattern is already used in `$PROJECT_ROOT/llm_providers/openclaw_provider.py:291`:
...
Recovery is provably exact because each mojibake sequence corresponds to a unique original character. Walk all string fields in the document, do `str.replace` for each pair. Reference implementation: `$PROJECT_ROOT/repro/evidence/7248/recover_mojibake_entries.py` in PR #7249.

## Why tests miss this bug

The existing test mocks (`DummyStreamResponse.iter_lines` in `$PROJECT_ROOT/tests/test_openrouter_provider.py:31` and `MagicMock().iter_lines` in `$PROJECT_ROOT/tests/test_openai_inference_proxy.py:323`) **silently swallow** the `decode_unicode` flag and yield pre-decoded strings:
...
Reference repro: `$PROJECT_ROOT/repro/evidence/7248/repro_utf8_mojibake_streaming.py` in PR #7249.
...
- Issue: $GITHUB_REPOSITORY#7248
- PR: $GITHUB_REPOSITORY#7249
