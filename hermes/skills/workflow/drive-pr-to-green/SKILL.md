...
changelog:
  ...
  - 1.5.0 (2026-07-07) Added candidate-list tier classification trap pitfall + lite-green terminology clarification. Title keywords are not authoritative for tier; re-classify by gh pr view --json files against the project's PROD_PATH_PREFIXES. Verified 32-PR sweep on 2026-07-07 collapsed 32 keyword candidates to 14 strict-non-prod, only 1 ready. lite-green = 3 gates (CI green fail-closed on pending, mergeable, CR APPROVED). lite /er is not documented - clarify first. See references/nonprod-sweep-candidate-filtering-2026-07-07.md.
...
For trigger-style PRs (workflow files, CI changes) the bar is the same. For `$PROJECT_ROOT/` production code changes, AGENTS.md requires `/es` evidence — that's a separate gate, not part of "green" for this skill.

## Pitfall — candidate-list tier classification trap (added 2026-07-07, your-project.com 32-PR /green sweep)

When the user asks to /green a batch of "non prod / safe / logging only" PRs, the agent's instinct is to filter by PR title keywords (`ci:`, `docs:`, `test:`, `feat(telemetry)`, `chore(`, `refactor:`). **This is wrong.** Title keywords are not authoritative. A `chore(Dockerfile)` PR can touch `$PROJECT_ROOT/Dockerfile` and be PROD; a `feat(observability): BQ rate-limit telemetry` PR can include `$PROJECT_ROOT/bq_logging.py` and be PROD. The authoritative classifier is **per-file-path** against the project's `PROD_PATH_PREFIXES` tuple.

Verified incident: 32 keyword candidates on your-project.com → 17 had ≥1 file under `$PROJECT_ROOT/` or `testing_*/` after live `gh pr view --json files` re-classification. The script `green_merge_nonprod.py` (PR #8207) implements this gate and refuses to auto-merge prod PRs by design.

**Mandatory preflight before claiming any candidate list:**

1. Run `gh pr list` with title-keyword filter (cheap, broad).
2. For each candidate, fetch files: `gh pr view <N> --repo OWNER/REPO --json files --jq '[.files[].path] | .[]'`.
3. Re-classify against the project's authoritative tier rule. For your-project.com: `PROD_PATH_PREFIXES = ($PROJECT_ROOT/, testing_mcp/, testing_ui/)`. Other repos may differ — always check first.
4. The final sweep list is the intersection: keyword-match AND all-files-non-prod.
5. Surface BOTH buckets to the user — the "rejected as prod" count is itself a useful artifact.

**Lite-green terminology clarification:** "lite /er" (the user's phrasing on 2026-07-07) is NOT a documented term. The canonical term is **lite-green** = 3 gates (G1 CI green fail-closed on pending + G2 mergeable + G3 CodeRabbit APPROVED). `/er` is the evidence-review protocol, distinct from `/green`. If the user says "lite /er," clarify which they mean: (a) lite-green with no `/er` at all (docs/infra/tooling default), or (b) lite-green with a lightweight `/er` (skip Skeptic Gate 7 but run a quick evidence check).

Full recipe, verified outputs, and the candidate-bucket artifact template are at `references/nonprod-sweep-candidate-filtering-2026-07-07.md`.
