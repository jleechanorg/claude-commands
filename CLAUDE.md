...
7. 🚨 **SCREENSHOT LOCATION:** All screenshots must be saved to `docs/` directory
8. 🚨 **GITHUB TOOL PRIORITY:** GitHub MCP tools primary, `gh` CLI as fallback
9. 🚨 **SERENA MCP PRIORITY:** Serena MCP for semantic operations, standard file tools as fallback
10. 🚨 **FILE CREATION PREVENTION:** ⚠️ MANDATORY
    - ❌ FORBIDDEN: Creating `_v2`, `_new`, `_backup`, `_temp` files
    - ✅ REQUIRED CHECK: "Can I edit an existing file instead?"
11. 🚨 **HOOK REGISTRATION REQUIREMENT:** ⚠️ MANDATORY - ALL hooks MUST be registered
12. 🚨 **CROSS-PLATFORM COMPATIBILITY:** ⚠️ MANDATORY - ALL scripts and hooks must work on both macOS and Ubuntu
