...
# Layer 0 (unconditional): Suppress for jleechanorg/claude-commands sentinels (merged or not).
# Also suppress any PR where state=awaiting_merge_approval.
# This worktree is claude-commands-specific — hardcode the repo to prevent hook hot-loops.
for _sentinel in "$HOME/.tmp"/HOOK_cr_done_*.json "$HOME/.tmp"/HOOK_pr*_awaiting_merge_approval.json; do
  ...
  # Suppress all jleechanorg/claude-commands sentinels (merged or not) + any state=awaiting_merge_approval
  if [ "$_sentinel_repo" = "jleechanorg/claude-commands" ] || [ "$_sentinel_state" = "awaiting_merge_approval" ]; then
    echo "[UserPromptSubmit] PR #${_sentinel_pr} — unconditional sentinel suppress (repo=$_sentinel_repo state=$_sentinel_state)" >&2
    exit 0
  fi
done
