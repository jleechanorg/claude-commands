...
Dual-writes to:
  1. Qdrant (hermes_mem0 collection) — for semantic search via mem0_recall.py
...
def main() -> None:
    ...
    # Skip if mem0 cannot run (fail-open: never block stop hook on gate errors)
    try:
        if not mem0_hooks_enabled():
            sys.exit(0)
    except Exception as exc:
        print(f"[mem0_save] mem0_hooks_enabled failed: {exc}", file=sys.stderr)
        sys.exit(0)
