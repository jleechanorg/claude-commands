...
# Define early so it can be used by triage check below.
_worktree_suppress() {
    local repo_root
...
# ── Triage text suppression for NO-PR worktrees ─────────────────────────────────
# The triage block contains "[ -z \"{{pr_number}}\" ]" and "STEP 0: gh pr view".
# If this worktree has no PR (suppress sentinel), suppress the triage entirely
# so the hook injects nothing. Exit 0 = no injection.
_is_triage_text=false
# Use Python for reliable multiline-safe triage detection.
if printf '%s' "$input" | python3 -c "
import sys, re
text = sys.stdin.read()
# Match the triage block: [ -z \"{{pr_number}}\" ] ... idle exit
# OR the STEP 0 line with unsubstituted {{pr_number}}
if re.search(r'\[ -z.*?pr_number.*?\].*?idle exit|STEP 0: gh pr view.*?\{\{pr_number\}\}', text, re.DOTALL):
    sys.exit(0)
sys.exit(1)
" 2>/dev/null; then
    _is_triage_text=true
fi
if [[ "$_is_triage_text" == "true" ]] && _worktree_suppress; then
    exit 0
fi
...
if [[ -z "$PR_NUMBER" ]]; then
    # Only strip green-gate workflow/templates if the input actually contains them.
    # Guarding here prevents bypassing slash-command composition for normal prompts
    # (e.g. /think /debug) that have no PR context — those should still be composed.
    if echo "$input" | grep -qE '\{\{[^}]+\}\}|\(STEP -1|\(STEP 0'; then
...
print(text.strip())
")
        # Only output if content was actually modified; silent passthrough avoids UI banner
        if [[ "$output" != "$input" ]]; then
            echo "$output"
        fi
        exit 0
    fi
    # No template variables — fall through to normal slash-command composition
fi
...
# Return the output — only if modified to avoid showing banner for passthrough prompts
if [[ "$output" != "$input" ]]; then
    echo "$output"
fi
