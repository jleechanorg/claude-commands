...
class TestFirebaseCredentialScrubbing(unittest.TestCase):
    """
    Test Firebase credential scrubbing logic to ensure:
    1. WorldArchitect.AI Firebase credentials are scrubbed (user-specific, private)
    2. AI Universe Firebase credentials are NOT scrubbed (shared service, public)
    """
    ...
    def setUp(self):
        ...
        self.temp_dir = tempfile.mkdtemp(prefix='test_firebase_scrub_')
        self.project_root = os.path.join(self.temp_dir, 'test_project')
        self.export_dir = os.path.join(self.temp_dir, 'export')
    ...
    def test_worldarchitectai_firebase_key_is_scrubbed(self):
        """Test that WorldArchitect.AI Firebase API key is scrubbed from exports."""
        # WorldArchitect.AI Firebase credentials (should be scrubbed)
        test_content = """
        export FIREBASE_API_KEY="AIzaSyARs7IekRptvhZIwtV7lwJh3axWFsn_4c8"
        export FIREBASE_PROJECT_ID="worldarchitecture-ai"
        """

        test_file = os.path.join(self.project_root, '.claude', 'commands', 'test_worldai.sh')
        with open(test_file, 'w') as f:
            f.write(test_content)

        # Apply content filtering (modifies file in place)
        self.exporter._apply_content_filtering(test_file)
        ...
        # Verify WorldArchitect.AI Firebase API key is scrubbed
        self.assertNotIn('AIzaSyARs7IekRptvhZIwtV7lwJh3axWFsn_4c8', filtered_content,
                         "WorldArchitect.AI Firebase API key should be scrubbed")
        self.assertIn('<YOUR_WORLDAI_FIREBASE_API_KEY>', filtered_content,
                      "WorldArchitect.AI Firebase API key should be replaced with placeholder")
        # Note: worldarchitecture-ai project ID is scrubbed elsewhere in exportcommands.py (line 813)

    def test_ai_universe_firebase_key_not_scrubbed(self):
        """Test that AI Universe Firebase API key is NOT scrubbed (public shared service)."""
        # AI Universe Firebase credentials (should NOT be scrubbed - public by design)
        test_content = """
        export AI_UNIVERSE_FIREBASE_API_KEY="AIzaSyDWT4aEG2UoKEtTxozniGC6uPZi1fgjtG8"
        export AI_UNIVERSE_FIREBASE_PROJECT_ID="ai-universe-b3551"
        """

        test_file = os.path.join(self.project_root, '.claude', 'commands', 'test_aiuniverse.sh')
        with open(test_file, 'w') as f:
            f.write(test_content)
        ...
        # Verify AI Universe credentials are NOT scrubbed
        self.assertIn('AIzaSyDWT4aEG2UoKEtTxozniGC6uPZi1fgjtG8', filtered_content,
                      "AI Universe Firebase API key should NOT be scrubbed (public shared service)")
        self.assertIn('ai-universe-b3551', filtered_content,
                      "AI Universe project ID should NOT be scrubbed (public shared service)")

    def test_both_firebase_projects_together(self):
        """Test that both Firebase projects are handled correctly in the same file."""
        # Mix of both WorldArchitect.AI and AI Universe credentials
        test_content = """
        # WorldArchitect.AI (user-specific, private)
        export FIREBASE_API_KEY="AIzaSyARs7IekRptvhZIwtV7lwJh3axWFsn_4c8"
        export FIREBASE_PROJECT_ID="worldarchitecture-ai"

        # AI Universe (shared service, public)
        export AI_UNIVERSE_FIREBASE_API_KEY="AIzaSyDWT4aEG2UoKEtTxozniGC6uPZi1fgjtG8"
        export AI_UNIVERSE_FIREBASE_PROJECT_ID="ai-universe-b3551"
        """

        test_file = os.path.join(self.project_root, '.claude', 'commands', 'test_mixed.sh')
        with open(test_file, 'w') as f:
            f.write(test_content)
        ...
        # Verify WorldArchitect.AI credentials are scrubbed
        self.assertNotIn('AIzaSyARs7IekRptvhZIwtV7lwJh3axWFsn_4c8', filtered_content,
                         "WorldArchitect.AI Firebase API key should be scrubbed")
        self.assertIn('<YOUR_WORLDAI_FIREBASE_API_KEY>', filtered_content,
                      "WorldArchitect.AI Firebase API key should be replaced with placeholder")
        ...
        # Verify AI Universe credentials are NOT scrubbed
        self.assertIn('AIzaSyDWT4aEG2UoKEtTxozniGC6uPZi1fgjtG8', filtered_content,
                      "AI Universe Firebase API key should NOT be scrubbed")
        self.assertIn('ai-universe-b3551', filtered_content,
                      "AI Universe project ID should NOT be scrubbed")

    def test_no_false_positives(self):
        """Test that other Firebase-like keys are not accidentally scrubbed."""
        # Content with similar patterns but different keys
        test_content = """
        # Some other Firebase project (not WorldArchitect.AI or AI Universe)
        export OTHER_FIREBASE_API_KEY="AIzaSyBXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
        export OTHER_PROJECT_ID="some-other-project"
        """

        test_file = os.path.join(self.project_root, '.claude', 'commands', 'test_other.sh')
        with open(test_file, 'w') as f:
            f.write(test_content)
        ...
        # Verify we only scrub the specific keys, not all Firebase keys
        self.assertIn('AIzaSyBXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX', filtered_content,
                      "Other Firebase API keys should not be scrubbed (only specific projects)")
        self.assertIn('some-other-project', filtered_content,
                      "Other project IDs should not be scrubbed")
