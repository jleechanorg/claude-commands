...
try:
    ...
    sys.path.insert(0, str(Path(__file__).parent.parent.parent.parent / ".claude" / "commands"))
    from composition_hook import get_available_commands
...
class TestCommandCategorization(unittest.TestCase):
    ...
    def _scan_command_files(self):
        ...
        for py_file in self.commands_dir.glob("*.py"):
            if (not py_file.name.startswith("test_") and 
                py_file.name != "composition_hook.py" and
                py_file.name != "integration_example.py" and
    ...
    def test_no_stale_categorizations(self):
        ...
        allowed_missing = {
            ...
            "/clean", "/complete", "/deploy", "/minimal", "/paranoid", 
            "/performance", "/thorough", "/verbose"
        }
    ...
    def test_categorization_makes_sense(self):
        ...
        action_words = {
            "test", "deploy", "execute", "push", "integrate", "learn", 
            "review", "replicate", "coverage", "arch", "optimize",
            "handoff", "orchestrate", "copilot"
        }
        ...
        self.assertGreater(action_coverage, 0.3, 
        ...
        self.assertGreater(modifier_coverage, 0.4,
    ...
    def test_categorization_counts_reasonable(self):
        ...
        self.assertGreater(protocol_count, natural_count,
