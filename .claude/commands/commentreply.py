    ...
    if success:
        ...
        if "422" in (stderr or ""):
            ...
            if (
                ...
                or "does not exist" in (stderr or "").lower()
            ):
                print(
...
def main():
    ...
    for i, comment in enumerate(all_targets, 1):
        ...
        # For both top-level and reply comments, check if we've already replied to this comment
        replied_by_actor = any(
            (c.get("in_reply_to_id") == comment_id)
            and (
        ...
        if replied_by_actor:
            comment_type_for_skip = "reply" if in_reply_to_id else "comment"
            print(f"   ↪️ Skip: already replied to this {comment_type_for_skip} by current actor")
            already_replied += 1
            continue
        ...
        if comment_type == "review":
            ...
            if already_has_review_reply:
                print(
                    "   ↪️ Skip: review body already has issue comment reply from current actor"
                )
                already_replied += 1
                continue
    ...
    post_final_summary(
        owner, repo, pr_number, len(processed_comments), successful_replies, commit_hash
    )
