...
class OrchestrationIntegrationTest:
        ...
        while time.time() - start_time < max_monitor:
            ...
            if success and output != last_output:
                ...
                if any(indicator in output.lower() for indicator in [
                    'pr #', 'pull request', 'successfully created', 'merge'
                ]):
    ...
    def find_created_pr(self) -> Optional[int]:
        ...
        # List recent PRs
        success, output = self.run_command(
            ['gh', 'pr', 'list', '--limit', '5', '--json', 'number,title,author,createdAt'],
            "List recent PRs"
        )
        ...
        try:
            ...
            for pr in prs:
                ...
                created_time = datetime.fromisoformat(pr['createdAt'].replace('Z', '+00:00'))
                time_diff = (datetime.now(created_time.tzinfo) - created_time).total_seconds()
...
IMPORTANT: This is a test - work quickly and efficiently."""
    ...
    def cleanup_agent_and_pr(self) -> bool:
        ...
        if self.pr_number:
            success, _ = self.run_command(
                ['gh', 'pr', 'close', str(self.pr_number), '--comment', 
