    ...
    while attempt <= max_attempts:
        try:
            ...
            if should_retry and attempt < max_attempts:
                print(
                    f"⏱️ Arch review timeout (attempt {attempt}), retrying in {delay}s..."
                )
                time.sleep(delay)
                attempt += 1
                continue
            ...
            while attempt <= max_attempts:
                try:
                    ...
                    if "timeout" in str(e).lower() and attempt < max_attempts:
                        ...
                        if should_retry:
                            print(
                                f"⏱️ Gemini timeout (attempt {attempt}), retrying in {delay}s..."
                            )
                            time.sleep(delay)
                            attempt += 1
                            continue
...
def analyze_with_ultra_thinking(target: str, target_type: str) -> Dict[str, Any]:
    ...
    print("🧠 Initiating deep review with ultra thinking mode...")
