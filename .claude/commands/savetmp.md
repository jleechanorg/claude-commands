...
**This is NOT documentation – run every step below.**
...
# /savetmp – Structured Evidence Archiver

**Purpose**: Capture testing methodology, evidence summaries, and supporting artifacts in a predictable `/tmp` evidence package scoped by repo → branch → work item.
...
/tmp/<repo_name>/<branch>/<work_name>/<timestamp>/
  ├── methodology.md   # Testing methodology details (if provided)
  ├── evidence.md      # Evidence summary
  ├── notes.md         # Extra context / TODOs
  ├── artifacts/       # Copied files/directories passed via --artifact
  ├── metadata.json    # Machine-readable summary of the run
  └── README.md        # Quick overview + artifact list
```
...
- [ ] Command exits with status `0` and prints `✅ Evidence archived`.
...
- The most recent run path is printed at the end—share it when handing off the work item or writing the PR summary.
