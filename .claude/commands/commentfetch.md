...
Creates a JSON file at `/tmp/copilot-$(git rev-parse --abbrev-ref HEAD | tr '/\\' '_')/comments.json` (or specified output) with:
...
# Check results  
cat /tmp/copilot-$(git rev-parse --abbrev-ref HEAD | tr '/\\' '_')/comments.json | jq '.metadata'
```
