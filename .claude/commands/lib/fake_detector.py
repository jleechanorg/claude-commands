...
class FakeDetector:
    ...
    def analyze_code_string(self, code: str, context: str = "code") -> List[FakePattern]:
    ...
    def _check_text_patterns(self, content: str, location: str) -> List[FakePattern]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if re.search(r'#\s*TODO.*(?:implement|fix|replace|temporary|placeholder|fake|demo)', line_stripped, re.IGNORECASE):
                patterns.append(FakePattern(
            ...
            if re.search(r'(?:ready|complete|success).*(?:percent|%)\s*[:\=]\s*\d+', line_stripped, re.IGNORECASE):
                patterns.append(FakePattern(
    ...
    def _check_function_patterns(self, func_node: ast.FunctionDef, location: str) -> List[FakePattern]:
        ...
        if param_names:
            ...
            for param in param_names:
                if param not in func_source or func_source.count(param) <= 1:  # Only appears in definition
        ...
        if len(return_nodes) == 1 and isinstance(return_nodes[0].value, (ast.Dict, ast.List, ast.Constant)):
            patterns.append(FakePattern(
                type='hardcoded_return',
                severity='critical',
                location=f"{location}:{func_node.lineno}",
                description=f"Function '{func_node.name}' always returns the same hardcoded value",
                evidence=f"Function has single hardcoded return statement",
                suggestion="Make return value depend on function inputs and logic"
            ))
        
        return patterns
    
    def _check_dict_patterns(self, dict_node: ast.Dict, location: str) -> List[FakePattern]:
        ...
        if hasattr(dict_node, 'keys'):
            ...
            fake_indicators = ['production_ready_percent', 'success_probability', 'verdict', 'summary']
            suspicious_keys = [k for k in key_values if any(indicator in k for indicator in fake_indicators)]
    ...
    def analyze_function_behavior(self, func, test_inputs: List[Any]) -> List[FakePattern]:
        ...
        try:
            ...
            if len(set(str(r) for r in results)) == 1:
                patterns.append(FakePattern(
                    ...
                    severity='critical',
                    location=func.__name__ if hasattr(func, '__name__') else 'unknown',
    ...
    def generate_report(self, patterns: List[FakePattern]) -> str:
        ...
        for severity in ['critical', 'high', 'medium', 'low']:
            if severity in by_severity:
                report.append(f"### {severity.upper()} ISSUES ({len(by_severity[severity])})")
