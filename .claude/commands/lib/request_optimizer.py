...
class RequestSizeEstimator:
    ...
    def calculate_multiedit_size(self, edits: List[Dict]) -> Tuple[int, bool]:
        ...
        needs_split = len(edits) > self.MAX_MULTIEDIT_OPS or total_size > self.MAX_CONTEXT_SIZE
        return total_size, needs_split
    
    def estimate_tool_request_size(self, tools: List[Dict], context: str = "") -> Tuple[int, str]:
    ...
    def split_multiedit(self, file_path: str, edits: List[Dict]) -> List[Tuple[str, List[Dict]]]:
        ...
        for edit in edits:
            edit_size = len(edit.get('old_string', '')) + len(edit.get('new_string', ''))
            
            if (len(current_chunk) >= self.MAX_MULTIEDIT_OPS or 
                current_size + edit_size > self.MAX_CONTEXT_SIZE * 0.7):
...
class TimeoutHandler:
    ...
    def record_timeout(self, operation_type: str):
        ...
        self.timeout_counts[operation_type] = self.timeout_counts.get(operation_type, 0) + 1
...
class RequestBatcher:
    ...
    def can_batch_together(self, op1: Dict, op2: Dict) -> bool:
        ...
        if (op1['type'] in ['read', 'edit'] and 
            op2['type'] in ['read', 'edit'] and
            op1['data'].get('file_path') == op2['data'].get('file_path')):
            return True
        
        # Analysis operations can be batched
        if (op1['type'] in ['analyze', 'review'] and 
            op2['type'] in ['analyze', 'review']):
            return True
...
class RequestOptimizer:
    ...
    def optimize_multiedit(self, file_path: str, edits: List[Dict]) -> List[Tuple[str, List[Dict]]]:
        """Optimize MultiEdit request"""
        return self.size_estimator.split_multiedit(file_path, edits)
    
    def check_request_size(self, tools: List[Dict], context: str = "") -> Tuple[bool, str]:
        ...
        if status == "split_required":
            return False, f"Request too large ({size} chars). Split into smaller requests."
        elif status == "warning":
    ...
    def get_optimization_report(self) -> str:
        ...
        if self.metrics:
            avg_duration = sum(m.duration_ms for m in self.metrics[-10:]) / min(len(self.metrics), 10)
            avg_size = sum(m.size_chars for m in self.metrics[-10:]) / min(len(self.metrics), 10)
...
def optimize_multiedit(file_path: str, edits: List[Dict]) -> List[Tuple[str, List[Dict]]]:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("Request Optimizer - Use import for functionality or pass any arg for report")