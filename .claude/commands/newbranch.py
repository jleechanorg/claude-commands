...
def check_uncommitted_changes():
    ...
    stdout, stderr, returncode = run_command(["git", "status", "--porcelain"], check=False)
...
def main():
    ...
    if check_uncommitted_changes():
        print("❌ ERROR: You have uncommitted changes. Please commit or stash them before creating a new branch.")
        print("\nRun: git status")
        return 1
    ...
    if returncode != 0:
        print(f"❌ ERROR: Failed to switch to main branch")
    ...
    if returncode != 0:
        print(f"❌ ERROR: Failed to pull from origin/main")
        print(f"stdout: {stdout}")
        print(f"stderr: {stderr}")
        return 1
    ...
    stdout, stderr, returncode = run_command(["git", "checkout", "-b", branch_name], check=False)
    if returncode != 0:
    ...
    stdout, stderr, returncode = run_command(["git", "push", "-u", "origin", branch_name], check=False)
    if returncode != 0:
        print(f"⚠️  Warning: Failed to set upstream tracking (this is usually okay)")
        print(f"stderr: {stderr}")
    
    print(f"✅ Successfully created and switched to branch: {branch_name}")
    print(f"📋 Branch is based on latest main and ready for development")
