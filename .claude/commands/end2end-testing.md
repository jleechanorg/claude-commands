...
4. **Add or update a test now.** Before reporting back: name the exact test
   file path touched and the specific assertion added or changed. Running
   an existing suite and reporting "N passed" does NOT satisfy this step —
   see "Tests Are Mandatory, Not Optional" in the skill file.
5. **If the test needs a fake LLM response fixture** (`$PROJECT_ROOT/mocks/*`
   only — never `testing_mcp/`/`testing_ui/`, which stay real-services-only):
   source it from real BigQuery traffic per "Realistic Fake LLM Responses
   Must Come From Real BQ Traffic" in the skill file, scrub PII
   (`user_id`, player names, emails — `campaign_id` may be kept), and
   verify field completeness against the agent's real request/response
   shape before committing the fixture.
...
## Tests Are Mandatory + BQ-Sourced Fixtures

- **Tests Are Mandatory, Not Optional**: every invocation must add or update a
  test — see the skill file for the full anti-pattern definition.
- **Realistic Fake LLM Responses Must Come From Real BQ Traffic**: `$PROJECT_ROOT/mocks/*`
  fixtures must be sourced from real `worldarchitecture-ai.llm_forensics.llm_payloads`
  traffic, PII-scrubbed, and field-complete — see the skill file for the
  verified recipe, the three measurement traps, and a worked example. This
  does not apply to `testing_mcp/`/`testing_ui/`.
