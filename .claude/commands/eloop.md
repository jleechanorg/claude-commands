...
**1d. Merged-PR zombie sweep** — kill workers burning tokens on already-merged PRs:
```bash
...
- `~/.openclaw/agent-orchestrator.yaml` — are agentRules and reactions configured correctly?
...
**6c. Pre-merge 6-green verification** (MANDATORY before ANY merge):
```bash
# Before merging ANY PR, verify 6-green. NEVER skip this.
PR_NUM=NNN; REPO="jleechanorg/REPO"
echo "=== PRE-MERGE 6-GREEN CHECK PR #$PR_NUM ==="
...
# Gate 3: CR APPROVED (actionable reviews only)
...
# BLOCK if not green
if [ "$CI" != "success" ] || [ "$MERGEABLE" = "dirty" ] || [ "$CR" != "APPROVED" ] || [ "$UNRESOLVED" != "0" ]; then
  echo "  *** BLOCKED — NOT 6-GREEN. DO NOT MERGE. ***"
  # DO NOT proceed to merge. Report blockers and move on.
else
  echo "  *** 6-GREEN — safe to merge ***"
fi
```
**NEVER merge a PR that fails this check.** If a PR is not 6-green, dispatch a worker to fix it — do not merge it to "show progress."

**6d. REST API fallback** — always prefer REST over GraphQL:
```bash
# Merge via REST (ONLY after 6c verification passes)
