...
grep -rE "^[^#]*\$HOME/|/Users/\$USER/projects/worktree_ralph|projects_other/ralph|orch_worldai_ralph|worldai_genesis2|worldai_ralph2" .claude/commands --include="*.md" --include="*.py" --include="*.sh" 2>/dev/null | grep -v '\$HOME/' | head -5
echo "   ... ($(grep -rE "^[^#]*\$HOME/|/Users/\$USER/projects/worktree_ralph|projects_other/ralph|orch_worldai_ralph|worldai_genesis2|worldai_ralph2" .claude/commands --include="*.md" --include="*.py" --include="*.sh" 2>/dev/null | grep -v '\$HOME/' | wc -l) total matches)"
