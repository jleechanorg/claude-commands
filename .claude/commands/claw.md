...
---"
  ...
  if [ -n "$ISSUE_ID" ]; then
    ...
    ao spawn "$ISSUE_ID" -p "$PROJECT_ID" 2>&1 | tee "$SPAWN_OUTPUT_FILE"
...
' 2>/dev/null || true)
  ...
  if [ -n "$SESSION_NAME" ]; then
    ...
    # --no-wait: fire and return; the session processes asynchronously
    ao send "$SESSION_NAME" --file "$TASK_FILE" --no-wait 2>&1
    SEND_EXIT=$?
