...
class PerCommentCache:
    ...
    def mark_handled(self, comment_id: str, reply_id: str, status: str = "replied") -> None:
        """Mark a comment as handled after successful reply.

        Mutates the cache immediately after a successful GitHub reply post.
        This enables resumable workflows - if processing stops mid-way,
        already-handled comments won't be reprocessed.

        Args:
            comment_id: Original comment ID that was replied to
            reply_id: ID of our reply on GitHub
            status: Handler status - "replied", "acknowledged", or "skipped"
        """
        comment = self.load_comment(comment_id)
        if comment is None:
            # Comment doesn't exist in cache - nothing to mark
            return
    ...
    def is_handled(self, comment_id: str) -> bool:
        """Check if a comment has been handled.

        Args:
            comment_id: Comment ID to check

        Returns:
            True if comment exists and has been handled, False otherwise
        """
        comment = self.load_comment(comment_id)
        if comment is None:
            return False

        handled = comment.get("handled")
        if not isinstance(handled, dict):
            return False

        return handled.get("status") is not None

    def get_unhandled_comments(self) -> List[Dict[str, Any]]:
        """Get all comments that need processing.

        Filters to comments that:
        1. Have requires_response=True
        2. Have NOT been marked as handled

        This is the core method for the "always fetch fresh, skip handled" pattern.

        Returns:
            List of unhandled comments that need responses
        """
        all_comments = self.load_all_comments()
        unhandled = []

        for comment in all_comments:
            # Skip comments that don't need responses
            if not comment.get("requires_response", False):
                continue

            # Skip already handled comments (with type guard for corrupted cache)
            handled = comment.get("handled")
            if isinstance(handled, dict) and handled.get("status") is not None:
                continue
