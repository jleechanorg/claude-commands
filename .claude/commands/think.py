...
def detect_fake_in_problem(problem: str) -> List[FakePattern]:
    ...
    fake_keywords = [
        'fake', 'demo', 'simulation', 'mock', 'placeholder', 
        'hardcoded', 'todo', 'unfinished', 'not implemented'
    ]
...
def format_thinking_prompt(level: str, problem: str, fake_patterns: List[FakePattern]) -> str:
    ...
    if fake_patterns:
        ...
        for pattern in fake_patterns[:5]:  # Show top 5
            base_prompt += f"- {pattern.type}: {pattern.description} ({pattern.location})\n"
...
def main():
    ...
    print("=" * 60)
    print()
    print("💡 In Claude Code, this would trigger the mcp__sequential-thinking__sequentialthinking tool")
