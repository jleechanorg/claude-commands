...
**PREFER MCPTestBase**: For E2E tests, ALWAYS inherit from `MCPTestBase` class instead of writing standalone scripts.
...
# For testing_mcp/test_*.py: use parent.parent (2 levels)
# For testing_mcp/subdir/test_*.py: use parent.parent.parent (3 levels)
...
### When to Use MCPTestBase vs Standalone

| Test Type | Use MCPTestBase? |
|-----------|------------------|
| E2E tests with MCP server | ✅ YES - Always |
| Multi-scenario tests | ✅ YES - Built-in iteration |
| Single API call tests | ✅ YES - Simpler code |
| Unit tests (no server) | ❌ NO - Use pytest |
| Browser-only tests | ❌ NO - Use Playwright directly |
