...
if [[ ! -d "${HOME}/projects/worktree_ralph" ]]; then
    echo "❌ ERROR: Source repository not found at ${HOME}/projects/worktree_ralph"
    exit 1
fi
echo "✅ Source repository found"

# Verify credentials
if [[ ! -f "${HOME}/projects/worktree_ralph/testing_http/testing_full/.env" ]]; then
...
if [[ ! -f "${HOME}/projects/worktree_ralph/roadmap/mvp_site_typescript_migration_eng_design.md" ]]; then
...
# Verify test cases
TEST_CASE_COUNT=$(find ${HOME}/projects/worktree_ralph/testing_llm -name "*.md" 2>/dev/null | wc -l)
...
if [[ ! -d "${HOME}/projects_other/ralph-orchestrator" ]]; then
    echo "❌ ERROR: Ralph not found at ${HOME}/projects_other/ralph-orchestrator"
    exit 1
fi
echo "✅ Ralph installation found"

# Verify Genesis installation
if [[ ! -f "${HOME}/projects/worktree_ralph/genesis/genesis.py" ]]; then
    echo "❌ ERROR: Genesis not found at ${HOME}/projects/worktree_ralph/genesis/genesis.py"
    exit 1
fi
echo "✅ Genesis installation found"

# Verify benchmark goal files
if [[ ! -f "${HOME}/projects/worktree_ralph/roadmap/genesis_typescript_migration_benchmark.md" ]]; then
    echo "❌ ERROR: Genesis benchmark goal file not found"
    exit 1
fi
if [[ ! -f "${HOME}/projects/worktree_ralph/roadmap/ralph_typescript_migration_benchmark.md" ]]; then
...
if [[ -d "${HOME}/projects_other/worldai_genesis2" ]]; then
    echo "❌ ERROR: worldai_genesis2 already exists - remove before starting benchmark"
    echo "   Run: rm -rf ${HOME}/projects_other/worldai_genesis2"
    exit 1
fi
if [[ -d "${HOME}/projects_other/worldai_ralph2" ]]; then
    echo "❌ ERROR: worldai_ralph2 already exists - remove before starting benchmark"
    echo "   Run: rm -rf ${HOME}/projects_other/worldai_ralph2"
    exit 1
fi
...
echo ""
echo "📋 Configuration:"
echo "  Source: ${HOME}/projects/worktree_ralph/$PROJECT_ROOT/"
echo "  Genesis Target: ${HOME}/projects_other/worldai_genesis2"
echo "  Ralph Target: ${HOME}/projects_other/worldai_ralph2"
...
GENESIS_DIR="${HOME}/projects_other/worldai_genesis2"
...
cd ${HOME}/projects/worktree_ralph
...
RALPH_DIR="${HOME}/projects_other/worldai_ralph2"
...
# Execute Ralph
cd ${HOME}/projects_other/ralph-orchestrator

python -m ralph_orchestrator \
    ${HOME}/projects/worktree_ralph/roadmap/ralph_typescript_migration_benchmark.md \
    --agent codex \
    --max-iterations 50 \
    --verbose \
