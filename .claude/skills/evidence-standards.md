...
treat an agy-backed judgment run as "mocked" or insufficient. Provider execution
policy is owned by `testing_mcp/CLAUDE.md`: evidence runs leave AGY enabled and
include successful raw AGY request/response captures. If a claim is specific to a
provider capability AGY does not exercise, state that limit as something the bundle
does **not** prove and use separately governed production or preview evidence; do not
weaken the `testing_mcp/` contract.
...
| A `testing_mcp/test_*.py` test that runs against a real local server + real LLM and FAILS pre-fix for the same reason as prod | **YES** (full Layer 2 red proof) | This is the gold standard and is sufficient on its own. Follow `testing_mcp/CLAUDE.md` without provider opt-outs. |
