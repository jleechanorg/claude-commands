...
**5-minute checkpoint cadence (MANDATORY for the sidekick supervisor AND every
lane owner):** at least once every ≤5 min the supervisor AND each lane owner
MUST (1) append a timestamped heartbeat + current phase to STATE.md, (2)
`br update <P1-resumption-bead-id> --status in_progress --append "heartbeat
<ISO ts> phase=<phase> next=<one-liner>"` (the P1 resumption bead is the
project's durable recovery pointer — see "Bead runbook" below; one writer per
bead, see rule below), then `br sync` to flush the DB↔JSONL, and (3) make a
local commit of state so a crash loses ≤5 min. The 5-min target is best-effort
and scoped to a LIVE session/watchdog — STATE.md in `/tmp` does NOT survive
host reboot, `/tmp` cleanup, or the supervisor process dying (the team-claude
teammate itself dies with the parent CLI per the swarm "Known limitation"
section), so absolute durability still requires the `br` P1 bead + an external
AO/launchd supervisor. Drive the cadence with `CronCreate`, a background
`tmux respawn-pane -k -t $SESSION \\;` heartbeat loop that re-keys the
checkpoint commands, or an `ao spawn`-managed supervisor — `tmux respawn-pane`
WITHOUT `-k` only reactivates a pane whose command already exited and is not a
timer, so do not lean on it alone; a heartbeat that only prints to chat is NOT
a checkpoint. **Single-writer rule:** designate ONE owner per bead (typically
the lead lane); other lanes append Progress Log to STATE.md only and never
touch the bead body. **Commit-safety (anti-clobber):** if the working tree
carries unrelated staged/modified work (another actor's diff, a staged
deletion), NEVER run `git commit` / `git add -A` in that worktree. Route the
commit to (a) an **isolated state repo** (`git init` a
`.tmp/<mission>-state-repo/`, copy STATE/HANDOFF files in, commit there) or (b)
a dedicated WIP branch with path-scoped `git add -- <state paths>`. State files
under a gitignored `.tmp/` need the isolated-repo (or `git add -f`) path since
a normal add skips them. **Non-repo missions:** `br` and the in-repo commit
route are not available — fall back to the `/tmp/<project-slug>/sidekick/<branch>/`
plain-file path and the bead runbook's fallback clause.
