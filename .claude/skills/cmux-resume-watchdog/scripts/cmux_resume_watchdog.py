...
STRUCTURAL_FAILURE_RE = re.compile(
    r"^\s*[⏺●⎿›»•·]\s|HTTP\s+\d{3}|\bcode\s+\d{3}\b|\"type\"\s*:\s*\"error\"|API Error:|Request rejected|overloaded|rate_limit|stalled mid-stream|idle timeout|fetch failed|connection failed|connection reset|RESOURCE_EXHAUSTED|Too many requests|hit your|network issue connecting|agent execution terminated due to error",
    re.I | re.M,
)
...
QUICK_NETWORK_HINT_RE = re.compile(
    r"(?im)(?:"
    r"there was a network issue connecting to the server|"
...
def classify_screen(
    ...
    if QUICK_NETWORK_HINT_RE.search(screen):
        return Decision("network", FASTEMBED_ACTION_THRESHOLD, "chrome", True, "WOULD_RESUME")
    ...
    if not confident_clear and not confident_stall:
        if structural_evidence and re.search(
            r"Token Plan|usage limit|429|quota|rate limit|resource.*exhausted|exceeded.*limit|account.*exhausted|overloaded|output token maximum|too many requests|hit your|402|insufficient credits",
            text,
            re.I,
        ):
            label = "quota"
            path = "structural"
        elif structural_evidence and re.search(
            r"empty or malformed|proxy or gateway|connection|connect|network|socket|econnreset|enotfound|etimedout|econnrefused|502|503|504|server error|fetch failed|api error|stalled mid-stream|idle timeout|529|connection closed|2064|network issue|agent execution terminated",
            text,
            re.I,
        ):
            label = "network"
            path = "structural"
        elif llm_predict is not None and (structural_evidence or score >= 0.45):
...
def list_terminal_surfaces(socket_path: str) -> list[Surface]:
    """Enumerate every terminal surface on the socket.

    Tries fast single RPC via ``cmux_surface_utils.iter_terminal_surfaces()``
    first, falling back to ``cmux tree --all --json``.
    """
    try:
        from cmux_surface_utils import iter_terminal_surfaces
        surfaces = [
            Surface(socket_path, ws_ref, surface_ref, title)
            for ws_ref, surface_ref, title in iter_terminal_surfaces(socket_path=socket_path)
        ]
        if surfaces:
            return surfaces
    except Exception:
