...
SCRIPT = REPO_ROOT / "scripts" / "cmux_resume_watchdog.py"
...
def test_ambiguous_fastembed_result_uses_structural_before_llm():
    watchdog = load_module()
    ...
    assert decision.kind == "quota"
    assert decision.path == "structural"
...
def test_gemini_network_issue_connecting_classified_as_network():
    """Regression: Gemini CLI 'network issue connecting to the server' must resume."""
    watchdog = load_module()
    screen = (
        "⚠ Agent execution terminated due to error.\n"
        "Error ID: 62816115-7ecf-4108-8ddd-f41a3a7a8bff-3149\n"
        "\n"
        "⚠ There was a network issue connecting to the server, please try again.\n"
        "Error ID: 62816115-7ecf-4108-8ddd-f41a3a7a8bff-3156\n"
        "\n"
        "❯ \n"
    )
    decision = watchdog.classify_screen(screen, lambda _: ("clear", 0.73), None)
    assert decision.eligible, f"screen should be eligible, got {decision}"
    assert decision.kind == "network", f"kind should be network, got {decision}"
    assert decision.action == "WOULD_RESUME"
    assert decision.path == "chrome"
