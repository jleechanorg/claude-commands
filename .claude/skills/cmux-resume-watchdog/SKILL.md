...
A single launchd daemon (`com.localhost.cmux-resume-watchdog`) that watches
...
| `com.localhost.cmux-resume-watchdog.plist.template` | launchd job template with `@HOME@` placeholder. |
...
launchctl print "gui/$(id -u)/com.localhost.cmux-resume-watchdog" | head -20
...
the skill from the $GITHUB_REPOSITORY worktree (the export's Source Surface 2
is `$GITHUB_REPOSITORY/.claude/`).

Re-export flow:
1. Update `$GITHUB_REPOSITORY` first.
2. From `$GITHUB_REPOSITORY` (or its export worktree), copy the new
   `scripts/cmux_resume_watchdog.py` + `tests/test_cmux_resume_watchdog.py`
   into `$GITHUB_REPOSITORY/.claude/skills/cmux-resume-watchdog/`.
