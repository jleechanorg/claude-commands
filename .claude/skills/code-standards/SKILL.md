...
2. Dispatch three adversarial, independent review lanes every time:
   `/zfc`, `/zfclevel`, and `/root-cause-first`.
...
5. Require every lane to report PASS/WARN/FAIL, evidence, required fixes, and
   any N/A caveats. A lane may return N/A for a subsection only if the source
   skill permits that; the lane itself still runs.
6. Reconcile the lane results into one practical report. Do not dilute a FAIL
...
mark any of the three lanes as skipped.
