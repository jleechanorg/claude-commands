...
- Oracle CLI: Use `oracle --engine browser` (default without API key) with tasks such as `--task architecture`, `--task ai_debug --notes tmp/bug-report.md`, `--task diff_review`, or `--task ui_debug --notes tmp/ui-bug.md`.
