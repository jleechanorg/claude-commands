...
- Oracle CLI: `source scripts/oracle_helpers.sh` then use:
  - `oracle_arch_preview` (dry-run bundle)
  - `oracle_arch` (architecture review)
  - `oracle_ai_debug` (AI pipeline bug with optional report path)
  - `oracle_diff_review` (reviews `git diff`)
  - `oracle_ui_debug` (frontend triage)
