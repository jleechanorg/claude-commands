...
```bash
bash .claude/skills/host-disk-guardian/scripts/host-disk-guardian.sh                # check + clean if critical
bash .claude/skills/host-disk-guardian/scripts/host-disk-guardian.sh --dry-run       # report only, never delete
```
...
field, or a failed `curl` all degrade to a log line; the disk-monitoring/auto-clean logic never
...
sed -e "s|@HOME@|$HOME|g" \
  ...
  .claude/skills/host-disk-guardian/install/org.jleechanorg.host-disk-guardian.plist.template \
  > ~/Library/LaunchAgents/org.jleechanorg.host-disk-guardian.plist
launchctl load ~/Library/LaunchAgents/org.jleechanorg.host-disk-guardian.plist
```

Remove:
```bash
launchctl unload ~/Library/LaunchAgents/org.jleechanorg.host-disk-guardian.plist
rm ~/Library/LaunchAgents/org.jleechanorg.host-disk-guardian.plist
```
