...
- **Did I audit every existing consumer of a shared mechanism before extending it?** This generalizes beyond regex/heuristics: exception types, decorators, base classes, error codes, shared config keys, or any other reused mechanism silently carries whatever behavior/severity policy its *existing* consumers already established. Adding a new producer (a new `raise SharedExceptionType`, a new caller of a shared decorator, a new writer to a shared config key) without checking what *already* consumes that mechanism means you inherit someone else's policy sight-unseen — possibly a stale or narrow one from a removed feature. Before extending a shared mechanism, enumerate its existing consumers/catch-sites and confirm your new use case's severity actually matches theirs, or document the divergence explicitly. Incident: a validator reused an exception class whose sole existing catch site had been left in a hard-fail (`re-raise`) state by an unrelated feature removal two months earlier; the new validator fired far more often than the original use case ever did, turning a dormant landmine into a live production incident, because nobody checked what the exception type's existing handler actually did before raising it from a new call site.
