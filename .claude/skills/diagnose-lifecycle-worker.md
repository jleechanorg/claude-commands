...
```bash
pgrep -af "lifecycle-worker" | grep -v grep
```
...
git branch -d <branch-name>   # safe if merged upstream
git branch -D <branch-name>   # force if not yet merged
```

**Verify the fix works:**
```bash
git fetch --force origin +refs/pull/<PR_NUMBER>/head:<branch-name>
```
...
```bash
source ~/.profile  # load SLACK_USER_TOKEN
curl -s -X POST "https://slack.com/api/chat.postMessage" \
  -H "Authorization: Bearer $SLACK_USER_TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"channel\": \"$JLEECHAN_DM_CHANNEL\", \"text\": \"*[AO Alert]* lifecycle-worker stalled on <project> PR #<N>. 3+ consecutive failures. Manual intervention required.\"}"
```
