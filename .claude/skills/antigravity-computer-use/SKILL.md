...
arr = np.array(img)
h, w = arr.shape[:2]
for row_start in range(h//2, h, 20):
...
**Why**: Allow dialogs are web-rendered (not in A11y tree). They block ALL agent progress silently. The agent shows `progress_activity` spinner but never executes commands. This was a real miss on 2026-04-03 — the agent was stalled until a human flagged it.
