...
**Prefer the Agent Manager** for all orchestration (new conversations, monitoring, steering). Use the **Editor's Cmd+L panel** as a fallback when:
...
```bash
# Step 0a: Try Agent Manager first
...
3. **Max 3 parallel conversations** — don't start 4th until one finishes or fails
...
2. **Check for errors** — look for "Too much traffic", "Error", "Failed" in conversation content
3. **Retry on error** — if error detected, click Retry or restart conversation with same task
4. **After 10 retries** — if same error persists, switch model (start new conversation with different model)
5. **Max 3 parallel** — ensure no more than 3 active conversations at once
6. **Report status** — document which conversations are running, which PRs they're working on
