...
## Safe Venv Deduplication & Backup Cleanup

Use `symlink-shared-venvs.sh` and `init_shared_venv.sh` from `disk-magician` to reclaim space from duplicate per-worktree `.venv` directories without breaking Python isolated environments:

```bash
# 1. Dry-run scan across all worktree roots (~/.worktrees, ~/projects, ~/project_agento, ~/projects_other)
./scripts/symlink-shared-venvs.sh --dry-run

# 2. Reclaim space by deduplicating worktree venvs & purging backups >24h
./scripts/symlink-shared-venvs.sh --clean --purge-bak-days 1

# 3. Initialize a central shared venv for any repo (~/.venvs/<repo_name>/venv)
./scripts/init_shared_venv.sh /path/to/repo
```
