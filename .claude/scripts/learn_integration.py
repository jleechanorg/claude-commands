...
class LearnCommandIntegration:
    ...
    def execute_learn(self, context: Optional[str] = None) -> Dict:
    ...
    def _generate_insights(self, patterns: List[Pattern]) -> List[str]:
        ...
        # Pattern frequency
        if len(patterns) > 3:
            insights.append(f"High correction frequency detected - {len(patterns)} patterns in one interaction")
            
        # Confidence analysis
        high_confidence = [p for p in patterns if p.confidence >= 0.8]
        if high_confidence:
            insights.append(f"{len(high_confidence)} high-confidence patterns identified")
            
        # Category distribution
        categories = {}
        for pattern in patterns:
            categories[pattern.category.value] = categories.get(pattern.category.value, 0) + 1
    ...
    def _analyze_meta_patterns(self) -> List[str]:
        ...
        # Learning velocity
        if stats["total"] > 10:
            meta_insights.append(f"Knowledge base growing: {stats['total']} total patterns")
        ...
        if stats["by_category"]:
            ...
            if max_cat > min_cat * 3:
                meta_insights.append("Pattern imbalance detected - some categories underrepresented")
        ...
        if total_confidence > 0:
            ...
            if high_ratio < 0.3:
                meta_insights.append("Many low-confidence patterns - more validation needed")
            elif high_ratio > 0.7:
                meta_insights.append("Strong pattern confidence - predictions should be accurate")
