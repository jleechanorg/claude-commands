...
from pattern_extractor import Pattern, PatternCategory, Confidence
...
"""
    ...
    def _find_section(self, category: PatternCategory) -> Optional[Dict]:
        ...
        return {
            ...
            "content": self.content[section_start:next_section if next_section != -1 else len(self.content)]
        }
    
    def _find_existing_pattern(self, name: str, section: Dict) -> Optional[Dict]:
    ...
    def _format_pattern(self, pattern: Pattern) -> str:
        ...
        if pattern.evidence:
            ...
            for evidence in pattern.evidence[:3]:  # Limit to 3 most recent
                formatted += f"- {evidence[:100]}{'...' if len(evidence) > 100 else ''}\n"
        ...
        if pattern.auto_apply:
            formatted += f"**Auto-Apply**: ✅ Enabled\n"
            
        formatted += f"**Last Seen**: {pattern.last_seen.strftime('%Y-%m-%d %H:%M')}\n\n"
    ...
    def _add_new_pattern(self, pattern: Pattern, section: Dict):
        ...
        if next_heading == -1:
            ...
            comment_marker = section["content"].find("<!-- Auto-populated", insert_point)
            if comment_marker != -1:
                insert_point = section["content"].rfind("\n", insert_point, comment_marker) + 1
        else:
    ...
    def _update_existing_pattern(self, pattern: Pattern, existing: Dict):
        ...
        next_section = self.content.find("\n## ", pattern_start + 1)
        pattern_end = min(
            x for x in [next_pattern, next_section, len(self.content)] 
            if x != -1
        )
    ...
    def _update_metadata(self, pattern: Pattern):
        ...
        timestamp_replacement = f"*Last Updated: {datetime.now().strftime('%Y-%m-%d %H:%M')}*"
        ...
        if recently_added != -1:
            ...
            entries = [e.strip() for e in current_entries.split("\n") if e.strip() and not e.startswith("<!--")]
    ...
    def get_statistics(self) -> Dict:
        ...
        # Count by confidence
        stats["by_confidence"]["high"] = len(re.findall(r"🟢", self.content))
        stats["by_confidence"]["medium"] = len(re.findall(r"🟡", self.content))
        stats["by_confidence"]["low"] = len(re.findall(r"🔴", self.content))
