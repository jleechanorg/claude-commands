...
class MemoryMCPIntegration:
    ...
    def __init__(self):
        self.local_fallback_file = Path.home() / ".cache" / "claude-learning" / "memory_mcp_fallback.json"
    ...
    def store_correction_in_memory_mcp(self, correction: Dict, conversation_context: Optional[Dict] = None) -> bool:
    ...
    def _store_in_memory_mcp(self, correction: Dict, conversation_context: Optional[Dict] = None) -> bool:
        ...
        if conversation_context:
            observations.append(f"Conversation context: {json.dumps(conversation_context)}")
    ...
    def _store_in_local_fallback(self, correction: Dict, conversation_context: Optional[Dict] = None) -> bool:
...
class AutoCorrectionProcessor:
    ...
    def process_message_for_learning(self, message: str, conversation_context: Optional[Dict] = None) -> Dict:
        """Complete pipeline: detect corrections and store in memory automatically"""
        
        # Step 1: Detect corrections
        detection_result = self.detector_pipeline.process_user_message(message, conversation_context)
        
        # Step 2: Store in Memory MCP
        storage_results = []
        for correction in detection_result['corrections_detected']:
            success = self.memory_integration.store_correction_in_memory_mcp(correction, conversation_context)
    ...
    def get_learning_statistics(self) -> Dict:
        ...
        return {
            ...
            'recent': len([c for c in corrections if 'created' in c and 
                          (datetime.now() - datetime.fromisoformat(c['created'])).days < 7])
        }
