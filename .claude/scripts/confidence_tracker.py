...
class ConfidenceTracker:
    ...
    def __init__(self):
        self.feedback_patterns = {
            'positive': [
                r'good', r'great', r'perfect', r'excellent', r'thanks', 
                r'correct', r'right', r'yes', r'exactly', r'nice'
            ],
            'negative': [
                r'wrong', r'incorrect', r'no', r'actually', r'don\'t',
                r'error', r'mistake', r'fix', r'change', r'instead'
            ],
            'correction': [
                r'don\'t.*do', r'use.*instead', r'actually.*should',
                r'prefer.*not', r'wrong.*should'
            ]
        }
    ...
    def detect_feedback_type(self, user_response):
        ...
        positive_count = sum(1 for pattern in self.feedback_patterns['positive']
                           if re.search(pattern, response_lower))
        negative_count = sum(1 for pattern in self.feedback_patterns['negative']
                           if re.search(pattern, response_lower))
        ...
        for pattern_id, pattern in memory.get("entities", {}).items():
            if pattern.get("type") == "user_correction":
                ...
                if (confidence >= min_confidence and 
                    applications >= min_applications and 
    ...
    def get_confidence_stats(self):
        ...
        patterns = [p for p in memory.get("entities", {}).values() 
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "stats":
            ...
            if patterns:
                ...
                for p in patterns:
                    print(f"  • {p['id']}: {p['confidence']:.2f} confidence, {p['applications']} applications")
            else:
