...
def enhanced_learn(learning_content):
    ...
    if corrections:
        ...
        for correction in corrections:
            ...
            results.append(f"✅ Stored correction: {correction['type']} (ID: {entity_id})")
    else:
    ...
    # Show memory stats
    memory = load_memory()
    total_corrections = len([e for e in memory["entities"].values() if e["type"] == "user_correction"])
...
def get_memory_stats():
    ...
    stats = {
        ...
        "corrections": len([e for e in memory["entities"].values() if e["type"] == "user_correction"]),
        "observations": len([e for e in memory["entities"].values() if e["type"] == "general_observation"]),
