...
def main() -> None:
    ...
    if not project_root:
        print(
            "ERROR: Unable to locate project root containing $PROJECT_ROOT/mcp_api.py. "
    ...
    if not mcp_server_path:
        print(
            f"ERROR: MCP server not found under {project_root}. Expected $PROJECT_ROOT/mcp_api.py",
            file=sys.stderr,
        )
        sys.exit(1)
