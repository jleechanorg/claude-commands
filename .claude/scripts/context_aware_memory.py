...
class ContextAnalyzer:
    ...
    def analyze_context(
        self, user_message: str, conversation_history: list[str] | None = None
    ) -> dict:
        ...
        if context_analysis["trigger_categories"]:
            ...
            context_analysis["consultation_priority"] = self._calculate_priority(
                context_analysis
            )
            context_analysis["relevant_contexts"] = context_analysis[
                "trigger_categories"
            ]
            context_analysis["query_keywords"] = self._extract_query_keywords(
                message_lower, context_analysis["trigger_categories"]
            )
    ...
    def _extract_query_keywords(self, message: str, categories: list[str]) -> list[str]:
        ...
        technical_terms = re.findall(
            r"\b(?:function|class|method|variable|import|test|code|script|file|module|component|system|api|database|server|client)\b",
            message,
            re.IGNORECASE,
        )
...
class MemoryQueryEngine:
    ...
    def __init__(self):
        self.local_memory_file = (
            Path.home() / ".cache" / "claude-learning" / "learning_memory.json"
        )
    ...
    def query_relevant_patterns(
        ...
        if result["patterns_found"]:
            result["memory_insights"] = self._generate_insights(
                result["patterns_found"], context
            )
            result["confidence_weighted_suggestions"] = self._rank_suggestions(
    ...
    def _query_local_memory(self, keywords: list[str]) -> list[dict]:
        ...
        try:
            with open(self.local_memory_file) as f:
                memory = json.load(f)
        except:
            return []
        ...
        corrections = [
            e for e in entities.values() if e.get("type") == "user_correction"
        ]
        ...
        relevant_patterns.sort(
            key=lambda x: (x["relevance_score"], x.get("confidence", 0)), reverse=True
        )
    ...
    def _generate_insights(self, patterns: list[dict], context: dict) -> list[str]:
        ...
        insights.append(
            f"💭 Found {len(patterns)} relevant pattern(s) from previous learning:"
        )
        ...
        for i, pattern in enumerate(patterns[:3], 1):  # Show top 3
            ...
            insights.append(
                f"  {i}. [{correction_type}] {pattern_text} (confidence: {confidence:.1f})"
            )
...
def test_context_aware_memory():
    ...
    for i, message in enumerate(test_messages, 1):
        ...
        print(
            f"  - Categories: {', '.join(context['trigger_categories']) if context['trigger_categories'] else 'None'}"
        )
        ...
        if result["confidence_weighted_suggestions"]:
            ...
            for suggestion in result["confidence_weighted_suggestions"][:2]:
                print(
                    f"  - {suggestion['recommendation']} (score: {suggestion['combined_score']:.2f})"
                )
