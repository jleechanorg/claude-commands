...
class TestClaudeBotServer(unittest.TestCase):
    ...
    def test_hello_response_quality(self):
        ...
        for prompt in test_cases:
            with self.subTest(prompt=prompt):
                ...
                self.assertFalse(
                    response.startswith('[Local:') and response.endswith(']'),
                    f"Response is only metadata for '{prompt}': {response}"
                )
                ...
                if os.getenv('TESTING') == 'true':
                    ...
                    self.assertTrue(
                        any(indicator in response for indicator in error_indicators),
                        f"Expected error handling in test environment for '{prompt}': {response}"
                    )
                    # Should be reasonably informative
                    self.assertGreaterEqual(len(response), 20,
                                          f"Error message too short for '{prompt}': {response}")
                else:
                    # In production, expect greeting content
                    greeting_words = ['hello', 'hi', 'claude', 'assist', 'help']
                    self.assertTrue(
                        any(word in response.lower() for word in greeting_words),
                        f"Response lacks greeting content for '{prompt}': {response}"
                    )
                    # Should be reasonably long
                    self.assertGreaterEqual(len(response), 20,
                                          f"Response too short for '{prompt}': {response}")
        ...
        for prompt, expected_content in test_cases:
            with self.subTest(prompt=prompt):
                ...
                if os.getenv('TESTING') == 'true':
                    ...
                    self.assertTrue(
                        any(content in response.lower() for content in expected_content),
                        f"Response lacks math content for '{prompt}': {response}"
                    )
        ...
        for prompt in test_cases:
            with self.subTest(prompt=prompt):
                ...
                if os.getenv('TESTING') == 'true':
                    ...
                    self.assertTrue(
                        any(word in response.lower() for word in help_words),
                        f"Response lacks help content for '{prompt}': {response}"
                    )
                    # Should be substantial
                    self.assertGreaterEqual(len(response), 30,
                                          f"Help response too short for '{prompt}': {response}")
        ...
        for prompt in test_cases:
            with self.subTest(prompt=prompt):
                ...
                if os.getenv('TESTING') == 'true':
                    ...
                    self.assertTrue(
                        any(word in response.lower() for word in debug_words),
                        f"Response lacks debug content for '{prompt}': {response}"
                    )
    ...
    def test_general_response_quality(self):
        ...
        for prompt in test_cases:
            with self.subTest(prompt=prompt):
                ...
                self.assertTrue(
                    prompt.lower() in response.lower() or 'claude' in response.lower(),
                    f"Response doesn't acknowledge prompt or identity: {response}"
                )
    ...
    def test_response_not_just_branch_header(self):
        ...
        for prompt in test_prompts:
            with self.subTest(prompt=prompt):
                ...
                import re
                self.assertFalse(
                    re.match(branch_header_pattern, response.strip()),
                    f"Response is ONLY branch header for '{prompt}': {response}"
                )

                # Should provide SOME response (even if error in testing)
                self.assertGreater(len(response.strip()), 0,
                                 f"Completely empty response for '{prompt}'")
    ...
    def test_response_consistency(self):
        ...
        for response in responses:
            self.assertIn('claude', response.lower())
