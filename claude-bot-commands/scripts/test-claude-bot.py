...
def test_form_encoded_request():
    ...
    try:
        data = {
            "prompt": 'Hello Claude, this is a test prompt. Please respond with "Test successful".'
        }
...
def test_json_request():
    ...
    try:
        data = {
            "prompt": 'Hello Claude, this is a JSON test. Please respond with "JSON test successful".'
        }
        headers = {"Content-Type": "application/json"}
        response = requests.post(
            "http://127.0.0.1:5001/claude", json=data, headers=headers, timeout=30
        )
