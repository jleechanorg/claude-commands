...
def main():
    ...
    with sync_playwright() as p:
        ...
        if len(planning_blocks) > 0:
            ...
            choice_buttons = last_block.query_selector_all(".choice-button:not(.choice-button-custom)")
            ...
            if choice_buttons:
                ...
                if new_value == choice_text:
                    ...
                    print(f"   ❌ Input not populated correctly. Expected: '{choice_text}'")
            ...
            if planning_blocks:
                ...
                if custom_button:
                    ...
                    is_focused = page.evaluate("document.activeElement === document.getElementById('user-input')")
                    print(f"   Input focused: {'✅ Yes' if is_focused else '❌ No'}")
                    
                    # Check placeholder
                    placeholder = user_input.get_attribute("placeholder") if user_input else ""
                    print(f"   Placeholder: '{placeholder}'")
                    
                    # Check if form was NOT submitted
                    loading_spinner = page.query_selector("#loading-spinner")
                    is_loading = loading_spinner.is_visible() if loading_spinner else False
                    print(f"   Form auto-submitted: {'❌ Yes (ERROR!)' if is_loading else '✅ No (correct)'}")
                    ...
                    if send_button:
                        ...
                        # Wait for response
                        try:
                            page.wait_for_selector("#loading-spinner", state="visible", timeout=2000)
                            page.wait_for_selector("#loading-spinner", state="hidden", timeout=30000)
                            print("   ✅ Custom action response received!")
                            page.screenshot(path=f"{SCREENSHOT_DIR}/07_custom_response.png")
                        except:
