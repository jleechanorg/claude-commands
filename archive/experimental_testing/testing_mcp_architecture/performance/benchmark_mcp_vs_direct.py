...
class PerformanceBenchmark:
    ...
    @patch_firestore()
    @patch_gemini()
    async def benchmark_campaign_creation(
        self, mock_gemini, mock_firestore
    ) -> dict[str, Any]:
        ...
        with mock_environment(self.mock_port) as env:
            ...
            mcp_result = await self.benchmark_function(
                mcp_create_campaign, iterations=50
            )
    ...
    @patch_firestore()
    @patch_gemini()
    async def benchmark_campaign_retrieval(
        self, mock_gemini, mock_firestore
    ) -> dict[str, Any]:
        ...
        direct_result = await self.benchmark_function(
            self.direct_calls.get_campaigns, "benchmark-user-123", iterations=100
        )
        results["direct"] = direct_result
        ...
        with mock_environment(self.mock_port) as env:
            ...
            mcp_result = await self.benchmark_function(
                mcp_get_campaigns, iterations=100
            )
            results["mcp"] = mcp_result
    ...
    async def benchmark_action_processing(
        ...
        with mock_environment(self.mock_port) as env:
            ...
            mcp_result = await self.benchmark_function(
                mcp_process_action, iterations=20
            )
            results["mcp"] = mcp_result
    ...
    async def run_full_benchmark(self) -> dict[str, Any]:
        ...
        # Overall summary
        overheads = []
        for _benchmark_name, benchmark_data in benchmark_results["benchmarks"].items():
        ...
        if overheads:
            benchmark_results["summary"] = {
                ...
                "acceptable_overhead": all(
                    o < 20 for o in overheads
                ),  # < 20% overhead acceptable
                "performance_verdict": "PASS"
                if all(o < 20 for o in overheads)
                else "FAIL",
            }
    ...
    def generate_report(self, results: dict[str, Any]) -> str:
        ...
        if "summary" in results:
            ...
            report.append(
                f"Average Overhead: {summary['average_overhead_percent']:.2f}%"
            )
        ...
        for benchmark_name, benchmark_data in results["benchmarks"].items():
            ...
            if "overhead" in benchmark_data:
                ...
                report.append(
                    f"MCP Overhead: {overhead['absolute_ms']:.2f}ms ({overhead['percent']:.1f}%) {status}"
                )
