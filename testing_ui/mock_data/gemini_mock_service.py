...
class MockGeminiService:
    ...
    def load_mock_responses(self):
        ...
        try:
            ...
            with open(os.path.join(self.mock_data_dir, 'dragon_knight_responses.json'), 'r') as f:
    ...
    def _get_dragon_knight_response(self, character, setting):
        ...
        if 'initial_story_response' in self.dragon_knight_data:
            ...
            return {
                ...
                "user_scene_number": 1,
                "session_header": base_response.get('structured_response', {}).get('session_header', ''),
                "planning_block": base_response.get('structured_response', {}).get('planning_block', {}),
                "dice_rolls": base_response.get('structured_response', {}).get('dice_rolls', []),
                "resource_updates": base_response.get('structured_response', {}).get('resource_updates', {})
            }
        else:
...
def mock_gemini_get_initial_story(prompt, selected_prompts=None, custom_options=None, 
                                  generate_companions=False, use_default_world=False, **kwargs):
