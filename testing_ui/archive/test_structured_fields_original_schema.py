    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            page.wait_for_selector(
                '.ai-message, .story-entry:has-text("Scene")', timeout=30000
            )
            time.sleep(3)  # Let structured fields render

            page.screenshot(
                path=f"{SCREENSHOT_DIR}/03_after_response.png", full_page=True
            )
                ...
                if elements:
                    ...
                    if elements[0]:
                        elements[0].screenshot(
                            path=f"{SCREENSHOT_DIR}/element_{selector.replace('.', '')}.png"
                        )
                else:
