    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if page.is_visible("#wizard-campaign-title"):
                ...
                page.fill(
                    "#wizard-campaign-prompt", "Test all structured response fields"
                )
            ...
            if page.is_visible("#game-view"):
                ...
                action_input = page.locator(
                    "#player-action, #user-input, textarea"
                ).first
                if action_input:
                    action_input.fill("I attack the goblin!")

                    send_button = page.locator(
                        "#send-action, button:has-text('Send')"
                    ).first
                    if send_button:
                        send_button.click()
                        ...
                        page.screenshot(
                            path=f"{SCREENSHOT_DIR}/03_character_response.png"
                        )
                        ...
                        if (
                            page.locator(
                                '.god-mode-response, [data-field="god_mode_response"]'
                            ).count()
                            > 0
                        ):
                        ...
                        page.screenshot(
                            path=f"{SCREENSHOT_DIR}/05_final.png", full_page=True
                        )
