...
class MobileResponsiveTest(BrowserTestBase):
    ...
    def _test_tablet_viewport(self, page: Page) -> bool:
        ...
        if visible_elements > 0:
            print(
                f"   ✅ {visible_elements}/{len(essential_elements)} elements visible on tablet"
            )
            return True
    ...
    def _test_mobile_viewport(self, page: Page) -> bool:
        ...
        mobile_essential_elements = ["#dashboard-view", "text=New Campaign"]
        ...
        if mobile_visible > 0:
            print(
                f"   ✅ {mobile_visible}/{len(mobile_essential_elements)} elements visible on mobile"
            )
            return True
    ...
    def _test_touch_interactions(self, page: Page) -> bool:
            ...
            try:
                if page.is_visible(element):
                    ...
                    if element_box:
                        ...
                        if (
                            element_box["width"] >= min_touch_size
                            and element_box["height"] >= min_touch_size
                        ):
                            print(
                                f"   ✅ Touch-friendly size: {element} ({element_box['width']}x{element_box['height']})"
                            )
                            touch_tested += 1
                        else:
                            print(
                                f"   ⚠️  Small touch target: {element} ({element_box['width']}x{element_box['height']})"
                            )
        ...
        if touch_tested > 0:
            print("   ✅ Touch interactions tested")
            return True
    ...
    def _test_responsive_layout(self, page: Page) -> bool:
        ...
        for viewport in viewports:
            ...
            if body_width <= viewport_width + 20:  # Allow small margins
                print(
                    f"   ✅ {viewport['name']}: No horizontal overflow ({body_width}px <= {viewport_width}px)"
                )
                layout_tests_passed += 1
            else:
                print(
                    f"   ⚠️  {viewport['name']}: Horizontal overflow detected ({body_width}px > {viewport_width}px)"
                )
            ...
            if responsive_classes:
                print(
                    f"   ✅ {viewport['name']}: Responsive classes detected: {responsive_classes}"
                )

        if layout_tests_passed > 0:
            print(
                f"   ✅ {layout_tests_passed}/{len(viewports)} viewports passed layout test"
            )
            return True
    ...
    def _test_orientation_changes(self, page: Page) -> bool:
        ...
        portrait_scrollable = page.evaluate(
        ...
        landscape_scrollable = page.evaluate(
            "document.body.scrollHeight > window.innerHeight"
        )
