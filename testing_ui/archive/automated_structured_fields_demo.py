...
from browser_test_base import SCREENSHOT_DIR, BrowserTestBase
...
class StructuredFieldsDemo(BrowserTestBase):
    ...
    def run_test(self, page: Page) -> bool:
        ...
        try:
            ...
            new_campaign_btn = page.locator(
                "text='New Campaign', button:has-text('New Campaign')"
            ).first
            if new_campaign_btn.is_visible():
                ...
                page.fill(
                    ...
                    "A fantasy adventure where I explore a dungeon filled with traps and treasures",
                )
                ...
                page.fill(
                    "#wizard-character-background",
                    "A brave adventurer skilled in both sword and spell",
                )

                # Launch campaign
                launch_btn = page.locator(
                    "#launch-campaign, button:has-text('Begin Adventure')"
                ).first
                if launch_btn.is_visible():
                    ...
                    action_input = page.locator(
                        "#player-action, textarea[placeholder*='What do you do']"
                    ).first
                    send_btn = page.locator("button:has-text('Send')").first

                    if action_input.is_visible() and send_btn.is_visible():
                        action_input.fill(
                            "I carefully examine the dungeon entrance for traps, then cast a light spell on my sword and enter cautiously."
                        )
                        send_btn.click()

                        print(
                            "⏳ Waiting for Gemini response (this will cost API credits)..."
                        )
                        ...
                        resources = page.locator(
                            ".resources, .alert-warning:has-text('Resources')"
                        ).first
                        if resources.is_visible():
                            resources.screenshot(
                                path=f"{SCREENSHOT_DIR}/field_resources.png"
                            )
                        ...
                        if god_mode_radio.is_visible():
                            ...
                            action_input.fill(
                                "What monsters and treasures await in the first chamber? Roll initiative for any encounters."
                            )
                            send_btn.click()
                                ...
                                for selector in selectors.split(", "):
                                    try:
                                        ...
                                        if element.is_visible():
                                            element.screenshot(
                                                path=f"{SCREENSHOT_DIR}/field_{field_name}.png"
                                            )
                            ...
                            page.screenshot(
                                path=f"{SCREENSHOT_DIR}/05_full_page_final.png",
                                full_page=True,
                            )
