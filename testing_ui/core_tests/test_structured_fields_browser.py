...
class StructuredFieldsTest(BrowserTestBase):
    ...
    def run_test(self, page: Page) -> bool:
        ...
        print("🧪 Testing structured fields with shared utilities:")
        ...
        try:
            ...
            field_results["debug_info"] = (
                field_results.get("debug_info", False) or debug_result
            )
        ...
        try:
            ...
            if page.is_visible("#new-campaign-view.active-view"):
                ...
                for selector in prompt_selectors:
                    if page.is_visible(selector):
                        page.fill(
                            selector, "Fantasy adventure to test all structured fields"
                        )
                        break
    ...
    def _test_character_mode_fields(
        self, page: Page, helper: BrowserTestHelper
    ) -> dict:
        ...
        if action_input.count() > 0 and send_button.count() > 0:
            helper.send_game_interaction(
                "I attack the goblin with my sword and cast shield spell!"
            )
            ...
            field_selectors = {
                ...
                "entities_mentioned": [
                    ".entities-mentioned",
                    '[data-field="entities_mentioned"]',
                ],
                "location_confirmed": [
                    ".location-confirmed",
                    '[data-field="location_confirmed"]',
                ],
            ...
            for field, selectors in field_selectors.items():
                ...
                for selector in selectors:
                    if page.locator(selector).count() > 0:
                        ...
                        if element:
                            element.screenshot(
                                path=f"{SCREENSHOT_DIR}/{field}_element.png"
                            )
    ...
    def _test_god_mode_response(self, page: Page, helper: BrowserTestHelper) -> bool:
        ...
        if action_input.count() > 0 and send_button.count() > 0:
            ...
            helper.send_game_interaction(
                "GOD MODE: Create a magical artifact in the room", mode="god"
            )
            helper.wait_for_ai_response()
            ...
            for selector in god_selectors:
                if page.locator(selector).count() > 0:
                    ...
                    if element:
                        element.screenshot(
                            path=f"{SCREENSHOT_DIR}/god_mode_response_element.png"
                        )
                        print("   📸 Element screenshot: god_mode_response_element.png")
                    return True
    ...
    def _test_debug_mode(self, page: Page, helper: BrowserTestHelper) -> bool:
        ...
        debug_toggle = page.locator(
            "#debug-toggle, .debug-toggle, button:has-text('Debug')"
        )
        ...
        for selector in debug_selectors:
            if page.locator(selector).count() > 0:
                ...
                if element:
                    ...
                    print("   📸 Element screenshot: debug_info_element.png")
                return True
