...
def try_multiple_selectors(page: Page, selectors: List[str], 
                          action: str = "click", timeout: int = 1000) -> Optional[str]:
    """
...
def get_active_view(page: Page) -> Optional[str]:
    """
...
def fill_form_field(page: Page, field_id: str, value: str, 
...
def get_element_text_safe(page: Page, selector: str, 
...
def wait_for_any_selector(page: Page, selectors: List[str], 
...
def extract_structured_fields(page: Page) -> Dict[str, Any]:
    ...
    fields["narrative_text"] = get_element_text_safe(
        page, 
        try_multiple_selectors(page, narrative_selectors, "text") or ""
    )
    
    # Session header
    session_selectors = [
        ".session-header",
        "[data-field='session_header']"
    ]
    fields["session_header"] = get_element_text_safe(
        page,
        try_multiple_selectors(page, session_selectors, "text") or ""
    )
    
    # Planning block
    planning_selectors = [
        ".planning-block",
        "[data-field='planning_block']"
    ]
...
def verify_api_call_made(page: Page, url_pattern: str, 
                        method: str = "POST", timeout: int = 5000) -> bool:
    """
    ...
    print(f"⚠️ API call verification requires request interception setup")
    return True
...
def wait_for_text_to_appear(page: Page, text: str, 
...
def navigate_to_test_game(page: Page, campaign_name: str = "All Fields Test Campaign", 
                         port: int = 6006, test_user_id: str = "test-user-123") -> bool:
    """
    ...
    try:
        ...
        if not wait_for_any_selector(page, ["[data-testid='campaign-list']", ".campaign-card", "text='My Campaigns'"], timeout=10000):
...
def capture_structured_fields_sequence(page: Page, test_name: str, 
                                     action_text: str = "I attack the goblin") -> Dict[str, str]:
    """
    ...
    if action_input:
        ...
        send_button = page.locator("#send-action, button:has-text('Send'), button[type='submit']").first
        if send_button:
            send_button.click()
        ...
        if not wait_for_text_to_appear(page, "Response time:", timeout=30000):
            ...
            if not wait_for_any_selector(page, [".narrative-text", ".ai-response", ".story-content"], timeout=30000):
                print("❌ No AI response received")
                return screenshots
    
    # Take screenshot of full response
    screenshots['full_ai_response'] = take_screenshot(page, test_name, "full_ai_response")
    ...
    for field_name, selectors in field_selectors.items():
        ...
        if element:
            screenshots[field_name] = take_screenshot(page, test_name, field_name, element_selector=element)
        else:
