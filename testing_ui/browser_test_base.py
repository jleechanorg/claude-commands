    ...
    def ensure_fresh_server(self):
        ...
        for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
            try:
                ...
                if any('main.py' in arg for arg in cmdline) and any('mvp_site' in arg for arg in cmdline):
                    print(f"   🛑 Killing existing Flask process (PID: {proc.info['pid']})")
                    proc.kill()
                    killed_any = True
            ...
            try:
                ...
                for conn in connections:
                    if hasattr(conn, 'laddr') and conn.laddr.port == 8080:
                        print(f"   🛑 Killing process on port 8080 (PID: {proc.info['pid']})")
                        proc.kill()
                        killed_any = True
                        break
...
class BrowserTestBase:
    ...
    def setup_browser(self) -> tuple[Browser, BrowserContext, Page]:
        ...
        page.on("requestfailed", lambda req: self.console_logs.append(f"request_failed: {req.url} - {req.failure}"))
    ...
    def navigate_with_test_mode(self, page: Page) -> bool:
        ...
        if not auth_check['windowTestAuthBypass'] and page.is_visible("text=Sign in with Google"):
    ...
    def take_screenshot(self, page: Page, description: str):
        ...
        filename = f"{self.screenshot_prefix}_{self.screenshots_taken:02d}_{description}.png"
    ...
    def execute(self):
        ...
        with FlaskServerManager():
            ...
            try:
                ...
                if browser:
                    page.screenshot(path=os.path.join(SCREENSHOT_DIR, f"{self.screenshot_prefix}_error.png"))
                return False
