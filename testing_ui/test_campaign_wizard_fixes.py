...
def test_campaign_wizard_character_display():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            assert "Sir TestCharacter the Bold" in character_text, f"Expected custom character name but got: {character_text}"
            assert "dragon knight" not in character_text.lower(), f"Found 'dragon knight' in character preview: {character_text}"
            print("✅ SUCCESS: Custom character name displays correctly in confirmation!")
            ...
            assert "ser arion dragon knight campaign" not in description_text.lower(), f"Found hardcoded dragon knight text in description: {description_text}"
...
def test_campaign_wizard_empty_character_display():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            assert "Auto-generated" in character_text, f"Expected 'Auto-generated' for empty character but got: {character_text}"
            assert "Ser Arion" not in character_text, f"Found Dragon Knight default in character preview: {character_text}"
...
def test_campaign_page_loads_with_story():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            page.fill("#description-input", "A brave hero begins their journey in a mystical kingdom.")
            ...
            while (time.time() - start_time) * 1000 < max_wait_time:
                ...
                page.wait_for_timeout(1000)
                print(f"⏳ Waiting for story content... ({int(time.time() - start_time)}s)")
            ...
            assert story_count >= 1, f"Expected at least 1 story entry, but found {story_count}"
            ...
            assert ai_count >= 1, f"Expected at least 1 AI response, but found {ai_count}"
            
            print("✅ SUCCESS: Campaign page loads with full story content!")
            
            # Verify no error messages are shown
            assert not page.locator(".alert-danger").is_visible(), "Error alert should not be visible"
            assert not page.locator(".alert-warning").is_visible(), "Warning alert should not be visible"
