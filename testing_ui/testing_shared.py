...
def validate_campaign_created_successfully(response_data, test_mode="http"):
    ...
    if test_mode == "http":
        ...
        assert response_data.get('success') == True, f"Campaign creation not successful: {response_data}"
        assert response_data.get('campaign_id') is not None, f"No campaign ID returned: {response_data}"
...
def validate_character_name_display(character_text, expected_character, test_mode="http"):
    """Common validation for character name display"""
    if expected_character:
        assert expected_character in character_text, f"Expected character '{expected_character}' not found in: {character_text}"
    
    # Common validations for both test types
    assert "dragon knight" not in character_text.lower(), f"Found 'dragon knight' in character text: {character_text}"
...
def validate_no_hardcoded_text(text_content, test_mode="http"):
    ...
    assert "ser arion dragon knight campaign" not in combined_text, f"Found hardcoded dragon knight text: {text_content}"
...
def validate_story_content_exists(story_data, minimum_length=50):
    ...
    if isinstance(story_data, list) and len(story_data) > 0:
        ...
        narrative_text = first_entry.get('narrative_text', '') if isinstance(first_entry, dict) else ''
...
def start_test_server(port="6006") -> Optional[subprocess.Popen]:
...
def validate_api_response_structure(response_data, expected_fields=None):
    ...
    for field in expected_fields:
        assert field in response_data, f"Missing required field '{field}' in response: {response_data}"
...
def validate_browser_element_text(element_text, expected_content, should_contain=True):
    ...
    if should_contain:
        assert expected_content in element_text, f"Expected '{expected_content}' in element text: {element_text}"
    else:
        assert expected_content not in element_text, f"Should not contain '{expected_content}' in element text: {element_text}"
