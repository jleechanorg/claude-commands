        ...
        try:
            ...
            if page.is_visible("text=New Campaign"):
                ...
                if page.is_visible("#wizard-campaign-title"):
                    ...
                    page.fill("#wizard-campaign-prompt", "A dangerous dungeon filled with monsters and combat encounters.")
            ...
            if message_input:
                ...
                combat_action = "I draw my sword and attack the goblin blocking the path!"
                ...
                if chat_area:
                    ...
                    if any(word in messages.lower() for word in ["attack", "damage", "hit", "combat", "battle", "fight"]):
            ...
            if not dice_found:
                print("   ⚠️  No dice roller found - trying to trigger it via combat action")
                ...
                if message_input:
                    ...
                    if chat_area:
                        messages = chat_area.text_content()
                        if any(word in messages.lower() for word in ["rolled", "1d20", "dice", "result"]):
            ...
            for selector in combat_action_selectors:
                if page.is_visible(selector):
                    ...
                    self.take_screenshot(page, f"combat_action_{selector.split(':')[0]}")
                    break
            ...
            if message_input:
                ...
                if chat_area:
                    messages = chat_area.text_content()
                    if any(word in messages.lower() for word in ["damage", "hit", "hurt", "wounds"]):
            ...
            if message_input:
                critical_action = "I roll a natural 20 for a critical hit with my sword!"
                ...
                if chat_area:
                    messages = chat_area.text_content()
                    if any(word in messages.lower() for word in ["critical", "crit", "maximum", "double"]):
            ...
            if message_input:
                ...
                if chat_area:
                    messages = chat_area.text_content()
                    if any(word in messages.lower() for word in ["victory", "defeated", "wins", "triumph"]):
