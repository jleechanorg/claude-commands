...
class ExecutionContext:
    ...
    pattern_guidance: Optional[str]
    memory_insights: Optional[QueryResult]
...
class EnhancedExecuteWrapper:
    ...
    def analyze_task_complexity(self, task: str) -> str:
        ...
        high_complexity = [
            "architecture", "refactor", "migration", "integration", "system",
            "multiple", "complex", "across", "entire", "comprehensive"
        ]
        
        # Medium complexity indicators  
        medium_complexity = [
            "implement", "create", "build", "modify", "update", "enhance",
            "api", "database", "test", "feature"
        ]
        
        # Low complexity indicators
        low_complexity = [
            "fix", "debug", "small", "quick", "simple", "single", "minor"
        ]
        ...
        if high_score >= 2 or len(task.split()) > 15:
            return "High"
        elif medium_score >= 2 or len(task.split()) > 8:
            return "Medium"
        elif low_score >= 1 or len(task.split()) <= 5:
            return "Low"
        else:
            return "Medium"  # Default
    
    def query_execution_patterns(self, task: str) -> Optional[QueryResult]:
    ...
    def generate_execution_approach(self, context: ExecutionContext) -> str:
        ...
        if context.complexity == "High":
            approach_parts.append("🚨 HIGH COMPLEXITY: Use subagent coordination with parallel execution")
        ...
        if context.memory_insights and context.memory_insights.matches:
            ...
            for match in context.memory_insights.matches[:3]:
                ...
                if "execution" in pattern.content.lower() or "approach" in pattern.content.lower():
                    approach_parts.append(f"• {pattern.content[:80]}...")
                
                # Check for workflow patterns
                if match.pattern.pattern_type == "workflow":
                    approach_parts.append(f"• Follow established workflow: {pattern.content[:60]}...")
                
                # Check for corrections
                if match.pattern.pattern_type == "correction":
                    approach_parts.append(f"• ⚠️ Avoid previous mistake: {pattern.content[:60]}...")
        
        return "\n".join(approach_parts)
    
    def determine_checkpoint_frequency(self, complexity: str, patterns: Optional[QueryResult]) -> str:
        ...
        if patterns and patterns.matches:
            for match in patterns.matches:
                ...
                if "checkpoint" in content_lower or "frequency" in content_lower:
                    ...
                    elif "less" in content_lower or "reduced" in content_lower:
                        return "Every 7 minutes OR 5-7 files (pattern-recommended)"
        
        return base_frequency[complexity]
    
    def generate_todowrite_checklist(self, context: ExecutionContext, command_type: str) -> str:
...
- [ ] Workflow patterns followed: {sum(1 for m in context.memory_insights.matches if m.pattern.pattern_type == 'workflow')}"""
    ...
    def format_pattern_guidance(self, context: ExecutionContext) -> str:
        ...
        output.append(f"Found {len(context.memory_insights.matches)} relevant patterns from {context.memory_insights.total_searched} total patterns")
        ...
        for pattern_type, matches in by_type.items():
            if pattern_type == "correction":
                ...
                for match in matches:
                    ...
                    output.append(f"  {confidence_marker} {match.pattern.content[:100]}...")
    ...
    def enhanced_execute(self, task: str, command_type: str = "execute") -> str:
        ...
        execution_context.recommended_approach = self.generate_execution_approach(execution_context)
        execution_context.checkpoint_frequency = self.determine_checkpoint_frequency(complexity, memory_insights)
        ...
        if command_type == "plan":
            ...
            response_parts.append("First, let me consult my memory for relevant patterns...")
        else:
        ...
        response_parts.append(f"**Recommended Approach**: {execution_context.recommended_approach}")
        response_parts.append(f"**Checkpoint Frequency**: {execution_context.checkpoint_frequency}")
