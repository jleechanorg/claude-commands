...
from enhanced_learn import LearningPattern, EnhancedLearner
...
class PatternQueryEngine:
    ...
    def calculate_pattern_relevance(self, pattern: LearningPattern, query: str, 
                                   intent_scores: Dict[str, float], 
                                   keywords: Set[str]) -> Tuple[float, List[str]]:
        ...
        for tag in pattern.tags:
            tag_lower = tag.lower()
            if tag_lower in query_lower or any(keyword in tag_lower for keyword in keywords):
                tag_matches += 1
        ...
        for intent, intent_score in intent_scores.items():
            if intent_score > 0.3:  # Significant intent
                if intent.replace('seeking_', '') in pattern_type_lower or \
                   any(tag.lower() == intent.replace('seeking_', '') for tag in pattern.tags):
    ...
    def query_patterns(self, query: str, limit: int = 10, 
                      min_relevance: float = 0.1) -> QueryResult:
        ...
        for pattern in self.learner.patterns:
            ...
            if relevance_score >= min_relevance:
                ...
                suggested_action = self._determine_suggested_action(pattern, intent_scores)
    ...
    def _determine_suggested_action(self, pattern: LearningPattern, 
        ...
        if pattern.pattern_type == "correction":
        ...
        elif pattern.pattern_type == "technical":
            ...
            elif top_intent == "seeking_preference":
                return "acknowledge_preference"
            elif top_intent == "seeking_process":
                return "outline_process_steps"
            elif top_intent == "seeking_technical":
                return "provide_technical_details"
            else:
                return "apply_learned_knowledge"
    
    def _generate_recommendations(self, matches: List[PatternMatch], query: str) -> List[str]:
        """Generate actionable recommendations based on pattern matches."""
        recommendations = []
        
        if not matches:
            recommendations.append("No relevant patterns found. Proceed with standard response.")
        ...
        if "correction" in by_type:
            ...
            high_confidence_corrections = [c for c in corrections if c.pattern.confidence >= 0.8]
    ...
    def format_query_results(self, result: QueryResult) -> str:
        ...
        output = []
        output.append(f"Memory Query Results ({len(result.matches)} matches from {result.total_searched} patterns):")
...
class MemoryConsciousResponseGenerator:
    ...
    def consult_memory(self, query: str) -> Optional[QueryResult]:
    ...
    def generate_memory_guidance(self, query: str) -> Tuple[Optional[str], Optional[QueryResult]]:
