...
class MemoryAwareCommandProcessor:
    ...
    def _query_test_patterns(self, context: CommandContext) -> QueryResult:
        ...
        test_type = (
            "ui"
            if "ui" in context.command_name
            else "http"
            if "http" in context.command_name
            else "integration"
        )
    ...
    def _parse_command_args(self, command: str, args: str) -> dict[str, Any]:
        ...
        if command in ["execute", "e", "plan"]:
            ...
            parsed["keywords"] = [
                kw for kw in implementation_keywords if kw in args.lower()
            ]
    ...
    def _assess_command_complexity(self, command: str, args: str) -> str:
        ...
        low_indicators = ["fix", "simple", "small", "quick", "minor", "single"]
        ...
        medium_score = sum(
            1 for indicator in medium_indicators if indicator in args_lower
        )
        ...
        if high_score >= 1 or word_count > 15:
            return "High"
        if medium_score >= 1 or word_count > 8:
            return "Medium"
        if low_score >= 1 or word_count <= 5:
            return "Low"
        return "Medium"
    ...
    def consult_memory(self, context: CommandContext) -> MemoryGuidance | None:
        ...
        query_func = self.command_patterns.get(
            context.command_name, self._query_generic_patterns
        )
        ...
        for match in result.matches:
            ...
            if pattern.pattern_type == "correction" and pattern.confidence >= 0.8:
                ...
                "avoid" in pattern.content.lower() or "don't" in pattern.content.lower()
            ):
    ...
    def format_memory_guidance(self, guidance: MemoryGuidance | None) -> str:
    ...
    def process_command(self, command: str, args: str) -> str:
        ...
        if command in ["execute", "e"]:
            return (
                self.execute_wrapper.enhanced_execute(args, "execute")
                + f"\n\n{memory_section}"
            )

        if command == "plan":
            return (
                self.execute_wrapper.enhanced_execute(args, "plan")
                + f"\n\n{memory_section}"
            )
        ...
        response_parts.append(
            f"Processing `/{command}` command with memory awareness..."
        )
