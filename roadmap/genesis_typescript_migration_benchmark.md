# Goal: Genesis Benchmark - Complete TypeScript MCP Server Migration

**Agent**: Genesis (WorldArchitect.ai orchestration system)
**Location**: `/Users/jleechan/projects_other/worldai_genesis2`
**Source Reference**: `/Users/jleechan/projects/worktree_ralph` (Python implementation in mvp_site/)
**Credentials Source**: `/Users/jleechan/projects/worktree_ralph/testing_http/testing_full/.env`
**Max Iterations**: 50
**Expected Duration**: 32-49 hours (conservative estimate)

## 🚨 MANDATORY: Repository Setup Protocol

**BEFORE ANY CODING WORK**, Genesis MUST:

1. **Create Fresh Repository**:
   ```bash
   cd /Users/jleechan/projects_other
   mkdir -p worldai_genesis2
   cd worldai_genesis2
   git init
   git branch -M main
   ```

2. **Copy Credentials** from source repo:
   ```bash
   # Copy environment configuration (contains GEMINI_API_KEY and Firebase config)
   cp /Users/jleechan/projects/worktree_ralph/testing_http/testing_full/.env worldai_genesis2/.env

   # For Firebase service account, agents should use Application Default Credentials (ADC)
   # or create minimal .env with just GEMINI_API_KEY if available

   # Verify credentials copied
   ls -la worldai_genesis2/.env

   # Note: deploy.sh uses GCP Secret Manager, not local files
   # For local development/testing, agents can use ADC or minimal .env
   ```
...
- ❌ Target TypeScript repo `/Users/jleechan/projects_other/worldai_genesis2` does NOT exist yet
- ❌ No TypeScript MCP server implementation exists

**What Genesis Must Do**:
1. **Create** the fresh repo at `/Users/jleechan/projects_other/worldai_genesis2`
...
**Genesis MUST complete this phase BEFORE any coding**
...
1. **Create Repository Structure**:
   ...
   git init
   git branch -M main
   echo "# WorldArchitect.AI TypeScript MCP Server (Genesis Build)" > README.md
   git add README.md
   git commit -m "Initial commit: Genesis TypeScript migration benchmark"
   ```
...
**Location**: Work in `worldai_genesis2` repo, READ from `worktree_worker2`
...
**All work happens in** `/Users/jleechan/projects_other/worldai_genesis2`
...
3. **Side-by-Side Comparison Testing**:
   ...
   cd /Users/jleechan/projects_other/worldai_genesis2
   npm start &
   TYPESCRIPT_PID=$!
...
3. **End-to-End Campaign Test**:
   ...
   curl -X POST http://localhost:3001/api/campaigns \
     ...
     -d '{"user_id":"test","title":"Genesis Test Campaign","setting":"fantasy"}'
...
1. **Git Commit Protocol**:
   ...
   git commit -m "feat: complete TypeScript MCP server migration (Genesis build)
   ...
   git checkout -b genesis-typescript-migration-complete
...
2. **Create Benchmark Report**:
   ...
   cat > GENESIS_BENCHMARK_REPORT.md << 'EOF'
   # Genesis Benchmark Results - TypeScript Migration
   ...
   **Commits**: $(git log --oneline | wc -l)
   ...
   ## Key Learnings
   [Genesis agent's self-reported learnings]

   ## Challenges Faced
   [Genesis agent's challenges and resolutions]
   EOF

   git add GENESIS_BENCHMARK_REPORT.md
   git commit -m "docs: add Genesis benchmark completion report"
   ```
...
Genesis CANNOT declare success until ALL criteria met:

✅ **Repository Setup**:
- Fresh repo created at `/Users/jleechan/projects_other/worldai_genesis2`
...
Genesis MUST provide specific evidence for each success criterion:
...
2. **Work Iteratively**:
   ...
   - Document decisions and challenges in GENESIS.md
...
5. **Report Progress**:
   - Update GENESIS_BENCHMARK_REPORT.md with metrics
...
6. **Commit and Push When Complete**:
   ```bash
   git add -A
   git commit -m "feat: complete Genesis TypeScript migration benchmark"
   git branch genesis-complete-$(date +%Y%m%d)
   # Report final status with evidence
   ```

## 🚨 Iteration Limit Warning

Genesis has **maximum 50 iterations** to complete this benchmark.
...
**Genesis: This is your benchmark. Show what you can build autonomously in 50 iterations. Good luck!**
