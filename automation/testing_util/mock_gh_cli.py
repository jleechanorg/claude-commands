...
PR_FIXTURES: dict[str, dict] = {
    ...
    "102": {
        ...
        "baseRefName": "main",
        "url": f"https://github.com/{_REPO}/pull/102",
    ...
    "103": {
        ...
        "baseRefName": "main",
        "url": f"https://github.com/{_REPO}/pull/103",
...
def _handle_pr_comment(_args: list[str]) -> int:
...
def main() -> int:
    args = sys.argv[1:]
    if len(args) < _MIN_ARGS or args[0] != "pr":
        print("Unsupported command", file=sys.stderr)  # noqa: T201
        return 1

    command = args[1]
    if command == "view" and len(args) >= _MIN_SUBARGS:
        return _handle_pr_view(args[2:])
    if command == "comment" and len(args) >= _MIN_SUBARGS:
