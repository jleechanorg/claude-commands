...
TARGET_REPO="${EVIDENCE_TARGET_REPO:-${GITHUB_OWNER}/${PROJECT_NAME:-your-project}.com}"
...
if [[ "$INSTALL_MODE" == "pypi" ]]; then
  ...
  "$VENV_DIR/bin/pip" install "${PIP_INSTALL_COMMON[@]}" "${GITHUB_OWNER}-orchestration==${ORCH_VERSION}" >/dev/null
  "$VENV_DIR/bin/pip" install "${PIP_INSTALL_COMMON[@]}" "${GITHUB_OWNER}-pr-automation[dev]==${AUTOMATION_VERSION}" >/dev/null
else
...
import sys
import ${GITHUB_OWNER}_pr_automation
print("python:", sys.version)
print("${GITHUB_OWNER}_pr_automation.__file__:", ${GITHUB_OWNER}_pr_automation.__file__)
print("dist version:", m.version("${GITHUB_OWNER}-pr-automation"))
PY
...
spec = importlib.util.find_spec("${GITHUB_OWNER}_pr_automation")
if spec is None or spec.origin is None:
    raise SystemExit("Could not locate installed ${GITHUB_OWNER}_pr_automation")
...
PY
  ...
  run_and_capture "pytest_automation" "$VENV_DIR/bin/python" -m pytest -q "$AUTOMATION_DIR/${GITHUB_OWNER}_pr_automation/tests" || true
fi
...
run_and_capture "cron_workflow1_pr_monitor" "$VENV_DIR/bin/${GITHUB_OWNER}-pr-monitor" \
  --no-act \
...
run_and_capture "cron_workflow2_fixpr" "$VENV_DIR/bin/${GITHUB_OWNER}-pr-monitor" \
  --no-act \
  --fixpr \
  --max-prs 5 \
  --cli-agent gemini \
...
run_and_capture "cron_workflow3_codex_update" "$VENV_DIR/bin/${GITHUB_OWNER}-pr-monitor" \
  --no-act \
  --codex-update \
...
run_and_capture "target_pr_pr_automation_no_act" "$VENV_DIR/bin/${GITHUB_OWNER}-pr-monitor" \
  --no-act \
  --target-pr "$TARGET_PR" \
...
run_and_capture "target_pr_fix_comment_no_act" "$VENV_DIR/bin/${GITHUB_OWNER}-pr-monitor" \
  --no-act \
  --fix-comment \
  --target-pr "$TARGET_PR" \
  --target-repo "$TARGET_REPO" \
...
run_and_capture "target_pr_fixpr_no_act" "$VENV_DIR/bin/${GITHUB_OWNER}-pr-monitor" \
  --no-act \
  --fixpr \
  --target-pr "$TARGET_PR" \
  --target-repo "$TARGET_REPO" \
