...
## Context
The current `automation/` system in `${PROJECT_NAME:-your-project}.com` uses local cron + shell/python scripts to run PR automation and watchdog tasks. OpenClaw now provides Mission Control + agent orchestration that can supervise long-running flows, route messages, and coordinate multi-model review.
