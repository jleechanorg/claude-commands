...
def dispatch_agent_for_pr(
    ...
    if use_slash_command:
        ...
        "🚨🚨🚨 PRIORITY 1: MERGE CONFLICT RESOLUTION (DO THIS FIRST - BEFORE ANYTHING ELSE):\n"
        f"   ⚠️⚠️⚠️ MERGE CONFLICTS MUST BE RESOLVED BEFORE FIXING TESTS. CHECK MERGE STATUS IMMEDIATELY:\n\n"
        ...
        "   ⚠️⚠️⚠️ YOU ARE FORBIDDEN FROM REPLACING THE PR BODY FROM SCRATCH. THIS IS NOT A SUGGESTION - IT IS A HARD REQUIREMENT.\n"
        "   ⚠️⚠️⚠️ THE PR BODY MAY ALREADY CONTAIN EVIDENCE (GIST LINKS, BEAD CITATIONS, EMBEDDED VIDEOS) THAT A HUMAN OR ANOTHER\n"
        "   AGENT PRODUCED. OVERWRITING IT DESTROYS THAT WORK PERMANENTLY - THERE IS NO UNDO.\n"
        "   ✅ REQUIRED METHOD:\n"
        f"   1. FIRST fetch the CURRENT body: `gh pr view {pr_number} --json body --jq .body` (or the GitHub API GET endpoint).\n"
        "   2. Start from that exact text. ONLY append a new section or edit the specific section relevant to your fix\n"
        "      (e.g. adding a note that conflicts were resolved). NEVER drop existing sections, links, or embedded media.\n"
        "   3. The edited body's length MUST NOT shrink versus the fetched body unless you are told exactly which\n"
        "      section to remove. A body that goes from hundreds of lines to a short templated stub is ALWAYS WRONG.\n"
        "   ❌ FORBIDDEN:\n"
        f"   - `gh pr edit {pr_number} --body '<freshly written text>'` using anything other than the fetched-body-plus-edit.\n"
        "   - Reformatting the PR body to match a template by generating a brand-new body instead of editing sections in place.\n"
        "   - Any REST/GraphQL PATCH to the PR body that was not built by editing the text fetched in step 1.\n"
        "   If you are unsure whether an edit preserves prior content, DO NOT EDIT THE BODY - post a PR comment instead.\n\n"
        "🚨🚨🚨 ABSOLUTE PROHIBITION - CI/REVIEW-GATE BYPASS COMMANDS:\n"
        "   ⚠️⚠️⚠️ YOU ARE FORBIDDEN FROM POSTING ANY COMMAND THAT TRIES TO FORCE A REVIEW GATE TO PASS WITHOUT A GENUINE REVIEW.\n"
        "   ❌ FORBIDDEN - NEVER POST THESE:\n"
        "   - `@coderabbitai approve` (or any bot 'approve' slash command) - this force-approves without a real review.\n"
        "   - Re-triggering `@coderabbitai full review` (or any bot re-review command) when a PRIOR trigger from you or\n"
        "     anyone else is still within its rate-limit/cooldown window (check recent PR comments for a rate-limit banner,\n"
        "     e.g. 'in the 95th percentile', 'Fair Usage Limits', or an ETA message, before triggering again).\n"
        "   - Any comment whose purpose is to bypass, dismiss, or force a gate (CI status, review approval, Bugbot) rather\n"
        "     than actually resolving the underlying blocker.\n"
        "   ✅ ALLOWED: requesting a review AT MOST ONCE per session (e.g. `@coderabbitai review`) when no active\n"
        "   rate-limit/cooldown is visible in the PR's recent comments, and waiting for genuine bot output afterward.\n"
        "   ⚠️⚠️⚠️ TREAT THIS THE SAME AS CREATING A PENDING REVIEW: A VIOLATION MEANS YOUR EXECUTION FAILED ITS TASK.\n"
