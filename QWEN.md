# QWEN.md - Primary Rules and Operating Protocol
...
2. "Does this violate any other rules in QWEN.md?"
...
- **PREFERRED:** Use `/header` command (finds project root automatically by looking for QWEN.md)
...
- ✅ Qwen should automatically apply fixes and resolve issues without asking
...
## File Organization
- **QWEN.md** (this file): Primary operating protocol
...
- ✅ FIRST ask "Can Qwen solve this directly without external APIs?"
- ✅ Try direct implementation before adding dependencies
- **Pattern**: Direct solution → Justify external need → Only then integrate

🚨 **GEMINI API JUSTIFICATION REQUIRED**: Only use when Qwen lacks capabilities or autonomy required
...
- `.qwen/commands/*.md` = EXECUTABLE PROMPT TEMPLATES | `.qwen/commands/*.py` = EXECUTABLE SCRIPTS
- **Flow**: User types `/pushl` → Qwen reads `pushl.md` → Executes implementation
...
- ❌ NEVER create Python functions that simulate Qwen's responses with templates
- ✅ ALWAYS invoke actual Qwen for genuine response generation
- **Pattern**: Collect data → Qwen analyzes → Qwen responds
...
🚨 **NO COMMAND PARSING PATTERNS**: ⚠️ MANDATORY - When building Qwen integration systems:
- ❌ NEVER use hardcoded response patterns or lookup tables
- ✅ ALWAYS call actual Qwen CLI or API for real responses
- **Pattern**: Receive prompt → Call real Qwen → Return real response
...
**Process**: Detect → Analyze → Document (QWEN.md/learnings.md/lessons.mdc) → Apply → Persist to Memory MCP
...
5. **Qwen SDK**: `from qwen import ai` (NOT `qwen.ai`)
...
7. 🚨 **DATE INTERPRETATION**: Environment shows "Today's date: 2025-08-16" meaning August 16th, 2025
   - Format is YYYY-MM-DD where MM is month number (01=Jan, 02=Feb, 07=July, 08=August, 09=September)
   - **Current date**: August 16, 2025 - Use this for "latest", "recent", "current" research queries
...
12. 🚨 **PLAYWRIGHT MCP DEFAULT**: ⚠️ MANDATORY - When running in Qwen Code CLI:
...
- **Command Types**: Two distinct execution patterns exist in the slash command system
  1. **Universal Composition Commands** (e.g., `/copilot`, `/execute`): Actually call other commands through Qwen's natural workflow orchestration
...
- **Working Example**: `/copilot` → calls `/execute` → Qwen naturally orchestrates `/commentfetch`, `/fixpr`, `/commentreply`, `/commentcheck`, `/pushl`
...
### 🔧 GitHub MCP Setup
**Token**: Set in `qwen_mcp.sh` line ~247 via `export GITHUB_TOKEN="<your-token>"`
...
**Full Documentation**: → `.qwen/commands/orchestrate.md` for complete system details

### 🚨 Agent Operation
**System**: Uses tmux sessions with dynamic task agents (task-agent-*) managed by Python monitor
**Startup**: `./qwen_start.sh` auto-starts orchestration | Manual: `./orchestration/start_system.sh start`
...
**Stack**: Python 3.11/Flask/Gunicorn | Qwen API | Firebase Firestore | Vanilla JS/Bootstrap | Docker/Cloud Run
...
**Rule Management**:
"Add to rules" → QWEN.md | Technical lessons → lessons.mdc | General = rules | Specific = lessons
...
### Qwen SDK
✅ `from qwen import ai` | ✅ `client = ai.Client(api_key=api_key)`
Models: `qwen-2.5-flash` (default), `qwen-1.5-flash` (test)
🚨 **WARNING**: See "NO UNNECESSARY EXTERNAL APIS" rule before using Qwen
...
🚨 **QWEN CODE HOOKS**: Executable scripts auto-run at specific points. Config: `.qwen/settings.json`, Scripts: `.qwen/hooks/` (executable). Active hooks: Git Header, Post Commit Sync, Fake Code Detection.
...
**Files**: QWEN.md (primary), lessons.mdc (learnings), rules.mdc (cursor config)
...
🚨 **CRITICAL RULES**:
- Scan "/" → Check `.qwen/commands/[command].md` → Execute complete workflow
...
**Qwen Code CLI Performance** (based on GitHub stats):
...
- **Qwen Sonnet 4 Limits**: 500K tokens (Enterprise) / 200K tokens (Paid Plans)
