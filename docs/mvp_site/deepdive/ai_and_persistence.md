...
| File | Public API | Responsibility | Keep? |
| --- | --- | --- | --- |
| `mocks/mock_llm_service.py` | `MockGeminiClient`, `get_mock_client`, `parse_state_updates_from_response` | Provides deterministic Gemini responses for tests. | Keep to support regression suites. 【F:mvp_site/mocks/mock_llm_service.py†L1-L200】 |
| `mocks/mock_llm_service_wrapper.py` | `get_client`, `generate_content`, `get_initial_story`, `continue_story` | Wrapper exposing the mock Gemini client with the same interface as real service. | Keep. 【F:mvp_site/mocks/mock_llm_service_wrapper.py†L1-L200】 |
