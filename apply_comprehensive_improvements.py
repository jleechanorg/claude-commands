...
def apply_comprehensive_improvements(input_file, output_file):
    ...
    emotion_patterns = [
        (r'She was, to her immense surprise, somewhat touched', 'She found herself hesitating, her prepared response dying on her lips'),
        (r'\bfelt a surge of pride\b', 'straightened slightly, satisfaction flickering across her features'),
        (r'\bwas deeply moved\b', 'felt something shift in her chest, unexpected and unwelcome'),
    ...
    content = re.sub(r'"Your orders, my Lady\?"', '"What\'s the plan, Commander?"', content)
    ...
    content = re.sub(r'"The situation requires"', '"The permutations of circumstance demand"', content)
    content = re.sub(r'"It is logical"', '"The calculus of probability suggests"', content)
    content = re.sub(r'"We must consider"', '"The variables before us dictate"', content)
    
    # Make young guards more nervous
    content = re.sub(r'"Yes, my Lady\. As you command\."', '"I— yes, my lady. As you command."', content)
    ...
    exposition_patterns = [
        ...
        (r'The military situation', 
         'The reports that arrived with each dawn'),
    ...
    threat_patterns = [
        ...
        (r'\bforbidden knowledge\b', 'knowledge that carved the Whispering Scars into the earth itself'),
        (r'\bterrible consequences\b', 'consequences that still echo in the Blighted Reaches'),
    ...
    combat_context = [
        (r'\btwelve hunters\b', 'twelve hunters—odds that would have been trivial at full strength'),
        (r'The hunters cornered', 'The hunters, emboldened by her obvious exhaustion, cornered'),
        (r'She moved with deadly', 'Despite the fire in her wounded side, she moved with deadly'),
        (r'faced the Apostate', 'faced the Apostate, her strength finally restored after days of recovery,'),
    ]
    
    for pattern, replacement in combat_context:
        content = re.sub(pattern, replacement, content, count=1)  # Only first occurrence
    
    # 10. VARIED INTERNAL STATES - Break monotonous thought patterns
    thought_variations = [
        (r'Her mind analyzed', 'Instinct screamed danger before her mind could analyze'),
        (r'She calculated the', 'Dread settled in her stomach as she calculated the'),
        (r'The strategic implications', 'A chill of recognition preceded her analysis of the strategic implications'),
        (r'Her tactical assessment', 'Fury clouded her judgment for a moment before her tactical assessment'),
    ]
    
    for pattern, replacement in thought_variations:
        content = re.sub(pattern, replacement, content, count=1)  # Add variety, not replace all
