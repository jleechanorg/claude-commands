...
# See: https://github.com/$USERorg/claude_llm_proxy
...
start_game_mcp_background() {
    ...
    local LOG_DIR="/tmp/$PROJECT_NAME/${CURRENT_BRANCH}"
...
if is_game_mcp_running; then
    ...
    if start_game_mcp_background; then
        ...
        if is_game_mcp_running; then
            ...
            LOG_DIR="/tmp/$PROJECT_NAME/${CURRENT_BRANCH}"
    ...
    case $MODE in
        ...
        qwen)
            ...
            if [ ! -f "$API_PROXY_PATH" ]; then
                ...
                echo -e "${BLUE}💡 Clone it with: cd ~/projects && git clone https://github.com/$USERorg/claude_llm_proxy.git${NC}"
                echo -e "${BLUE}💡 Repository: https://github.com/$USERorg/claude_llm_proxy${NC}"
            ...
            if curl -s --connect-timeout 3 http://localhost:8000/health > /dev/null 2>&1; then
                ...
                if [ -z "$API_BASE_URL" ]; then
                    ...
                    INSTANCE_CMD="vastai create instance $BEST_INSTANCE --image pytorch/pytorch:2.1.0-cuda12.1-cudnn8-devel --disk 60 --ssh --label qwen-$(date +%Y%m%d-%H%M) $ENV_VARS --env GIT_REPO=https://github.com/$USERorg/claude_llm_proxy.git --onstart-cmd 'git clone \$GIT_REPO /app && cd /app && bash startup_llm.sh'"
        ...
        if curl -s --connect-timeout 3 http://localhost:8000/health > /dev/null 2>&1; then
            ...
            2)
                ...
                INSTANCE_CMD="vastai create instance $BEST_INSTANCE \\
                    ...
                    --ssh \\
                    $ENV_VARS \\
                    --env GIT_REPO=https://github.com/$USERorg/claude_llm_proxy.git \\
