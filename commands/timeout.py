...
class TimeoutOptimizer:
    ...
    def generate_optimization_summary(self, config: Dict) -> str:
        ...
        if config['multiedit_threshold'] <= 2:
            optimizations.append(f"MultiEdit enforced for >{config['multiedit_threshold']} edits")
        
        if config['max_file_lines'] < 100:
            optimizations.append(f"File reads limited to {config['max_file_lines']} lines")
    ...
    def generate_behavior_prompt(self) -> str:
        ...
        prompt_parts.append(f"- Thinking limit: {config['thinking_limit']} thoughts maximum")
        ...
        if config['tool_batching']:
            ...
            prompt_parts.append(f"- Use MultiEdit for >{config['multiedit_threshold']} file edits")
        ...
        if config['response_format'] == 'actions_only':
            ...
            prompt_parts.append(f"- Max {config['context_lines']} lines context per item")
