...
'''
    ...
    def _generate_readme(self):
        ...
        # Remove any existing version history sections to ensure single placement at bottom
        # Remove emoji version (current format)
        version_pattern = r'## 📚 Version History.*?(?=\n## |\Z)'
        readme_content = re.sub(version_pattern, '', readme_content, flags=re.DOTALL)
        
        # Remove plain version (old format)  
        version_pattern = r'## Version History.*?(?=\n## |\Z)'
        readme_content = re.sub(version_pattern, '', readme_content, flags=re.DOTALL)
