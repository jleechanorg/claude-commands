...
class ClaudeCommandsExporter:
    ...
    def _detect_version(self):
        ...
        # Strategy 4: Generate version based on export statistics and date
        # Use year.month.commands_count format for meaningful versioning
        year = time.strftime('%Y')
        month = time.strftime('%m')
        version = f"{year[-1:]}.{int(month)}.{self.commands_count}"
        print(f"   📋 Generated version based on date and export stats: {version}")
        return version
    
    def _increment_version(self, version):
        """Increment patch version for new export"""
        try:
    ...
    def _replace_llm_placeholders(self, content):
        ...
        # Generate comprehensive version history entry
        version_entry = f'''### v{version} ({current_date})
