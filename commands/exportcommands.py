...
'''
    ...
    def _generate_readme(self):
        ...
        # Remove any existing version history to ensure single placement
        if '## 📚 Version History' in readme_content:
            # Remove existing version history section (everything from the header to the next ## header or end)
            version_pattern = r'## 📚 Version History.*?(?=## |\Z)'
            readme_content = re.sub(version_pattern, '', readme_content, flags=re.DOTALL)
        
        # Also remove old format version history if it exists
        if '## Version History' in readme_content:
            version_pattern = r'## Version History.*?(?=## |\Z)'
            readme_content = re.sub(version_pattern, '', readme_content, flags=re.DOTALL)
