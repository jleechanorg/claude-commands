...
'''
    ...
    def _create_export_branch(self):
        ...
        subprocess.run(['git', 'config', 'user.email', 'noreply@anthropic.com'], check=True, shell=True)
        subprocess.run(['git', 'config', 'user.name', 'Claude Code Export'], check=True, shell=True)
        ...
        subprocess.run(['git', 'checkout', '-b', self.export_branch], check=True, shell=True)
    ...
    def _commit_and_push(self):
        ...
        try:
            ...
            result = subprocess.run(['git', 'status', '--porcelain'], capture_output=True, text=True, shell=True)
        ...
        status_result = subprocess.run(['git', 'status', '--porcelain'], capture_output=True, text=True, shell=True)
...
Orchestration Generated with Claude Code CLI"""
        ...
        # Push branch
        subprocess.run(['git', 'push', '-u', 'origin', self.export_branch], check=True, shell=True)
