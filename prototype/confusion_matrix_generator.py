...
def generate_confusion_matrix(validator_name: str, predictions: Dict, ground_truth: Dict) -> Dict:
    ...
    for test_id, truth in ground_truth.items():
        ...
        # Analyze each entity
        all_entities = set(truth.get("entities_found", []) + truth.get("entities_missing", []))
...
def generate_all_confusion_matrices():
    ...
    for validator_name, predictions in validator_predictions.items():
        matrix = generate_confusion_matrix(validator_name, predictions, ground_truth_labels)
