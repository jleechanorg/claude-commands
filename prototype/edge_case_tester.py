...
def test_edge_cases():
    ...
    for edge_case in edge_cases:
        ...
        for validator, results in validator_results.items():
            ...
            if set(predicted) == set(expected):
                report["edge_case_test_report"]["challenge_analysis"][challenge]["validators_handling_well"].append(validator)
            else:
                report["edge_case_test_report"]["challenge_analysis"][challenge]["validators_struggling"].append(validator)
    
    # Generate recommendations
    if "Only pronouns used" in report["edge_case_test_report"]["challenge_analysis"]:
        struggling = report["edge_case_test_report"]["challenge_analysis"]["Only pronouns used"]["validators_struggling"]
        if len(struggling) > 2:
    ...
    if "Names cut off mid-word" in report["edge_case_test_report"]["challenge_analysis"]:
        handling_well = report["edge_case_test_report"]["challenge_analysis"]["Names cut off mid-word"]["validators_handling_well"]
...
if __name__ == "__main__":
    ...
    for validator, data in report["edge_case_test_report"]["validators"].items():
        print(f"\n{validator}:")
        print(f"  Accuracy: {data['accuracy']:.1%} ({data['correct_count']}/{data['total_count']})")
