...
class SchemaValidator:
    ...
    def _manual_validate(self, result: Dict[str, Any]) -> List[str]:
        ...
        required_fields = ["all_entities_present", "entities_found", 
                          "entities_missing", "confidence"]
        for field in required_fields:
            if field not in result:
                errors.append(f"Missing required field: {field}")
        
        # Check field types
        if "all_entities_present" in result and not isinstance(result["all_entities_present"], bool):
            errors.append("all_entities_present must be boolean")
            
        if "entities_found" in result and not isinstance(result["entities_found"], list):
            errors.append("entities_found must be a list")
            
        if "entities_missing" in result and not isinstance(result["entities_missing"], list):
    ...
    def create_valid_result(self, 
        ...
        # Add optional fields
        for key in ["entity_references", "entity_states", "errors", 
                   "warnings", "metadata", "validation_details"]:
            if key in kwargs:
