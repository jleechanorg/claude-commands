...
def calculate_accuracy_metrics():
    ...
    report = {
        "accuracy_test_report": {
            ...
            "summary": {
                "best_f1": max(validator_metrics.items(), key=lambda x: x[1]["f1_score"])[0],
                "best_precision": max(validator_metrics.items(), key=lambda x: x[1]["precision"])[0],
                "best_recall": max(validator_metrics.items(), key=lambda x: x[1]["recall"])[0],
                "rankings": sorted(
                    [(name, metrics["f1_score"]) for name, metrics in validator_metrics.items()],
                    key=lambda x: x[1],
                    reverse=True
                )
            }
...
if __name__ == "__main__":
    ...
    print("=" * 50)
    print(f"Validators tested: {len(report['accuracy_test_report']['validator_metrics'])}")
    print("\nF1 Scores:")
    for name, f1 in report['accuracy_test_report']['summary']['rankings']:
        metrics = report['accuracy_test_report']['validator_metrics'][name]
        print(f"  {name}: F1={f1:.3f} (P={metrics['precision']:.3f}, R={metrics['recall']:.3f})")
