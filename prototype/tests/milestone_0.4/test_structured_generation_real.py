...
class RealLLMTestHarness(TestHarness):
            ...
            if isinstance(npcs, dict):
                ...
                for npc_name, npc_data in npcs.items():
                    if isinstance(npc_data, dict):
                        ...
                        if (
                            npc_data.get("present", True)
                            and npc_data.get("conscious", True)
                            and npc_data.get("hp", 1) > 0
                            and not npc_data.get("hidden", False)
                        ):
                ...
                for npc in npcs:
                    if isinstance(npc, dict) and "name" in npc:
                        ...
                        if npc.get("hp_current", 1) > 0 and not npc.get(
                            "hidden", False
                        ):
    ...
    def _generate_real_narrative(
        self, prompt: str, approach: TestApproach
    ...
    def _get_system_instruction(self, approach: TestApproach) -> str:
        ...
        if approach == TestApproach.PYDANTIC_ONLY:
...
- location_confirmed: the current location name"""
    ...
    def _run_validation_only(
        ...
        narrative, api_metrics = self._generate_real_narrative(
            prompt, TestApproach.VALIDATION_ONLY
        )
        ...
        logger.info(
            f"Expected entities: {self._extract_expected_entities(scenario['game_state'])}"
        )
        ...
        return TestResult(
            ...
            estimated_tokens=api_metrics.get("input_tokens", 0)
            + api_metrics.get("output_tokens", 0),
            ...
            desync_pattern="missing_entities"
    ...
    def _run_pydantic_only(
        ...
        manifest = create_from_game_state(
            scenario["game_state"], scenario.get("session", 1), scenario.get("turn", 1)
        )
        ...
        # Generate with real API
        narrative, api_metrics = self._generate_real_narrative(
            prompt, TestApproach.PYDANTIC_ONLY
        )
        ...
        structured_data = (
            self.gemini_client.parse_structured_response(narrative, "json")
    ...
    def _run_combined(self, campaign_id: str, scenario: dict[str, Any]) -> TestResult:
        ...
        # Generate with real API
        response_text, api_metrics = self._generate_real_narrative(
            prompt, TestApproach.COMBINED
        )
        generation_time = api_metrics["duration"] * 1000

        # Parse structured response if present
        structured_data = (
            self.gemini_client.parse_structured_response(response_text, "json")
            if self.gemini_client
            else None
        )
        ...
        entities_found = (
            entities_from_json
            if entities_from_json
            else validation_result.entities_found
        )
    ...
    def run_limited_tests(
        self, campaigns: list[str], scenarios: list[str], approach: TestApproach
    ) -> list[TestResult]:
        ...
        for campaign_id in campaigns:
            for scenario_id in scenarios:
                logger.info(
                    f"Testing {campaign_id} - {scenario_id} with {approach.value}"
                )
