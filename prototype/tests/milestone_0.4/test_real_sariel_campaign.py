...
def main():
    ...
    for approach in [TestApproach.VALIDATION_ONLY, TestApproach.PYDANTIC_ONLY, TestApproach.COMBINED]:
        print(f"\n{'='*70}")
        ...
        for i, interaction in enumerate(campaign_data['interactions'][:10]):
            ...
            try:
                ...
                if approach == TestApproach.VALIDATION_ONLY:
                    ...
                    system_instruction = "Generate narrative with entity tracking in JSON format."
                else:  # COMBINED
                    system_instruction = "Generate narrative ensuring all entities are mentioned, return JSON."
                
                response = harness.gemini_client.generate_narrative(prompt, system_instruction)
                ...
                if response["success"]:
                    ...
                    entities_missing = [e for e in expected_entities if e not in entities_found]
    ...
    for approach, data in results_by_approach.items():
        ...
        if failures:
            print(f"  Failed interactions:")
            for f in failures[:3]:
                print(f"    • #{f['interaction']}: \"{f['player_input']}\" - Missing {f['missing']}")
    ...
    # Save results
    output_file = f"real_sariel_campaign_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
