...
from test_structured_generation import TestHarness, TestApproach
...
def test_all_approaches():
    ...
    for i, approach in enumerate(TestApproach):
        r = results[i]
        print(f"{approach.value:20} | Success: {r.success_rate:6.1%} | Time: {r.total_time_ms:6.2f}ms | Cost: ${r.estimated_cost:.4f}")
    
    # Calculate improvements
    baseline = results[0].success_rate
    pydantic_improvement = (results[1].success_rate - baseline) / baseline * 100 if baseline > 0 else 0
    combined_improvement = (results[2].success_rate - baseline) / baseline * 100 if baseline > 0 else 0
