...
class TestHarness:
    ...
    def _load_config(self, config_path: Optional[str]) -> Dict[str, Any]:
        ...
        return {
            ...
            "scenarios": ["multi_character", "split_party", "combat_injured",
                         "hidden_characters", "npc_heavy"],
    ...
    def run_all_tests(self):
        ...
        self.logger.info(f"Starting test harness with {len(self.config['campaigns'])} campaigns")
        
        for campaign_id in self.config['campaigns']:
            for scenario_id in self.config['scenarios']:
                for approach in self.config['approaches']:
                    self._run_single_test(campaign_id, scenario_id, TestApproach(approach))
        
        self._save_results()
        self._generate_summary()
    
    def _run_single_test(self, campaign_id: str, scenario_id: str, approach: TestApproach):
        """Run a single test"""
    ...
    def _run_validation_only(self, campaign_id: str, scenario: Dict[str, Any]) -> TestResult:
        ...
        narrative = self._generate_mock_narrative(game_state, approach=TestApproach.VALIDATION_ONLY)
    ...
    def _run_pydantic_only(self, campaign_id: str, scenario: Dict[str, Any]) -> TestResult:
        ...
        result = TestResult(
            ...
            validation_time_ms=validation_time,
            total_time_ms=(time.time() - start_time) * 1000,
            estimated_tokens=len(narrative.split()) * 1.3 + len(structured_prompt.split()) * 1.3,
    ...
    def _generate_structured_narrative(self, manifest: SceneManifest, prompt: str) -> str:
    ...
    def _run_combined(self, campaign_id: str, scenario: Dict[str, Any]) -> TestResult:
        ...
        result = TestResult(
            ...
            entities_missing=validation_result.entities_missing,
            desync_detected=len(validation_result.entities_missing) > 0,
            desync_pattern="combined_miss" if validation_result.entities_missing else None,
            narrative_length=len(narrative),
            narrative_excerpt=narrative[:200] + "..." if len(narrative) > 200 else narrative
        )
...
def main():
    ...
    print(f"  Total tests: {len(harness.config['campaigns']) * len(harness.config['scenarios']) * len(harness.config['approaches'])}")
