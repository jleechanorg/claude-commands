...
from config import (
    get_api_key, TEST_MODEL, MAX_TOKENS, TEMPERATURE,
    MAX_RETRIES, TIMEOUT_SECONDS, SAFETY_SETTINGS,
    COST_PER_1K_INPUT_TOKENS, COST_PER_1K_OUTPUT_TOKENS,
    MAX_BUDGET_USD, WARNING_THRESHOLD_USD
)
...
class CostTracker:
    ...
    def add_usage(self, input_tokens: int, output_tokens: int):
        ...
        # Log usage
        logger.info(f"API Call #{self.call_count}: {input_tokens} in, {output_tokens} out, ${call_cost:.4f}")
        
        # Check thresholds
        if self.total_cost_usd >= MAX_BUDGET_USD:
            raise Exception(f"BUDGET EXCEEDED: ${self.total_cost_usd:.2f} >= ${MAX_BUDGET_USD}")
        elif self.total_cost_usd >= WARNING_THRESHOLD_USD:
            logger.warning(f"BUDGET WARNING: ${self.total_cost_usd:.2f} of ${MAX_BUDGET_USD} used")
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_cost_per_call": round(self.total_cost_usd / max(1, self.call_count), 4),
            "duration_seconds": round(duration, 1),
            "budget_used_percent": round((self.total_cost_usd / MAX_BUDGET_USD) * 100, 1)
        }
...
class GeminiClient:
    ...
    def generate_narrative(self, prompt: str, system_instruction: Optional[str] = None) -> Dict[str, Any]:
    ...
    def parse_structured_response(self, response_text: str, format_type: str = "json") -> Optional[Dict]:
