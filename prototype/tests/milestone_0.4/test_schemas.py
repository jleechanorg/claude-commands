...
from schemas.entities_simple import (
    PlayerCharacter, NPC, Location, SceneManifest, 
    HealthStatus, Stats, EntityStatus, Visibility,
...
def test_basic_schemas():
    ...
    print(f"✓ Created scene manifest with {len(scene.player_characters)} PC(s) and {len(scene.npcs)} NPC(s)")
...
def test_game_state_conversion():
    ...
    print(f"✓ Converted game state to manifest")
    ...
    print(f"  - Combat: {manifest.combat_state.get('in_combat', False) if manifest.combat_state else False}")
    
    # Check visibility
    hidden_npcs = [npc for npc in manifest.npcs if npc.visibility == Visibility.INVISIBLE]
