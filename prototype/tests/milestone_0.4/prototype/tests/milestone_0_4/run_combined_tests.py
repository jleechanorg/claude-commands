...
def run_combined_tests():
    ...
    try:
        with open("analysis/test_results/baseline_test_results.json", 'r') as f:
            ...
            improvements["baseline"] = ((overall_success - baseline_success) / baseline_success * 100) if baseline_success > 0 else 0
    except:
        pass
        
    try:
        with open("analysis/test_results/pydantic_test_results.json", 'r') as f:
            pydantic = json.load(f)
            pydantic_success = pydantic["summary"]["average_success_rate"]
            improvements["pydantic"] = ((overall_success - pydantic_success) / pydantic_success * 100) if pydantic_success > 0 else 0
    except:
        pass
    ...
    if 'baseline' in improvements and 'pydantic' in improvements:
        if improvements['baseline'] > 0 and improvements['pydantic'] > 0:
            print(f"  Combined approach outperforms both individual approaches!")
        elif improvements['pydantic'] < 0:
            print(f"  Validation layer may be hindering structured generation")
        else:
            print(f"  Marginal improvement over structured approach alone")
    
    # Overhead analysis
    avg_validation_time = sum(r.validation_time_ms for r in all_results) / len(all_results)
    ...
    combined_report = {
        ...
        "synergy_metrics": {
            ...
            "beats_both_approaches": improvements.get('baseline', 0) > 0 and improvements.get('pydantic', 0) > 0
        }
    }
