...
def run_pydantic_tests():
    ...
    simple_success = sum(
        r.success_rate for s in simple_scenarios for r in results_by_scenario.get(s, [])
    ) / (len(simple_scenarios) * len(campaigns))
    complex_success = sum(
        r.success_rate
        for s in complex_scenarios
        for r in results_by_scenario.get(s, [])
    ) / (len(complex_scenarios) * len(campaigns))
