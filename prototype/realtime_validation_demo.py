...
class GameSession:
    ...
    def simulate_player_action(self, action: str) -> Dict:
        ...
        result = self.narrative_service.generate_narrative_with_validation(
            self.game_state,
            f"Describe the result of: {action}"
        )
    ...
    def get_performance_metrics(self) -> Dict:
        ...
        return {
            ...
            "validation_accuracy": 
                self.validation_stats["valid_narratives"] / 
                self.validation_stats["total_narratives"],
            "desyncs_prevented": self.validation_stats["desync_prevented"],
            "avg_validation_time_ms": 
                (self.validation_stats["total_time"] * 1000) / 
                self.validation_stats["total_narratives"],
            "estimated_hourly_cost": 
                self.validation_stats["total_narratives"] * 0.000175 * 
...
def simulate_gameplay_session():
    ...
    for i, action in enumerate(actions, 1):
        ...
        if i > 1:  # Skip first action
            ...
            if any("hidden" in states for states in result['game_state']['states'].values()):
