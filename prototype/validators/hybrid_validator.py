...
class HybridValidator(BaseValidator):
    ...
    def __init__(self, 
                 token_validator: Optional[BaseValidator] = None,
                 fuzzy_validator: Optional[BaseValidator] = None,
                 llm_validator: Optional[BaseValidator] = None,
    ...
    def _combine_results(self, results: Dict[str, ValidationResult]) -> ValidationResult:
        ...
        elif self.combination_strategy == "unanimous":
        ...
        elif self.combination_strategy == "confidence_based":
    ...
    def _weighted_vote_combination(self, results: Dict[str, ValidationResult]) -> ValidationResult:
        ...
        weights = [self.validator_weights.get(name, 0.33) for name in results.keys()]
    ...
    def _unanimous_combination(self, results: Dict[str, ValidationResult]) -> ValidationResult:
    ...
    def _majority_vote_combination(self, results: Dict[str, ValidationResult]) -> ValidationResult:
        ...
        for entity in all_entities:
            ...
            missing_votes = sum(1 for r in results.values() if entity in r.entities_missing)
    ...
    def _confidence_based_combination(self, results: Dict[str, ValidationResult]) -> ValidationResult:
        ...
        combined.entity_states = best_result.entity_states.copy() if best_result.entity_states else {}
    ...
    def _merge_entity_states(self, results: Dict[str, ValidationResult]) -> Dict[str, List[str]]:
    ...
    def validate(self, 
        ...
        for name, validator in self.validators.items():
            try:
                ...
                result = validator.validate(narrative_text, expected_entities, location, **kwargs)
                results[name] = result
                self.logger.info(f"{name} found: {result.entities_found}, confidence: {result.confidence:.2f}")
