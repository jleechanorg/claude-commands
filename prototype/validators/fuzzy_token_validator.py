...
class FuzzyTokenValidator(BaseValidator):
    ...
    def _find_role_based_matches(self, text: str, entity_name: str) -> list[dict]:
        ...
        if likely_role and likely_role in self.role_patterns:
            for role_term in self.role_patterns[likely_role]:
                pattern = re.compile(
                    r"\b" + re.escape(role_term) + r"\b", re.IGNORECASE
                )
    ...
    def _contextual_pronoun_resolution(
        self, text: str, pronouns: list[dict], entities: list[str]
    ) -> dict[str, list[dict]]:
        """
    ...
    def validate(
        ...
        **kwargs,
        ...
        for entity in expected_entities:
            ...
            entity_record = next(
                (e for e in manifest.entities if e["name"] == entity), None
            )

            if entity_record:
                descriptors = entity_record["descriptors"]
                exact_matches = find_entity_mentions(
        ...
        low_confidence_matches = [
            m for m in all_matches if m.get("confidence", 1.0) < 0.7
        ]
