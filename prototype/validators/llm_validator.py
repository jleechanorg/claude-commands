...
class LLMValidator(BaseValidator):
    ...
    def __init__(self, gemini_service=None, use_simple_prompt=False, 
                 api_key_path=None, timeout=30.0, max_retries=3,
                 retry_delay=1.0):
    ...
    def _create_prompt(self, narrative_text: str, expected_entities: List[str]) -> str:
        ...
        if self.use_simple_prompt:
            return SIMPLE_PROMPT_TEMPLATE.format(
                expected_entities=entities_str,
    ...
    def _fallback_parse(self, response_text: str) -> Dict[str, Any]:
        ...
        result["entities_found"] = list(set(
            e.capitalize() for e in result["entities_found"] 
            if e and len(e) > 2
        ))
        result["entities_missing"] = list(set(
            e.capitalize() for e in result["entities_missing"] 
            if e and len(e) > 2
        ))
                ...
                if self.gemini_service:
                    ...
                    with concurrent.futures.ThreadPoolExecutor() as executor:
                        ...
                        try:
                            ...
                            self.logger.warning(f"Attempt {attempt + 1}/{self.max_retries} timed out")
