...
def test_browser_simulation():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for camp in campaigns[:3]:  # Show first 3
                print(f"   - {camp.get('title', 'Untitled')} (ID: {camp.get('id', 'unknown')})")
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            if response.status_code == 200:
                ...
                print(f"   State: {json.dumps(campaign_details.get('state', {}), indent=2)[:200]}...")
                
                capture_response_as_image(response, f"04_campaign_details_{timestamp}.txt")
                ...
                if response.status_code == 200:
                    ...
                    capture_response_as_image(response, f"05_story_response_{timestamp}.txt")
        else:
    ...
    print(f"✅ Server is responding with real data")
    print(f"✅ All HTTP interactions captured to files")
