...
def test_campaign_api_response_structure():
    ...
    print("📤 Creating campaign to analyze response structure...")
    ...
    if response.status_code == 201:
        ...
        for key, value in creation_response.items():
            ...
            if isinstance(value, str | int | bool):
                ...
                print(
                    f"   {key}: {value_type} (length: {len(value) if hasattr(value, '__len__') else 'N/A'})"
                )
            ...
            if "story" in campaign_details and campaign_details["story"]:
                ...
                if isinstance(story_data, list) and story_data:
                    ...
                    if isinstance(first_story_entry, dict):
                        print(
                            f"   Story entry structure: {list(first_story_entry.keys())}"
                        )
...
def test_api_endpoint_availability():
    ...
    for endpoint in endpoints_to_test:
        ...
        try:
                ...
                if path == "/api/campaigns" and response.status_code == 201:
                    try:
                        ...
                        if campaign_id:
                            ...
                            print(
                                f"📍 Testing GET {retrieval_path} (Campaign retrieval)..."
                            )
                            ...
                            if get_response.status_code == 200:
                                print(
                                    f"   ✅ {get_response.status_code} - Campaign retrieval available"
                                )
                            else:
                                print(
                                    f"   ❌ {get_response.status_code} - Campaign retrieval failed"
                                )
            ...
            results[path] = {"status": "error", "success": False, "error": str(e)}
