...
def test_wizard_character_setting_display():
    ...
    print(f"📤 Testing character/setting display via API...")
    ...
    if response.status_code == 201:
        ...
        if get_response.status_code == 200:
            ...
            if campaign_content:
                try:
                    validate_character_name_display(campaign_content, expected_character, "http")
                    print(f"✅ Character name display validation passed!")
                    print(f"   Expected character '{expected_character}' found in content")
                ...
                try:
                    ...
                    raise Exception("Character/setting display bug: Dragon Knight content in custom campaign")
                ...
                if expected_setting in campaign_content:
                    ...
                    print(f"⚠️  Setting '{expected_setting}' not found in content (may be normal for async generation)")
            
            else:
                print("⚠️  No content found for character/setting validation (may be normal for async generation)")
...
def test_red_green_character_display():
    ...
    print(f"   Creating campaign with empty character: '{red_campaign_data['character_name']}'")
    ...
    if red_response.status_code == 201:
        ...
        red_campaign_id = validate_campaign_created_successfully(red_campaign_response, "http")
        ...
        if red_get_response.status_code == 200:
            ...
            try:
                ...
                print("🔴 RED TEST RESULT: PASS - No Dragon Knight defaults (bug is fixed)")
                red_result = "PASS"
            except AssertionError as e:
                print(f"🔴 RED TEST RESULT: FAIL - Dragon Knight defaults detected: {e}")
                red_result = "FAIL"
    else:
    ...
    if green_response.status_code == 201:
        ...
        green_campaign_id = validate_campaign_created_successfully(green_campaign_response, "http")
        
        # Check GREEN test results
        green_get_response = session.get(f"{base_url}/api/campaigns/{green_campaign_id}")
        if green_get_response.status_code == 200:
            ...
            if character_found and no_contamination:
                print("🟢 GREEN TEST RESULT: PASS - Custom character displayed correctly")
                green_result = "PASS"
            elif character_found and not no_contamination:
                print("🟢 GREEN TEST RESULT: PARTIAL - Character found but Dragon Knight contamination")
                green_result = "PARTIAL"
            elif not character_found and no_contamination:
                print("🟢 GREEN TEST RESULT: PARTIAL - No contamination but character not found")
                green_result = "PARTIAL"
            else:
                print("🟢 GREEN TEST RESULT: FAIL - Character not found and contamination present")
                green_result = "FAIL"
    else:
    ...
    if response.status_code == 201:
        ...
        if get_response.status_code == 200:
            ...
            if custom_setting in campaign_content:
                ...
                print(f"⚠️  Custom setting '{custom_setting}' not found (may be normal for async generation)")
            
            # Check that it doesn't use default world setting incorrectly
            default_world_indicators = ["world of assiah", "assiah", "default world"]
            found_defaults = [indicator for indicator in default_world_indicators 
                            if indicator.lower() in campaign_content.lower()]
            
            if found_defaults and custom_setting.lower() not in campaign_content.lower():
                print(f"⚠️  Default world indicators found when custom setting expected: {found_defaults}")
            else:
