...
def test_campaign_creation_workflow():
    ...
    homepage_response = session.get(f"{base_url}?test_mode=true&test_user_id={test_user_id}")
    ...
    if response.status_code == 201:
        ...
        if get_response.status_code == 200:
            ...
            print(f"✅ Campaign data retrieved")
            ...
            if 'story' in campaign_details and campaign_details['story']:
                ...
                if expected_character in story_content:
                    print(f"✅ Expected character '{expected_character}' found in story")
                else:
                    print(f"⚠️  Expected character '{expected_character}' not found in story (may be normal for async generation)")
...
def test_multiple_campaign_creation():
    ...
    for i, campaign_type in enumerate(campaign_types):
        ...
        if response.status_code == 201:
            ...
            raise Exception(f"Multiple campaign creation failed")
