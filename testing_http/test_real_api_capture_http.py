...
def test_real_api_campaign_creation():
    ...
    setup_test_environment(use_real_api=True, port="8087")  # Different port for real API tests
    ...
    print(f"📤 Creating campaign with REAL APIs for response capture")
    print(f"   Title: {campaign_data['title']}")
    print(f"   Character: {campaign_data['character_name']}")
    print(f"   Setting: {campaign_data['setting']}")
    print(f"💰 This will make actual Gemini API calls and cost money!")
    ...
    if response.status_code == 201:
        ...
        if get_response.status_code == 200:
            ...
            print(f"✓ Retrieved campaign data with keys: {list(campaign_details.keys())}")
            ...
            if 'narrative_history' in campaign_details:
                ...
                print(f"📖 Narrative history entries: {len(narrative_history) if narrative_history else 0}")
                ...
                if narrative_history and len(narrative_history) > 0:
                    ...
                    if isinstance(first_entry, dict):
                        ...
                        response_structure["narrative_structure"] = {
                            ...
                            "field_types": {k: type(v).__name__ for k, v in first_entry.items()}
                        }
                        
                        # Check for structured fields
                        structured_fields = ['planning_block', 'god_mode_response', 'session_header', 'resources', 'dice_rolls']
                        found_structured = [field for field in structured_fields if field in first_entry]
                        ...
                        if 'god_mode_response' in first_entry:
                            ...
                            if expected_character in god_mode_text:
                                print(f"✓ Character '{expected_character}' properly integrated in story")
                            else:
                                print(f"⚠️  Character '{expected_character}' not found in story text")
                        
                        # Sample content lengths for mock generation
                        content_lengths = {}
                        for field in found_structured:
                            if field in first_entry and isinstance(first_entry[field], str):
                                content_lengths[field] = len(first_entry[field])
                        
                        response_structure["content_analysis"] = {
                            "character_integrated": expected_character in str(first_entry),
            ...
            capture_filename = f"/tmp/worldarchitectai/real_api_capture_{int(time.time())}.json"
        ...
        raise Exception(f"Real API campaign creation failed with status {response.status_code}")
...
def test_real_api_performance_comparison():
    ...
    mock_session, _ = setup_http_test_session(mock_base_url, generate_test_user_id("mock-perf"))
    
    campaign_data = CAMPAIGN_TEST_DATA["custom_campaign"].copy()
    
    mock_start = time.time()
    mock_response = mock_session.post(f"{mock_base_url}/api/campaigns", json=campaign_data)
    ...
    if mock_response.status_code == 201:
        ...
        mock_get_response = mock_session.get(f"{mock_base_url}/api/campaigns/{mock_campaign_id}")
        ...
        real_session, _ = setup_http_test_session(real_base_url, generate_test_user_id("real-perf"))
        
        real_start = time.time()
        real_response = real_session.post(f"{real_base_url}/api/campaigns", json=campaign_data)
        ...
        if real_response.status_code == 201:
            ...
            real_get_response = real_session.get(f"{real_base_url}/api/campaigns/{real_campaign_id}")
