...
def test_campaign_creation_custom_type():
    ...
    if response.status_code == 201:
        ...
        assert campaign_data['success'] is True, f"Campaign creation not successful: {campaign_data.get('success')}"
        assert campaign_data['campaign_id'] is not None, "Campaign ID is None"
        
        print(f"✓ Response fields validated: success={campaign_data['success']}, campaign_id='{campaign_data['campaign_id']}'")
...
def test_campaign_api_retrieval():
    ...
    if response.status_code == 201:
        ...
        if get_response.status_code == 200:
            ...
            assert 'campaign' in retrieved_data, "Retrieved data missing campaign object"
            assert 'story' in retrieved_data, "Retrieved data missing story array"
            assert 'game_state' in retrieved_data, "Retrieved data missing game_state object"
            
            campaign_info = retrieved_data['campaign']
            assert 'title' in campaign_info, "Campaign object missing title field"
            assert 'created_at' in campaign_info, "Campaign object missing created_at field"
            ...
            print(f"✓ Campaign structure validated: title='{title_field}', created_at='{created_at_field}'")
            
            # Use shared validation for story content
            if 'narrative_history' in retrieved_data and retrieved_data['narrative_history']:
                if validate_story_content_exists(retrieved_data['narrative_history']):
                    print("✅ SUCCESS: Campaign creation and retrieval with story content!")
                else:
                    print("⚠️  Story content exists but may be too short")
                    print("✅ SUCCESS: Campaign creation and basic retrieval working!")
            else:
                print("⚠️  No narrative history in retrieved data (may be normal for async generation)")
                print("✅ SUCCESS: Campaign creation and basic retrieval working!")
