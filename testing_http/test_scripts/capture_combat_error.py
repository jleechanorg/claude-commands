...
normal_resp = requests.post(
    ...
    headers=headers,
    json={"input": "I look around and assess the situation.", "mode": "character"},
)
...
for i, combat_input in enumerate(combat_inputs, 1):
    ...
    if combat_resp.status_code == 500:
        try:
            ...
            if "AttributeError" in str(
                error_data
            ) and "'list' object has no attribute 'items'" in str(error_data):
                ...
                log(
                    "\n✅ Full error details saved to: /tmp/combat_error_full_details.json",
                    "SUCCESS",
                )
                ...
                log(
                    "This confirms PR #314's finding about the AttributeError!",
                    "SUCCESS",
                )
                break
                ...
                log(
                    f"Different 500 error: {error_data.get('error', 'Unknown')}",
                    "WARNING",
                )
        ...
        log(
            "Combat succeeded without error - bug may be fixed or not triggered",
            "WARNING",
        )
