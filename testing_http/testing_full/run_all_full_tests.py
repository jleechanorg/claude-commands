...
from test_config_full import validate_config, CostTracker
...
def run_test(test_file, log_file):
    ...
    if "TEST PASSED" in result.stdout:
        ...
        print(f"⚠️  WARNING: Could not determine test result from output, using exit code")
...
def main():
    ...
    with open(log_filename, 'w') as log_file:
        ...
        tee_print("\n💰 Cost Estimates:")
        tee_print(f"  - Per test: ~$0.001-0.002")
        tee_print(f"  - Total ({len(TEST_FILES)} tests): ~${len(TEST_FILES) * 0.0015:.3f}")
