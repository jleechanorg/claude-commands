...
def get_test_session():
    ...
    session.headers.update({
        ...
        'X-Test-User-ID': 'test-full-api-user'
    })
...
class CostTracker:
    ...
    def track_gemini(self, estimated_tokens=1000):
        ...
        if self.get_total_cost() > MAX_TEST_COST_USD:
            raise Exception(f"Cost limit exceeded: ${self.get_total_cost():.4f} > ${MAX_TEST_COST_USD}")
        
        # Check call limit
        if self.gemini_calls > MAX_GEMINI_CALLS_PER_TEST:
            raise Exception(f"Gemini call limit exceeded: {self.gemini_calls} > {MAX_GEMINI_CALLS_PER_TEST}")
    ...
    def print_summary(self):
        ...
        print(f"   Firestore: {self.firestore_reads} reads, {self.firestore_writes} writes")
