...
def test_red_empty_custom_campaign():
    ...
    print(f"📤 Creating RED test campaign (empty fields)")
    ...
    if response.status_code == 201:
        ...
        if get_response.status_code == 200:
            ...
            dragon_knight_indicators = ["ser arion", "dragon knight", "celestial imperium", "empress sariel"]
            ...
            if len(found_bug_indicators) > 0:
                print(f"🔴 RED TEST RESULT: Bug detected! Found Dragon Knight content: {found_bug_indicators}")
                ...
                print("   This suggests the bug is already fixed or doesn't occur in this scenario")
                red_result = "NO_BUG"
            ...
            raise Exception(f"RED campaign retrieval failed with status {get_response.status_code}")
...
def test_green_filled_custom_campaign():
    ...
    print(f"📤 Creating GREEN test campaign (properly filled)")
    ...
    if response.status_code == 201:
        ...
        if get_response.status_code == 200:
            ...
            if expected_character in full_content:
                print(f"✓ Expected character '{expected_character}' found in GREEN test")
                character_found = True
            else:
                print(f"⚠️  Expected character '{expected_character}' not found in GREEN test")
            ...
            if expected_setting in full_content:
                ...
                print(f"⚠️  Expected setting '{expected_setting}' not found in GREEN test")
                setting_found = False
            ...
            raise Exception(f"GREEN campaign retrieval failed with status {get_response.status_code}")
        
    else:
        print(f"❌ Failed to create GREEN campaign: {response.text}")
        raise Exception(f"GREEN campaign creation failed with status {response.status_code}")
...
def compare_red_green_results(red_result, green_result):
    ...
    if red_result == "NO_BUG" and green_result == "WORKING_CORRECTLY":
        ...
        return "PERFECT"
    elif red_result == "BUG_DETECTED" and green_result == "WORKING_CORRECTLY":
        print("⚠️  PARTIAL: Bug exists but workaround (filled fields) works")
        return "PARTIAL_FIX"
    elif red_result == "NO_BUG" and green_result == "ISSUES_DETECTED":
        print("🤔 UNEXPECTED: No bug in empty case but issues in filled case")
        return "UNEXPECTED"
    elif red_result == "BUG_DETECTED" and green_result == "ISSUES_DETECTED":
        print("❌ BROKEN: Bug exists and even workaround has issues")
        return "BROKEN"
    else:
        print(f"🤷 UNKNOWN: Unexpected combination of results")
        return "UNKNOWN"
