    ...
    for step_info in demo_steps:
        ...
        if step_num == 1:
            ...
            if get_response.status_code == 200:
                ...
                print(f"   📄 Retrieved campaign content ({len(campaign_content)} characters)")
                
                # Validate character display
                try:
                    validate_character_name_display(campaign_content, demo_expected_character, "http")
                    print(f"   ✅ Character '{demo_expected_character}' displayed correctly")
...
def demonstrate_before_after_comparison():
    ...
    print(f"   Creating campaign with empty character: '{before_data['character_name']}'")
    ...
    if before_response.status_code == 201:
        ...
        before_campaign_id = validate_campaign_created_successfully(before_campaign_response, "http")
        
        # Check BEFORE results
        before_get_response = session.get(f"{base_url}/api/campaigns/{before_campaign_id}")
        if before_get_response.status_code == 200:
            ...
            try:
                ...
                print("   ✅ BEFORE: No Dragon Knight contamination (bug already fixed)")
                before_result = "GOOD"
            except AssertionError as e:
    ...
    if after_response.status_code == 201:
        ...
        after_campaign_id = validate_campaign_created_successfully(after_campaign_response, "http")
        
        # Check AFTER results
        after_get_response = session.get(f"{base_url}/api/campaigns/{after_campaign_id}")
        if after_get_response.status_code == 200:
            ...
            if character_found and no_contamination:
                print("   ✅ AFTER: Custom character displayed correctly, no contamination")
