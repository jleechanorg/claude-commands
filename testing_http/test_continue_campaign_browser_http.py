...
def test_campaign_continuation_workflow():
    ...
    if create_response.status_code != 201:
        raise Exception(f"Failed to create initial campaign: {create_response.status_code}")
    ...
    if get_response.status_code == 200:
        ...
        validate_api_response_structure(campaign_details, ['campaign', 'game_state', 'story'])
        ...
        if 'story' in campaign_details and campaign_details['story']:
            ...
            if isinstance(story_entries, list) and len(story_entries) > 0:
                print(f"✅ Campaign has {len(story_entries)} story entries - ready for continuation")
            else:
                print("⚠️  Campaign has story data but no entries")
        else:
            print("⚠️  Campaign has no story content yet (may be normal for new campaigns)")
        ...
        continue_response = session.post(f"{base_url}/api/campaigns/{campaign_id}/actions", json=player_action)
        ...
        if continue_response.status_code in [200, 201]:
            ...
            print("⚠️  Campaign continuation endpoint not found - testing alternative approach")
...
def test_campaign_list_retrieval():
    ...
    list_endpoints = [
        f"/api/campaigns",  # Standard REST endpoint
        f"/api/user/campaigns",  # User-specific endpoint
                ...
                if isinstance(list_data, list):
                    ...
                    for key in possible_keys:
                        if key in list_data and isinstance(list_data[key], list):
                            ...
                            print(f"   ✅ Found campaigns in '{key}' with {campaign_count} items")
                            campaigns_found = True
                            break
                    if campaigns_found:
                        break
                    ...
                    found_campaign_ids = [cid for cid in created_campaigns if cid in response_str]
                    if found_campaign_ids:
                        print(f"   ✅ Found {len(found_campaign_ids)} campaign IDs in response")
                        campaigns_found = True
                        break
    ...
    if campaigns_found:
        print("✅ SUCCESS: Campaign list retrieval working - continuation dashboard functionality available")
    else:
        print("⚠️  Campaign list retrieval not found - may use different approach")
        print("   Note: Campaigns were created successfully, list endpoint may be different")
...
def test_campaign_state_persistence():
    ...
    for i in range(3):
        ...
        if get_response.status_code == 200:
            ...
            print(f"   ✅ Retrieved campaign data ({len(str(campaign_data))} characters)")
        else:
    ...
    if len(retrievals) == 3:
        ...
        consistent_id = all(
            retrieval.get('campaign', {}).get('id') == campaign_id or 
            retrieval.get('id') == campaign_id or
            campaign_id in str(retrieval)
        ...
        print(f"📊 State persistence analysis:")
