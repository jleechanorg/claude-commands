...
USER_ID = f"e2e-prompted-arc-{datetime.now(UTC).strftime('%Y%m%d%H%M%S')}"
OUTPUT_DIR = "/tmp/arc_milestones_prompted_e2e"  # noqa: S108
...
def capture_provenance() -> dict:
    ...
    try:
        provenance["git_head"] = subprocess.check_output(  # noqa: S603
            [GIT_CMD, "rev-parse", "HEAD"], text=True, timeout=5
        ).strip()
        provenance["git_branch"] = subprocess.check_output(  # noqa: S603
            [GIT_CMD, "branch", "--show-current"], text=True, timeout=5
        ).strip()
        provenance["git_status"] = subprocess.check_output(  # noqa: S603
            [GIT_CMD, "status", "-sb"], text=True, timeout=5
        ).strip()
        provenance["git_origin_main"] = subprocess.check_output(  # noqa: S603
            [GIT_CMD, "rev-parse", "origin/main"], text=True, timeout=5
        ).strip()
    ...
    provenance["env"] = {
        ...
        "CAPTURE_SYSTEM_INSTRUCTION": os.getenv("CAPTURE_SYSTEM_INSTRUCTION", "unset"),
    }
    return provenance
...
def mcp_call(method: str, params: dict) -> dict:
    ...
    payload = {
        "jsonrpc": "2.0",
        "id": f"{method}-{datetime.now(UTC).timestamp()}",
        "method": method,
        "params": params,
    }
...
def main():  # noqa: PLR0912, PLR0915
    ...
    try:
        ...
        log(f"  FAILED: {e}")
        results["steps"].append(
            {"name": "health_check", "passed": False, "error": str(e)}
        )
    ...
    if "arc_milestones" in game_state:
        initial_arc_milestones = game_state.get("arc_milestones")
    elif "arc_milestones" in custom_state:
        initial_arc_milestones = custom_state.get("arc_milestones")
    else:
        initial_arc_milestones = {}
    ...
    results["steps"].append(
        {
            "name": "action_1_enter",
            "passed": bool(action1_narrative)
            and "error" not in str(action1_narrative).lower(),
    ...
    results["steps"].append(
        {
            ...
            "passed": bool(action2_narrative)
            and "error" not in str(action2_narrative).lower(),
    ...
    custom_state = action3_game_state.get("custom_campaign_state") or {}
    if "arc_milestones" in action3_game_state:
        arc_milestones_after = action3_game_state["arc_milestones"]
    elif "arc_milestones" in custom_state:
        arc_milestones_after = custom_state.get("arc_milestones")
    else:
        arc_milestones_after = {}

    results["steps"].append(
        {
            "name": "action_3_complete_heist",
            "passed": bool(action3_narrative)
            and "error" not in str(action3_narrative).lower(),
    ...
    custom_state = final_game_state.get("custom_campaign_state") or {}
    if "arc_milestones" in final_game_state:
        final_arc_milestones = final_game_state["arc_milestones"]
    elif "arc_milestones" in custom_state:
        final_arc_milestones = custom_state.get("arc_milestones")
    else:
        final_arc_milestones = {}
    ...
    if final_arc_milestones and isinstance(final_arc_milestones, dict):
        ...
        for arc_name, arc_data in normalized_milestones.items():
            ...
            if arc_data.get("normalized"):
                ...
                log(
                    f"  ⚠️ Schema violation: '{arc_name}' was string, normalized to dict"
                )
    ...
    log(
        f"Steps: {results['summary']['steps_passed']}/{results['summary']['steps_total']} passed"
    )
