        ...
        with mock_environment(self.mock_port) as env:
            ...
            async def run_test():
                async with client:
                    ...
                    self.assertEqual(campaign_response['status'], 'success')
                    ...
                    self.assertEqual(campaigns_response['status'], 'success')
                    ...
                    self.assertEqual(created_campaign['name'], "End-to-End Test Campaign")
                    ...
                    self.assertEqual(campaign_details['status'], 'success')
                    self.assertEqual(campaign_details['data']['id'], campaign_id)
                    ...
                    self.assertEqual(campaign_state['status'], 'success')
                    self.assertEqual(campaign_state['data']['id'], campaign_id)
        ...
        with mock_environment(self.mock_port) as env:
            ...
            async def run_test():
                async with client:
                    ...
                    self.assertEqual(character_response['status'], 'success')
        ...
        with mock_environment(self.mock_port) as env:
            ...
            async def run_test():
                async with client:
                    ...
                    self.assertTrue(result_data.get('game_state_updated', False))
        ...
        with mock_environment(self.mock_port) as env:
            ...
            async def run_test():
                async with client:
                    ...
                    initial_response = await client.get_user_settings(user_id=self.test_user_id)
                    self.assertEqual(initial_response['status'], 'success')
                    ...
                    final_response = await client.get_user_settings(user_id=self.test_user_id)
                    self.assertEqual(final_response['status'], 'success')

                    final_settings = final_response['data']
                    self.assertEqual(final_settings['theme'], 'dark')
                    self.assertEqual(final_settings['notifications'], False)
            ...
            async def run_test():
                async with client:
                    ...
                    for export_format in export_formats:
                        ...
                        self.assertEqual(export_response['status'], 'success')
        ...
        with mock_environment(self.mock_port) as env:
            ...
            async def run_test():
                async with client:
                    ...
                    self.assertEqual(user1_campaign['status'], 'success')
                    ...
                    self.assertEqual(user2_campaign['status'], 'success')
                    ...
                    self.assertEqual(user1_campaigns['status'], 'success')

                    user1_campaign_ids = [c['id'] for c in user1_campaigns['data']['campaigns']]
                    self.assertIn(user1_campaign_id, user1_campaign_ids)
                    ...
                    self.assertEqual(user2_campaigns['status'], 'success')

                    user2_campaign_ids = [c['id'] for c in user2_campaigns['data']['campaigns']]
                    self.assertIn(user2_campaign_id, user2_campaign_ids)
                    ...
                    self.assertEqual(user1_settings['data']['user_id'], user1_id)
                    self.assertEqual(user2_settings['data']['user_id'], user2_id)
        ...
        with mock_environment(self.mock_port) as env:
            ...
            async def run_test():
                async with client:
                    ...
                    self.assertEqual(valid_campaign['status'], 'success')
                    ...
                    try:
                        ...
                        if invalid_char_response['status'] == 'error':
                            self.assertIn('not_found', invalid_char_response['error_type'])
                    except Exception:
                        # Expected for non-existent campaign
                        pass
                    ...
                    self.assertEqual(valid_character['status'], 'success')
        ...
        with mock_environment(self.mock_port) as env:
            ...
            async def run_test():
                async with client:
                    ...
                    self.assertGreaterEqual(successful_campaigns, num_campaigns * 0.8)  # 80% success rate
        ...
        with mock_environment(self.mock_port) as env:
            ...
            async def run_test():
                async with client:
                    ...
                    self.assertEqual(resource_campaign['status'], 'success')
                    resource_data = resource_campaign['data']

                    # Key fields should match
                    self.assertEqual(tool_data['id'], resource_data['id'])
                    self.assertEqual(tool_data['name'], resource_data['name'])
                    self.assertEqual(tool_data['description'], resource_data['description'])
