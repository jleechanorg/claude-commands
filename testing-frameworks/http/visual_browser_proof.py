...
def simulate_browser_interaction():
    ...
    browser2 = create_browser_frame(
        "New Campaign - WorldArchitect.AI", f"{BASE_URL}/new", content2
    )
    print(browser2)
    ...
    browser4 = create_browser_frame(
        "Loading... - WorldArchitect.AI", BASE_URL, content4
    )
    print(browser4)
    ...
    browser5 = create_browser_frame(
        "Dragon's Revenge - WorldArchitect.AI", f"{BASE_URL}/game", content5
    )
    print(browser5)
    ...
    try:
        ...
        print(
            f"✅ GET /static/style.css - {response.status_code} ({len(response.content)} bytes)"
        )

        response = requests.get(f"{BASE_URL}/static/app.js")
        print(
            f"✅ GET /static/app.js - {response.status_code} ({len(response.content)} bytes)"
        )
