...
def run_real_browser_test():
    ...
    if create_resp.status_code == 201:
        ...
        if normal_resp.status_code == 200:
            log(
                f"Normal interaction works: {len(normal_resp.json().get('story', ''))} chars",
                "SUCCESS",
            )
            ...
            if combat_resp.status_code == 500:
                ...
                if "AttributeError" in str(
                    error
                ) and "'list' object has no attribute 'items'" in str(error):
                ...
                log(
                    f"Combat story: {len(combat_resp.json().get('story', ''))} chars",
                    "SUCCESS",
                )
            else:
