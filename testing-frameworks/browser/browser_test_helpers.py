...
class BrowserTestHelper:
    ...
    def navigate_with_test_auth(
        self, path: str = "/", test_user_id: str = "test-user-123"
    ):
        """
    ...
    def create_test_campaign(
        self, campaign_title: str = "Browser Test Campaign", debug_mode: bool = True
    ):
        """
        ...
        try:
            ...
            try:
                ...
                self.page.wait_for_selector(
                    "#campaign-title", state="visible", timeout=5000
                )
                ...
                if debug_mode:
                    self.debug_element_state(
                        "#campaign-title", "Campaign Title After Wait"
                    )
                ...
                if debug_mode:
                    self.debug_element_state(
                        "#campaign-title", "Campaign Title After Force Visible"
                    )
                ...
                if debug_mode:
                    self.debug_element_state(
                        "#campaign-title", "Campaign Title After Scroll"
                    )
            ...
            try:
                ...
                self.page.evaluate(
                    f"document.getElementById('campaign-title').value = '{campaign_title}'"
                )
                    ...
                    try:
                        ...
                        if launch_button.count() > 0:
                            ...
                            try:
                                ...
                                print(
                                    f"⚠️ Click failed, using JavaScript for: {selector}"
                                )
                                # Use JavaScript to click the button
                                self.page.evaluate(
                                    f"document.querySelector('{selector.replace(':', '\\:')}').click()"
                                )
            ...
            try:
                ...
                print(
                    f"✅ Test campaign '{campaign_title}' created and loaded successfully"
                )
                return True
        ...
        try:
            ...
            try:
                ...
                if bbox:
                    ...
                    print(
                        "   📐 Bounding Box: None (element not in viewport or hidden)"
                    )
            except:
                print("   📐 Bounding Box: Error getting bounds")

            # Check for overlapping elements
            try:
                overlapping = self.page.evaluate(
                    """
                    (selector) => {
                        const el = document.querySelector(selector);
                        if (!el) return null;
                ...
                print("   🎯 Element Coverage: Error checking overlaps")
    ...
    def wait_for_ai_response(self, timeout: int = 30000):
        ...
        try:
            ...
            self.page.wait_for_selector(
                "#loading-spinner", state="hidden", timeout=timeout
            )
