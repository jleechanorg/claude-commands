...
class PerformanceTest(BrowserTestBase):
    ...
    def run_test(self, page: Page) -> bool:
        ...
        try:
            ...
            if page.is_visible("text=New Campaign"):
                ...
                if page.is_visible("#wizard-campaign-title"):
                    ...
                    page.fill(
                        ...
                        "A long epic adventure for performance testing.",
                    )
            ...
            chat_area = page.query_selector("#chat-messages") or page.query_selector(
                ".chat-messages"
            )
            if chat_area:
                print("   📜 Testing scroll performance...")
                ...
                # Scroll to top
                page.evaluate(
                    "document.querySelector('#chat-messages, .chat-messages').scrollTop = 0"
                )
                page.wait_for_timeout(500)

                # Scroll to bottom quickly
                page.evaluate(
                    "document.querySelector('#chat-messages, .chat-messages').scrollTop = document.querySelector('#chat-messages, .chat-messages').scrollHeight"
                )
                page.wait_for_timeout(500)
            ...
            if memory_info:
                print(
                    f"   💾 Memory usage: {memory_info['used']}MB / {memory_info['total']}MB (limit: {memory_info['limit']}MB)"
                )
