...
def test_api_schema_compliance():
    ...
    resp = requests.post(
        f"{BASE_URL}/api/campaigns", headers=TEST_HEADERS, json=campaign_data
    )
    ...
    for field, description in schema_fields.items():
        if field in response_data:
            ...
            if (
                field == "dice_rolls"
                and not isinstance(response_data[field], list)
                or field == "entities_mentioned"
                and not isinstance(response_data[field], list)
            ):
                compliance_issues.append(
                    f"{field} should be a list, got {type(response_data[field])}"
                )
            elif field in ["state_updates", "debug_info"] and not isinstance(
                response_data[field], dict
            ):
                compliance_issues.append(
                    f"{field} should be a dict, got {type(response_data[field])}"
                )
        else:
