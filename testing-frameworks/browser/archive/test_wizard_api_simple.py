...
from test_ui_util import (
    enable_console_logging,
    fill_wizard_step1,
    navigate_to_page,
    navigate_wizard_to_step,
    run_ui_test,
)
    ...
    def capture_request(request):
        if "api/campaigns" in request.url and request.method == "POST":
            try:
                ...
                api_calls.append({"url": request.url, "data": data})
                print("\n✅ CAPTURED API CALL!")
    ...
    if api_calls:
        for call in api_calls:
            ...
            if "character" in data and "setting" in data:
                ...
                if (
                    data["character"] == "Astarion who ascended"
                    and data["setting"] == "Baldur's Gate 3 World"
                ):
                    ...
                    print(
                        f"   Expected character: 'Astarion who ascended', got: '{data['character']}'"
                    )
                    print(
                        f"   Expected setting: 'Baldur's Gate 3 World', got: '{data['setting']}'"
                    )
            else:
