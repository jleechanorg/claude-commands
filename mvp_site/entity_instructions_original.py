...
class EntityInstructionGenerator:
    ...
    def _create_entity_instruction(self, entity: str, player_references: List[str],
        ...
        if "cassian" in entity_lower and any("scared" in ref.lower() or "helpless" in ref.lower() for ref in player_references):
            specific_instruction += " This is an emotional moment requiring Cassian's response to the player's vulnerability."
    ...
    def _is_player_character(self, entity: str) -> bool:
        ...
        pc_indicators = ['sariel']  # Add more PC names as needed
        return any(indicator in entity.lower() for indicator in pc_indicators)
    ...
    def _is_location_owner(self, entity: str, location: Optional[str]) -> bool:
        ...
        location_mappings = {
            'valerius': ['valerius', 'study'],
            'cressida': ['cressida', 'chamber'],
            'kantos': ['kantos', 'magister', 'archive'],
        }
        
        for owner, location_keywords in location_mappings.items():
            if owner in entity_lower:
    ...
    def create_cassian_specific_instruction(self, player_input: str) -> str:
        """Create specific instruction for handling Cassian references"""
        if "cassian" not in player_input.lower():
            return ""
        
        emotional_keywords = ['scared', 'helpless', 'forgiveness', 'sorry']
        is_emotional = any(keyword in player_input.lower() for keyword in emotional_keywords)
        
        if is_emotional:
            return (
                "CRITICAL: The player is making an emotional appeal to Cassian. "
                "Cassian MUST appear and respond appropriately to this vulnerable moment. "
                "Do not let Cassian disappear or ignore this direct emotional reference."
            )
        else:
            return (
                "IMPORTANT: Cassian has been directly mentioned and must be present "
    ...
    def create_location_specific_instructions(self, location: str, expected_entities: List[str]) -> str:
        ...
        location_instructions = {
            'throne room': "Court setting requires appropriate nobles, guards, or advisors to be present for authenticity.",
            'valerius\'s study': "Valerius should be present in his own study unless explicitly away. Include scholarly atmosphere.",
            'lady cressida\'s chambers': "Lady Cressida should be present in her private chambers. Include personal, intimate setting details.",
            'great archives': "Scholarly NPCs like Magister Kantos should be present among the books and knowledge.",
            'chamber of whispers': "Mystical or scholarly figures should be present in this specialized location."
        }
        
        location_lower = location.lower()
        for loc_key, instruction in location_instructions.items():
