...
async function handleInteractionParallel(userInput, mode, currentCampaignId) {
    try {
        ...
        const response = await fetch(`/api/campaigns/${currentCampaignId}/interaction`, {
            method: 'POST',
            headers: {
        ...
        if (data.enhancement_needed && data.missing_entities && data.missing_entities.length > 0) {
            // Start background enhancement
            console.log(`Starting background enhancement for ${data.missing_entities.length} missing entities`);
 ...
 */
async function enhanceStoryInBackground(originalResponse, missingEntities, sequenceId, campaignId) {
    ...
    try {
        ...
        const response = await fetch(`/api/campaigns/${campaignId}/enhance-entities`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
function replaceStoryEntry(sequenceId, enhancedNarrative) {
    const storyEntry = document.querySelector(`[data-sequence-id="${sequenceId}"]`);
    if (!storyEntry) {
    ...
    const narrativeElement = storyEntry.querySelector('.narrative-content') || storyEntry;
...
function showEnhancementIndicator(sequenceId) {
    ...
    indicator.innerHTML = '<span class="spinner-border spinner-border-sm"></span> Enhancing story...';
