...
const appendToStory = (
  ...
  if (actor === 'gemini') {
    ...
    // actor is 'user'
    label =
      mode === 'character' ? 'Main Character' : mode === 'god' ? 'God' : 'You';
  }
...
const generateStructuredFieldsHTML = (fullData, debugMode) => {
  ...
  if (
    debugMode &&
    fullData.debug_info &&
    fullData.debug_info.dice_rolls &&
    fullData.debug_info.dice_rolls.length > 0
  ) {
    html +=
      '<div class="dice-rolls" style="background-color: #e8f4e8; padding: 8px; margin: 10px 0; border-radius: 5px;">';
    html +=
      '<strong>🎲 Dice Rolls:</strong><ul style="margin: 5px 0; padding-left: 20px;">';
  ...
  if (
    ...
    html +=
      '<div class="state-updates" style="background-color: #f5f5f5; padding: 10px; margin-top: 10px; border-radius: 5px; font-size: 0.9em;">';
    html += '<strong>🔧 State Updates:</strong><br>';
    html +=
      '<pre style="margin: 5px 0; font-size: 0.85em;">' +
      JSON.stringify(fullData.state_updates, null, 2) +
      '</pre>';
    html += '</div>';
  }
  ...
  if (debugMode && fullData.debug_info) {
    ...
    if (
      ...
      html +=
        '<div class="dm-notes" style="background-color: #f8f4ff; padding: 10px; margin-top: 10px; border-radius: 5px; font-style: italic;">';
      html +=
        '<strong>📝 DM Notes:</strong><ul style="margin: 5px 0; padding-left: 20px;">';
