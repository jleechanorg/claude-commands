...
function parsePlanningBlocksJson(planningBlock) {
    ...
    for (const [key, choice] of Object.entries(planningBlock.choices)) {
        if (!choice || typeof choice !== 'object') {
            console.warn(`parsePlanningBlocks: Invalid choice structure for key: ${key}`);
            continue;
        }
...
suite.test('Should parse deep think mode with STRING format pros/cons', function() {
    const planningBlock = {
        "thinking": "This is a critical moment. I need to weigh my options carefully.",
        "choices": {
            "attack_head_on": {
                "text": "Attack Head-On",
    ...
    this.assertContains(html, "• Quick resolution", "Should contain bullet point pros");
    this.assertContains(html, "• High risk of injury", "Should contain bullet point cons");
    this.assertContains(html, "Low - this seems reckless", "Should contain confidence assessment");
    
    // Should contain single blue analysis section
    this.assertContains(html, 'analysis-content', "Should contain analysis-content class");
    this.assertContains(html, '✅ Pros:', "Should contain pros label");
    this.assertContains(html, '❌ Cons:', "Should contain cons label");
    this.assertContains(html, '🎯 Assessment:', "Should contain assessment label");
});
...
suite.test('Should escape HTML in STRING format for safety', function() {
    const planningBlock = {
        ...
        "choices": {
            "test_choice": {
                ...
                "risk_level": "low",
                "analysis": {
                    "pros": ["<script>alert('xss')</script>Safe option", "No danger<img src=x onerror=alert('xss')>"],
    ...
    this.assertContains(html, '&lt;/script&gt;', "Should escape closing script tags");
    this.assertContains(html, '&lt;img', "Should escape img tags");
    this.assertContains(html, '&lt;b&gt;', "Should escape bold tags");
    
    // Should NOT contain unescaped HTML
    this.assertNotContains(html, '<script>', "Should not contain unescaped script tags");
    ...
    this.assertContains(html, "alert(&#x27;xss&#x27;)", "Should escape quotes in JavaScript");
});
...
suite.test('Should handle empty analysis fields gracefully', function() {
    ...
    this.assertNotContains(html, 'pros-section', "Should not render empty pros section");
    this.assertNotContains(html, 'cons-section', "Should not render empty cons section");
    this.assertNotContains(html, 'confidence-section', "Should not render empty confidence section");
});
...
suite.test('Should handle mixed choice types correctly', function() {
    ...
    this.assertContains(html, "Standard Choice", "Should contain standard choice");
    
    // Deep think choice SHOULD have analysis sections
    this.assertContains(html, "Thoughtful approach, well-considered", "Should contain deep think pros");
    this.assertContains(html, "Takes more time, requires focus", "Should contain deep think cons");
    this.assertContains(html, "High confidence in this approach", "Should contain confidence");
    
    // Should have both choice types
    this.assertContains(html, 'choice-button-expanded', "Should have expanded choice");
    this.assertContains(html, 'deep-think-choice', "Should have deep think class");
});
...
suite.test('Should handle malformed planning blocks gracefully', function() {
    ...
    this.assertNotContains(html, 'choice-button-expanded', "Should not render as expanded choice");
});
