...
class RobustJSONParser:
    ...
    def parse(text: str) -> tuple[dict[str, Any] | None, bool]:
        ...
        # Strategy 1.5: Code execution artifact removal
        # If text is very short or starts with special chars, might be code execution output
        if len(text) < 10 or text[0] in ['\n', ' ', '\t', '\r']:
            logging_util.info("Detected potential code execution artifacts (short/whitespace response)")
            # Try to find JSON in the text by looking for { or [
            json_start = -1
            ...
            if json_start >= 0:
                cleaned_text = text[json_start:].strip()
                result, success = try_parse_json(cleaned_text)
                if success:
                    logging_util.info("Successfully parsed JSON after removing code execution prefix")
                    return result, True
        ...
        # Strategy 5: Code execution markdown block extraction
        ...
        try:
            ...
            if match:
                ...
                result, success = try_parse_json(extracted_json)
                if success:
                    logging_util.info("Successfully extracted JSON from markdown code block")
                    return result, True
        except Exception as e:
            logging_util.debug(f"Markdown code block extraction failed: {e}")

        # Strategy 6: Last resort - try to fix and parse again
        try:
        ...
        log_full_errors = os.environ.get("LOG_JSON_ERRORS_FULL", "false").lower() == "true"

        if log_full_errors:
            # Log first 2000 characters of actual malformed JSON
            logging_util.error(
                f"All JSON parsing strategies failed. Content preview (first 2000 chars):\n"
                f"{text[:2000]}"
            )
        else:
