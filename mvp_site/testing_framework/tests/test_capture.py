...
class TestCaptureManager(unittest.TestCase):
    ...
    def test_capture_interaction_success(self):
        ...
        with self.capture_manager.capture_interaction(
            ...
            self.capture_manager.record_response(interaction["id"], {"result": "success"})
    ...
    def test_save_captures(self):
        ...
        with self.capture_manager.capture_interaction("service1", "op1", {"data": "test1"}) as interaction:
            self.capture_manager.record_response(interaction["id"], {"result": "ok"})
        
        with self.capture_manager.capture_interaction("service2", "op2", {"data": "test2"}) as interaction:
            self.capture_manager.record_response(interaction["id"], {"result": "ok"})
    ...
    def test_get_summary(self):
        ...
        with self.capture_manager.capture_interaction("firestore", "get", {}) as interaction:
            self.capture_manager.record_response(interaction["id"], {"docs": []})
        
        with self.capture_manager.capture_interaction("firestore", "set", {}) as interaction:
            self.capture_manager.record_response(interaction["id"], {"ok": True})
        
        with self.capture_manager.capture_interaction("gemini", "generate", {}) as interaction:
            self.capture_manager.record_response(interaction["id"], {"text": "response"})
...
class TestCaptureFirestoreClient(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.capture_client = CaptureFirestoreClient(self.mock_client, self.capture_manager)
...
class TestCaptureGeminiClient(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.capture_manager = CaptureManager(self.temp_dir)
        self.mock_client = MagicMock()
        self.capture_client = CaptureGeminiClient(self.mock_client, self.capture_manager)
...
class TestCaptureAnalyzer(unittest.TestCase):
    ...
    def test_compare_with_mock(self):
        ...
        self.assertEqual(comparison["total_comparisons"], 2)  # Only success interactions
        ...
        self.assertEqual(len(comparison["missing_mocks"]), 0)  # firestore.set was error, so not compared
...
class TestUtilityFunctions(unittest.TestCase):
    ...
    def test_cleanup_old_captures(self):
        ...
        self.assertTrue(os.path.exists(non_capture_file))  # Should remain (not a capture file)
