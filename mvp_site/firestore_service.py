...
@log_exceptions
def track_push_compliance(session_id, push_command, push_attempted, push_verified, verification_command, verification_output):
    """Track push compliance for a git push operation."""
    if not session_id:
        raise ValueError("Session ID is required for push compliance tracking.")
    
    db = get_db()
    push_compliance_ref = db.collection(constants.PUSH_COMPLIANCE_COLLECTION)
    ...
    compliance_data = {
        ...
        'verification_output': verification_output[:500] if verification_output else "",  # Limit output size
        'created_at': firestore.SERVER_TIMESTAMP
    }
    
    try:
        push_compliance_ref.add(compliance_data)
        logging_util.info(f"Tracked push compliance for session {session_id}: attempted={push_attempted}, verified={push_verified}")
    except Exception as e:
        logging_util.error(f"Failed to track push compliance for session {session_id}: {e}", exc_info=True)
        raise


@log_exceptions
def get_session_push_compliance_rate(session_id):
    """Calculate push compliance rate for a session."""
    if not session_id:
        return 0.0
    ...
    try:
        # Get all push compliance records for this session
        docs = push_compliance_ref.where('session_id', '==', session_id).stream()
        ...
        for doc in docs:
            data = doc.to_dict()
            if data.get('push_attempted', False):  # Only count actual push attempts
                total_pushes += 1
                if data.get('push_verified', False):
                    verified_pushes += 1
        
        if total_pushes == 0:
            return 1.0  # No pushes yet, assume compliant
        
        compliance_rate = verified_pushes / total_pushes
        logging_util.info(f"Session {session_id} push compliance rate: {compliance_rate:.2f} ({verified_pushes}/{total_pushes})")
        
        return compliance_rate
        
    except Exception as e:
        logging_util.error(f"Failed to calculate push compliance rate for session {session_id}: {e}", exc_info=True)
        return 0.0


@log_exceptions
def get_session_push_compliance_stats(session_id):
    """Get detailed push compliance statistics for a session."""
    if not session_id:
        return {
            'total_pushes': 0,
            'verified_pushes': 0,
            'compliance_rate': 0.0,
            'recent_unverified_pushes': []
        }
    ...
    try:
        # Get all push compliance records for this session, ordered by timestamp
        docs = push_compliance_ref.where('session_id', '==', session_id).order_by('timestamp', direction=firestore.Query.DESCENDING).stream()
        ...
        for doc in docs:
            ...
            if data.get('push_attempted', False):  # Only count actual push attempts
                ...
                if data.get('push_verified', False):
                    ...
                    # Track recent unverified pushes (last 5)
                    if len(recent_unverified_pushes) < 5:
                        recent_unverified_pushes.append({
                            'timestamp': data.get('timestamp'),
        ...
        compliance_rate = verified_pushes / total_pushes if total_pushes > 0 else 1.0
        
        return {
            'total_pushes': total_pushes,
            'verified_pushes': verified_pushes,
            'compliance_rate': compliance_rate,
            'recent_unverified_pushes': recent_unverified_pushes
        }
        
    except Exception as e:
        logging_util.error(f"Failed to get push compliance stats for session {session_id}: {e}", exc_info=True)
        return {
            'total_pushes': 0,
            'verified_pushes': 0,
            'compliance_rate': 0.0,
...
@log_exceptions
def get_global_push_compliance_stats():
    """Get global push compliance statistics across all sessions."""
    db = get_db()
    push_compliance_ref = db.collection(constants.PUSH_COMPLIANCE_COLLECTION)
    
    try:
        # Get all push compliance records
        docs = push_compliance_ref.stream()
        ...
        logging_util.error(f"Failed to get global push compliance stats: {e}", exc_info=True)
        return {
            'total_pushes': 0,
            'verified_pushes': 0,
            'compliance_rate': 0.0,
