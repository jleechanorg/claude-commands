...
def test_choice_id_mobile_responsive():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            desktop_font_size = choice_id_element.evaluate("el => getComputedStyle(el).fontSize")
            ...
            tablet_font_size = choice_id_element.evaluate("el => getComputedStyle(el).fontSize")
            ...
            mobile_font_size = choice_id_element.evaluate("el => getComputedStyle(el).fontSize")
            ...
            print(f"Font size progression: Desktop {desktop_px}px → Tablet {tablet_px}px → Mobile {mobile_px}px")
            
            # Assertions for responsive behavior
            assert mobile_px < desktop_px, f"Mobile font-size ({mobile_px}px) should be smaller than desktop ({desktop_px}px)"
            assert mobile_px <= 12, f"Mobile font-size ({mobile_px}px) should be small (≤12px for readability)"
            assert mobile_px >= 8, f"Mobile font-size ({mobile_px}px) should be readable (≥8px)"
            
            # Tablet should be between mobile and desktop, or same as mobile
            assert tablet_px <= desktop_px, f"Tablet font-size ({tablet_px}px) should not be larger than desktop ({desktop_px}px)"
...
def test_choice_button_mobile_layout():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            mobile_padding = choice_button.evaluate("el => getComputedStyle(el).padding")
            mobile_font_size = choice_button.evaluate("el => getComputedStyle(el).fontSize")
            mobile_margin_bottom = choice_button.evaluate("el => getComputedStyle(el).marginBottom")
            ...
            assert "0.08rem" in mobile_padding or "1.28px" in mobile_padding, "Mobile padding should be reduced"
            assert "0.7rem" in mobile_font_size or "11.2px" in mobile_font_size, "Mobile font should be 0.7rem"
