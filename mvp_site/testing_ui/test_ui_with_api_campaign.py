...
def create_campaign_via_api():
    ...
    if response.status_code == 201:
        ...
        raise Exception(f"Failed to create campaign: {response.status_code} - {response.text}")
    ...
    with sync_playwright() as p:
                ...
                try:
                    ...
                    if "session_header" in data or "planning_block" in data:
                        ...
                        if "planning_block" in data:
                            print(f"   planning_block: {data['planning_block'][:100]}...")
                except:
                    pass
        ...
        try:
            ...
            if len(existing_entries) > 0:
                print("✓ Found existing story entries, but testing interaction to verify dice rolls")
                # Send combat action
                user_input = page.locator("#user-input")
                if user_input.count() > 0:
                    # Type combat action
                    user_input.fill("I attack the goblin with my sword! Roll for attack and damage.")
            ...
            if len(story_entries) >= 1:  # At least initial story
                    ...
                    if session_header.count() > 0 and session_header.is_visible():
                        ...
                        print(f"  Style: Gray background, monospace font")
                    ...
                    if dice_rolls.count() > 0 and dice_rolls.is_visible():
                        ...
                        print(f"\n✓ DICE ROLLS found:")
                        print(f"  Style: Green background")
                    ...
                    if resources.count() > 0 and resources.is_visible():
                        ...
                        print(f"\n✓ RESOURCES found:")
                        print(f"  Style: Yellow background")
                        print(f"  Content: {content}")
                    else:
                        print("\n✗ Resources NOT FOUND")
                    ...
                    page.screenshot(path=f"{screenshots_dir}/09_full_page_final.png", full_page=True)
