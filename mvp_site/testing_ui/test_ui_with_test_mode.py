...
def test_mobile_responsive_choice_id_sizing():
    ...
    with sync_playwright() as p:
        ...
        try:
            # Navigate with test mode enabled
            test_url = "http://localhost:8082?test_mode=true&test_user_id=mobile-test-user"
            page.goto(test_url)
            print(f"✓ Navigated to app with test mode: {test_url}")
                ...
                if choice_ids.count() > 0:
                    ...
                    if viewport['width'] <= 768:
                        ...
                        if actual_size == expected_mobile_size:
                            ...
                            print(f"⚠️ Mobile font-size issue: expected {expected_mobile_size}, got {actual_size}")
                    ...
                    if choice_buttons.count() > 0:
                        ...
                        if viewport['width'] <= 768:
                            if choice_id_style['fontSize'] == button_style:
                                print("✓ Choice ID and button font-sizes match on mobile")
                            else:
                                print(f"⚠️ Font-size mismatch - ID: {choice_id_style['fontSize']}, Button: {button_style}")
                else:
...
def test_campaign_wizard_functionality():
    ...
    with sync_playwright() as p:
        ...
        try:
            # Navigate with test mode enabled
            test_url = "http://localhost:8082?test_mode=true&test_user_id=wizard-test-user"
            page.goto(test_url)
            print(f"✓ Navigated to app with test mode: {test_url}")
            ...
            wizard_present = page.locator("#campaign-wizard").is_visible() if page.locator("#campaign-wizard").count() > 0 else False
            ...
            if wizard_present:
                ...
                assert title_input.is_visible(), "Campaign title input should be visible"
                ...
                assert dragon_knight_radio.is_checked(), "Dragon Knight should be selected by default"
                ...
                assert setting_input.is_visible(), "Setting input (wizard-setting-input) should be visible"
                ...
                current_step = page.locator(".step-indicator.active").get_attribute("data-step")
                ...
                if current_step == "3":
                    ...
                    default_world_checked = page.locator("#wizard-default-world").is_checked()
                    print(f"Default world enabled: {'✓' if default_world_checked else '✗'}")
