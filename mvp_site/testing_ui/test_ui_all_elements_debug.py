    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if create_btn.count() > 0:
                ...
                page.screenshot(path=f"{screenshots_dir}/01_initial_story.png", full_page=True)
                ...
                if user_input.count() > 0:
                    ...
                    if last_entry.count() > 0:
                        last_entry.screenshot(path=f"{screenshots_dir}/03_full_response.png")
                        print("✓ Screenshot: Full response")
                        
                        # Session Header
                        session_header = last_entry.locator(".session-header")
                        if session_header.count() > 0:
                            session_header.screenshot(path=f"{screenshots_dir}/04_session_header.png")
                            print("✓ Screenshot: Session header")
                            print(f"  Content preview: {session_header.inner_text()[:100]}...")
                        else:
                        ...
                        if narrative.count() > 0:
                            narrative.screenshot(path=f"{screenshots_dir}/05_narrative.png")
                            print("✓ Screenshot: Narrative")
                            print(f"  Content preview: {narrative.inner_text()[:100]}...")
                        else:
                        ...
                        if dice_rolls.count() > 0:
                            dice_rolls.screenshot(path=f"{screenshots_dir}/06_dice_rolls.png")
                        ...
                        if resources.count() > 0:
                            resources.screenshot(path=f"{screenshots_dir}/07_resources.png")
                        ...
                        if planning_block.count() > 0:
                            planning_block.screenshot(path=f"{screenshots_dir}/08_planning_block.png")
                            print("✓ Screenshot: Planning block")
                            print(f"  Content preview: {planning_block.inner_text()[:150]}...")
                        else:
                        ...
                        if debug_info.count() > 0:
                            debug_info.screenshot(path=f"{screenshots_dir}/09_debug_info.png")
                            print("✓ Screenshot: Debug info (DEBUG MODE CONFIRMED)")
                            print(f"  Content preview: {debug_info.inner_text()[:100]}...")
                        else:
                            print("⚠️  Debug info not found - debug mode may not be enabled")
                        ...
                        page.screenshot(path=f"{screenshots_dir}/10_full_page_final.png", full_page=True)
            ...
            print("✓ Test complete. Browser will stay open for 15 seconds for inspection...")
            time.sleep(15)
            browser.close()
