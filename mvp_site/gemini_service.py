...
logging_util.basicConfig(level=logging_util.logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
def parse_llm_response_for_state_changes(llm_text_response: str) -> dict:
    ...
    if not matches:
        logging_util.warning("⚠️ ALERT: No STATE_UPDATES_PROPOSED block found in LLM response! This means game state will NOT be updated. The AI should always generate STATE_UPDATES_PROPOSED blocks to track entities and game state.")
        return {}
    ...
    for json_string in reversed(matches):
        ...
        try:
            ...
            if isinstance(proposed_changes, dict):
                logging_util.info("Successfully parsed a valid state update block.")
                return proposed_changes
            else:
                logging_util.warning(f"Found a state update block, but parsed JSON was not a dictionary. Type: {type(proposed_changes)}. Content: {proposed_changes}")
        except json.JSONDecodeError:
            logging_util.warning(f"Found a state update block, but it contained invalid JSON. Content: `{json_string}`")
            continue # Try the next block

    logging_util.warning("Found state update block(s), but none contained valid JSON.")
    return {}
