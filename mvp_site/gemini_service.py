...
def _get_text_from_response(response: Any) -> str:
    ...
    Handles both standard responses and code execution responses where
    response.text might be empty or contain mixed content.
    """
    # Detect code execution usage first
    code_execution_used = False
    code_execution_parts_count = 0
            ...
            if hasattr(candidate, 'content') and hasattr(candidate.content, 'parts'):
                for part in candidate.content.parts:
                    ...
                    if hasattr(part, 'executable_code') or hasattr(part, 'code_execution_result'):
                        ...
                        code_execution_parts_count += 1
    except Exception as e:
        logging_util.debug(f"Error checking for code execution: {e}")

    # Log if code execution was detected
    if code_execution_used:
        logging_util.info(
            f"Code execution detected in response ({code_execution_parts_count} parts). "
    ...
    try:
        if response.text:
            ...
            # If code execution was used and text is suspiciously short, investigate further
            if code_execution_used and len(text) < 50:
                logging_util.warning(
                    f"Code execution used but response.text is very short ({len(text)} chars). "
                    "May contain code execution artifacts instead of JSON."
                )
        ...
        logging_util.error(f"Unexpected error accessing response.text: {e}")

    # Fallback: Extract text from parts manually (for code execution responses)
    if code_execution_used:
        logging_util.info("Attempting to extract text from response parts (code execution fallback)")
        try:
            if hasattr(response, 'candidates') and response.candidates:
                candidate = response.candidates[0]
                if hasattr(candidate, 'content') and hasattr(candidate.content, 'parts'):
                    # Collect all text parts, skipping code execution artifacts
                    text_parts = []
                    for part in candidate.content.parts:
                        # Skip executable_code and code_execution_result parts
                        if hasattr(part, 'executable_code') or hasattr(part, 'code_execution_result'):
                            continue
                    ...
                    if text_parts:
                        ...
                        logging_util.info(
                            f"Successfully extracted text from parts ({len(combined_text)} chars)"
                        )
                        return combined_text
                    else:
                        logging_util.error(
                            "No text parts found after filtering code execution artifacts"
                        )
        except Exception as e:
            logging_util.error(f"Error extracting text from parts: {e}")

    logging_util.warning(
        f"Response did not contain valid text. Response object type: {type(response).__name__}"
    )
