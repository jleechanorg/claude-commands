...
class TestBannedNamesLoading(unittest.TestCase):
    ...
    def test_banned_names_file_exists(self):
        ...
        self.assertTrue(os.path.exists(world_loader.BANNED_NAMES_PATH),
    ...
    def test_banned_names_contains_master_directive(self):
        ...
        content = world_loader.load_banned_names()
        self.assertIn("MASTER DIRECTIVE", content, 
                     "Banned names content should contain MASTER DIRECTIVE")
        self.assertIn("ABSOLUTELY FORBIDDEN", content,
    ...
    def test_banned_names_contains_all_primary_names(self):
        ...
        content = world_loader.load_banned_names()
        primary_names = [
            "Alaric", "Blackwood", "Corvus", "Elara", "Kaelen",
            "Lyra", "Seraphina", "Thorne", "Valerius", "Isolde"
        ]
        for name in primary_names:
            self.assertIn(name, content, f"Primary banned name '{name}' not found in content")
    ...
    def test_world_content_includes_banned_names(self):
        ...
        try:
            ...
            self.assertIn("BANNED", full_content.upper(),
                         "World content should include banned names section")
            self.assertIn("MASTER DIRECTIVE", full_content,
                         "World content should include the master directive")
            # Check that at least some banned names are present
            self.assertIn("Lyra", full_content, "Banned name 'Lyra' should be in world content")
            self.assertIn("Kaelen", full_content, "Banned name 'Kaelen' should be in world content")
        except FileNotFoundError as e:
            self.skipTest(f"Skipping world content test - required files not found: {e}")
