    ...
    def test_story_context_reloaded_for_each_command(
        ...
        def mock_gemini_side_effect(*args, **kwargs):
            ...
            if call_count["count"] == 1:
                ...
                # Verify that the first command's text is in the story_history
                if "GOD MODE: Set HP" in call_args_str or "Set HP" in call_args_str:
                    ...
                    self.fail(
                        ...
                        f"Call args (first 1000 chars): {call_args_str[:1000]}"
                    )
