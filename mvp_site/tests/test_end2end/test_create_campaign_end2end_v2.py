...
class FakeFirestoreDocument:
    ...
    def collection(self, name):
        """Get a subcollection."""
        path = f"{self._parent_path}/{self.id}/{name}" if self._parent_path else f"{self.id}/{name}"
...
class FakeFirestoreCollection:
    ...
    def document(self, doc_id=None):
        ...
        if doc_id not in self._docs:
            path = f"{self._parent_path}/{self.name}" if self._parent_path else self.name
...
class TestCreateCampaignEnd2EndV2(unittest.TestCase):
    ...
    def test_create_campaign_success(self, mock_genai_client_class, mock_firestore_client):
        ...
        # Mock token counting
        fake_genai_client.models.count_tokens.return_value = MagicMock(total_tokens=1000)
