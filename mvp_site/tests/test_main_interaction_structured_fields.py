...
class TestMainInteractionStructuredFields(unittest.TestCase):
    ...
    def create_mock_gemini_response(self, with_structured_fields=True):
        ...
        mock_response.debug_tags_present = {
            "dm_notes": True,
            "dice_rolls": True,
            "state_changes": False,
        }
        ...
        if with_structured_fields:
            ...
            mock_structured_response.session_header = self.structured_data[
                "session_header"
            ]
            mock_structured_response.planning_block = self.structured_data[
                "planning_block"
            ]
    ...
    def setup_common_mocks(
        self, mock_firestore_service, mock_gemini_service, mock_gemini_response
    ):
    ...
    def test_interaction_endpoint_with_structured_fields(
        ...
        mock_gemini_response = self.create_mock_gemini_response(
            with_structured_fields=True
        )

        # Setup all common mocks
        self.setup_common_mocks(
            mock_firestore_service, mock_gemini_service, mock_gemini_response
        )
        ...
        self.assertEqual(
            response_data[constants.FIELD_RESOURCES], self.structured_data["resources"]
        )
        self.assertEqual(
        ...
        self.assertEqual(
            firestore_structured_fields[constants.FIELD_SESSION_HEADER],
            self.structured_data["session_header"],
        )

    @patch("main.firestore_service")
    @patch("main.gemini_service")
    def test_interaction_endpoint_without_structured_fields(
        self, mock_gemini_service, mock_firestore_service
    ):
    ...
    @patch("main.firestore_service")
    @patch("main.gemini_service")
    def test_interaction_endpoint_with_empty_structured_fields(
        self, mock_gemini_service, mock_firestore_service
    ):
        ...
        mock_response.state_updates = {}
        mock_response.get_narrative_text = Mock(
            return_value="You explore the empty room."
        )
    ...
    @patch("main.firestore_service")
    @patch("main.gemini_service")
    def test_interaction_endpoint_preserves_structured_field_types(
        self, mock_gemini_service, mock_firestore_service
    ):
        ...
        mock_response.state_updates = {}
        mock_response.get_narrative_text = Mock(
            return_value="Complex response with various data types."
        )
