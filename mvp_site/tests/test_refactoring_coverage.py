...
class TestHelperFunctions(unittest.TestCase):
    ...
    def test_prepare_entity_tracking(self, mock_logging, mock_create_from_game_state, mock_prompt_injection):
        ...
        manifest, entities, instruction = _prepare_entity_tracking(
            game_state, [], 1
        )
    ...
    def test_build_continuation_prompt(self):
        ...
        result = _build_continuation_prompt(
            "checkpoint", "memories", "seq123",
            '{"state": "data"}', "entity_instruction",
            "timeline", "current"
        )
...
class TestMainHelpers(unittest.TestCase):
    ...
    def test_strip_other_debug_content(self):
        ...
        def strip_other_debug_content(text):
            ...
            processed_text = re.sub(r'\[DEBUG_RESOURCES_START\][\s\S]*?\[DEBUG_RESOURCES_END\]', '', processed_text)
            processed_text = re.sub(r'\[DEBUG_VALIDATION_START\][\s\S]*?\[DEBUG_VALIDATION_END\]', '', processed_text)
            processed_text = re.sub(r'\[DEBUG_ENTITY_START\][\s\S]*?\[DEBUG_ENTITY_END\]', '', processed_text)
    ...
    def test_strip_state_updates_only(self):
        ...
        def strip_state_updates_only(text):
            if not text:
                return text
            return re.sub(r'\[STATE_UPDATES_PROPOSED\][\s\S]*?\[END_STATE_UPDATES_PROPOSED\]', '', text)
