...
from main import KEY_SUCCESS, KEY_ERROR, KEY_CAMPAIGN_ID, KEY_CAMPAIGN, KEY_STORY, KEY_MESSAGE
import constants
...
class TestCampaignRoutes(unittest.TestCase):
    ...
    def test_get_campaigns_error(self, mock_firestore_service):
        ...
        mock_firestore_service.get_campaigns_for_user.side_effect = Exception("Database error")
    ...
    def test_create_campaign_success(self, mock_gemini_service, mock_firestore_service):
        ...
        response = self.client.post('/api/campaigns', headers=self.test_headers, json=campaign_data)
        ...
        self.assertEqual(call_args[1]['selected_prompts'], campaign_data['selected_prompts'])
        self.assertTrue(call_args[1]['generate_companions'])
        self.assertTrue(call_args[1]['use_default_world'])

    @patch('main.firestore_service')
    @patch('main.gemini_service')
    def test_create_campaign_minimal_data(self, mock_gemini_service, mock_firestore_service):
    ...
    def test_update_campaign_success(self, mock_firestore_service):
        ...
        response = self.client.patch('/api/campaigns/test-campaign', headers=self.test_headers, json=update_data)
    ...
    def test_update_campaign_service_error(self, mock_firestore_service):
        ...
        mock_firestore_service.update_campaign_title.side_effect = Exception("Update failed")
    ...
    def test_export_campaign_txt_success(self, mock_doc_gen, mock_firestore_service):
        ...
        with patch('os.path.exists', return_value=True), \
            ...
            response = self.client.get('/api/campaigns/test-campaign/export?format=txt', headers=self.test_headers)
    ...
    def test_export_campaign_not_found(self, mock_firestore_service):
        ...
        response = self.client.get('/api/campaigns/nonexistent/export', headers=self.test_headers)
    ...
    def test_export_campaign_unsupported_format(self, mock_firestore_service):
        ...
        response = self.client.get('/api/campaigns/test-campaign/export?format=invalid', headers=self.test_headers)
...
class TestFrontendRoutes(unittest.TestCase):
    ...
    def test_serve_root_path(self):
        ...
        with patch('main.send_from_directory') as mock_send:
            ...
            # Should serve index.html from static folder
    ...
    def test_serve_existing_static_file(self):
        ...
        with patch('os.path.exists', return_value=True), \
            ...
            # Should serve the actual file
            mock_send.assert_called_with(self.app.static_folder, 'styles.css')
