...
class TestServerInfrastructure(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.testserver_script = os.path.join(self.project_root, 'claude_command_scripts', 'commands', 'testserver.sh')
    ...
    def test_port_allocation_range(self):
        ...
        port_utils_script = os.path.join(self.project_root, 'claude_command_scripts', 'port-utils.sh')
        
        if os.path.exists(port_utils_script):
            try:
                # Source the script and test port allocation functions
                result = subprocess.run(
                    ['bash', '-c', f'source {port_utils_script} && echo "Port utils loaded"'],
                    capture_output=True,
                    text=True,
                    timeout=5,
                    cwd=self.project_root
    ...
    def test_branch_specific_logging(self):
        ...
        if not os.path.exists(logs_dir):
            try:
                ...
                self.skipTest(f"Cannot create logs directory {logs_dir} - permission denied")
        ...
        try:
            ...
            self.skipTest(f"Cannot write to log file {test_branch_log} - permission denied")
        except Exception as e:
    ...
    def test_integration_with_push_commands(self):
        ...
        push_script = os.path.join(self.project_root, 'claude_command_scripts', 'commands', 'push.sh')
        ...
        if os.path.exists(push_script):
            ...
            self.assertTrue(os.access(push_script, os.X_OK), "push.sh should be executable")
        
        if os.path.exists(integrate_script):
            # Test that integrate script exists and is executable
            self.assertTrue(os.access(integrate_script, os.X_OK), "integrate.sh should be executable")
        ...
        try:
            ...
            self.assertIn('Integration with /push and /integrate commands', result.stdout)
