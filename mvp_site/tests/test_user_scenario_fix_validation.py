...
class TestUserScenarioFixValidation(unittest.TestCase):
    ...
    "debug_info": {"dm_notes": ["Major plot advancement"]}'''  # Missing closing brace and comma
        ...
        # This should NOT return raw JSON - should extract the god mode response cleanly
        narrative_text, structured_response = parse_structured_response(malformed_god_mode_json)
        ...
        self.assertNotIn('{"', narrative_text,
                        "User should never see JSON structure")
        ...
        self.assertIn(expected_content, narrative_text,
                     "User should see the actual god mode response content")
        self.assertIn("Reality bends to your will", narrative_text,
    ...
    def test_various_malformation_scenarios(self):
        ...
        scenarios = [
            ...
            ('Invalid JSON', '''{
                ...
                "location_confirmed": "Battlefield"'''),  # Missing comma
            
            # Truncated response
            ('Truncated', '''{
                "narrative": "",
                "god_mode_response": "Ancient magic awakens and'''),  # Cut off mid-sentence
        ]
        
        for scenario_name, malformed_json in scenarios:
            with self.subTest(scenario=scenario_name):
                narrative_text, structured_response = parse_structured_response(malformed_json)
                ...
                self.assertTrue(len(narrative_text.strip()) > 0,
                              f"Scenario '{scenario_name}' returned empty result")
                
                # Content should be readable text, not JSON
                self.assertFalse(narrative_text.strip().startswith('{'),
