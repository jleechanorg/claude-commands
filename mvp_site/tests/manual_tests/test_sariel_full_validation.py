...
class TestSarielFullValidation(unittest.TestCase):
    ...
    def test_sariel_full_replays(self):
        ...
        for run in range(10):
            ...
            print(f"- Cassian problem: {'SOLVED ✓' if run_result['cassian_mentioned'] else 'FAILED ✗'}")
            print(f"- Game state valid: {'YES ✓' if run_result['game_state_valid'] else 'NO ✗'}")
            print(f"{'='*40}")
    ...
    def _run_single_campaign(self, run_number: int) -> Dict[str, Any]:
        ...
        for i, prompt_data in enumerate(self.sariel_data['prompts'][1:6]):
            ...
            print(f"Expected entities: {prompt_data['context'].get('expected_entities', [])}")
            ...
            if response.status_code != 200:
                ...
                result['validation_errors'].append(f"Interaction {i+1} failed: {response.status_code}")
                continue
            ...
            if state_response.status_code == 200:
                ...
                print(f"\nGAME STATE SUMMARY:")
                print(f"- Player character: {game_state_data.get('player_character_data', {})}")
                print(f"- NPCs: {list(game_state_data.get('npc_data', {}).keys())}")
                print(f"- Location: {game_state_data.get('world_data', {}).get('current_location_name', 'Unknown')}")
                print(f"- Combat: {game_state_data.get('combat_state', {}).get('in_combat', False)}")
                
                # Track entities
                expected_entities = prompt_data['context'].get('expected_entities', [])
                found_entities = self._check_entities_in_narrative(narrative, expected_entities)
                ...
                print(f"- Missing: {[e for e in expected_entities if e not in found_entities]}")
        ...
        if result['total_interactions'] > 0:
            result['entity_tracking_rate'] = result['entity_tracking_successes'] / result['total_interactions']
        else:
            result['entity_tracking_rate'] = 0
        
        return result
    
    def _validate_game_state(self, game_state: Dict[str, Any], interaction_num: int) -> Dict[str, Any]:
        ...
        for npc_id, npc_info in npc_data.items():
            if not isinstance(npc_info, dict):
                validation['valid'] = False
                validation['errors'].append(f"NPC {npc_id} data is not a dictionary: {type(npc_info)}")
    ...
    def _check_entities_in_narrative(self, narrative: str, expected_entities: List[str]) -> List[str]:
    ...
    def _print_final_summary(self, all_results: List[Dict[str, Any]]):
        ...
        print(f"- Cassian problem solved: {cassian_successes}/{total_runs} ({cassian_successes/total_runs*100:.0f}%)")
        print(f"- Valid game states: {valid_states}/{total_runs} ({valid_states/total_runs*100:.0f}%)")
        
        # Entity tracking stats
        total_entity_checks = sum(r['total_interactions'] for r in all_results)
        total_entity_successes = sum(r['entity_tracking_successes'] for r in all_results)
        
        print(f"\nENTITY TRACKING:")
        print(f"- Total entity checks: {total_entity_checks}")
        print(f"- Successful tracks: {total_entity_successes}")
        print(f"- Overall success rate: {total_entity_successes/total_entity_checks*100:.1f}%")
        ...
        for r in all_results:
            ...
            print(f"Run {r['run']}: Cassian {cassian}, State {state}, "
