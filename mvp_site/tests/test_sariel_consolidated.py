...
class TestSarielConsolidated(unittest.TestCase):
    ...
    def test_sariel_campaign(self):
        ...
        # Load test data
        test_data_path = os.path.join(os.path.dirname(__file__), 'data', 'sariel_campaign_prompts.json')
        ...
        for replay_num in range(self.num_replays):
            ...
            self.assertEqual(response.status_code, 201, f"Failed to create campaign: {response.get_data(as_text=True)}")
            ...
            for i in range(self.num_interactions):
                ...
                self.assertEqual(response.status_code, 200, f"Failed at interaction {interaction_num}")
                ...
                state_validation = self._validate_game_state(game_state_data, interaction_num)
                ...
                interaction_result = {
                    ...
                    'missing_entities': list(set(expected_entities) - set(found_entities)),
                ...
                print(f"Interaction {interaction_num}: {status} " +
        ...
        success_rate = (total_success / total_interactions) * 100 if total_interactions > 0 else 0
        entity_rate = (total_entities_found / total_entities_expected) * 100 if total_entities_expected > 0 else 0
        
        print(f"\n{'='*60}")
        print("SUMMARY STATISTICS")
        print(f"{'='*60}")
        print(f"Total interactions: {total_interactions}")
        print(f"Successful interactions: {total_success}/{total_interactions} ({success_rate:.1f}%)")
        print(f"Entities found: {total_entities_found}/{total_entities_expected} ({entity_rate:.1f}%)")
        
        # Check for Cassian Problem
        cassian_problems = [r for r in all_results if 'Cassian' in r['missing_entities']]
        if cassian_problems:
            print(f"\nCassian Problem occurrences: {len(cassian_problems)}")
            for cp in cassian_problems[:3]:  # Show first 3
                print(f"  - Replay {cp['replay']}, Interaction {cp['interaction']}: {cp['user_input']}")
        ...
        self.assertGreaterEqual(entity_rate, 60.0,
