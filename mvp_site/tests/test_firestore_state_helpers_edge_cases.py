...
from firestore_service import (
    update_state_with_changes, DELETE_TOKEN,
    _handle_append_syntax, _handle_dict_merge
)
...
class TestFirestoreStateHelpersEdgeCases(unittest.TestCase):
    ...
    def test_update_state_delete_nested_keys(self):
        ...
        changes = {
            'player': {
                'temp_buff': DELETE_TOKEN,
                'stats': {'mp': DELETE_TOKEN}
            }
        }
    ...
    def test_update_state_complex_nested_structure(self):
        ...
        self.assertEqual(result['game']['world']['regions']['forest']['items'], 
                        ['herb'])
        # Cave unchanged
        self.assertEqual(result['game']['world']['regions']['cave'], 
                        {'enemies': ['bat'], 'items': ['ore']})
        # Desert added
        self.assertEqual(result['game']['world']['regions']['desert'], 
