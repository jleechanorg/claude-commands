...
class TestFirestoreEmptyNarrativeBug(unittest.TestCase):
    ...
    def test_empty_narrative_with_structured_fields_persists(self, mock_get_db):
        ...
        structured_fields = {
            ...
            "state_changes": {
                "thinking_mode": {"active": True, "depth": "deep"}
            }
        }
        ...
        user_campaigns = self.fake_firestore.collection('users').document(user_id).collection('campaigns')
        ...
        self.assertGreater(len(story_docs), 0,
                          "Story entry should exist despite empty narrative")
        ...
        self.assertIn('planning_block', entry_data,
                     "Planning block should be preserved")
        self.assertIn('state_changes', entry_data,
                     "State changes should be preserved")
        
        # Verify empty narrative was handled properly 
        self.assertNotEqual(entry_data['text'], '',
                           "Empty narrative should be replaced with placeholder text")
        self.assertIn('Internal thoughts', entry_data['text'],
    ...
    def test_bug_reproduction_scenario(self):
        ...
        self.assertFalse(would_save_with_original_logic,
        ...
        should_save_with_fix = structured_fields is not None and len(structured_fields) > 0
        self.assertTrue(should_save_with_fix,
