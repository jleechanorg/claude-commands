...
class TestCompleteCombinedApproach(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.game_state = {
            ...
            'world_data': {
                'current_location_name': 'The Silver Stag Tavern'
            }
        }
    ...
    def test_step1_structured_generation_prompt_creation(self):
        ...
        manifest = create_from_game_state(self.game_state, session_number=2, turn_number=15)
        ...
        structured_prompt = create_structured_prompt_injection(manifest_text, expected_entities)
    ...
    def test_step2_structured_response_parsing(self):
        ...
        self.assertEqual(structured_response.entities_mentioned, mock_llm_response["entities_mentioned"])
        self.assertEqual(structured_response.location_confirmed, mock_llm_response["location_confirmed"])
    ...
    def test_complete_combined_approach_integration(self):
        ...
        structured_prompt, expected_entities = self.test_step1_structured_generation_prompt_creation()
        ...
        print(f"   🎯 Structured Generation: ✅ PASS")
        print(f"   🎯 Response Parsing: ✅ PASS")
        print(f"   🎯 Schema Validation: ✅ PASS (100% coverage)")
        print(f"   🎯 Sync Validation: ✅ PASS ({sync_validation.confidence:.0%} confidence)")
        print(f"   🎯 Overall: ✅ 100% SUCCESS")
        ...
        print(f"   The Cassian Problem is SOLVED!")
    
    def test_failure_case_handling(self):
        """Test how the system handles failure cases"""
        print("\n🧪 Testing failure case handling")
        
        # Test case where LLM doesn't return JSON and misses entities clearly
        ...
        self.assertIsInstance(structured_response, NarrativeResponse)
        self.assertEqual(structured_response.entities_mentioned, [])  # Empty for fallback
        ...
        print(f"   ✅ Graceful fallback for non-JSON response")
        print(f"   ✅ Validation still detects missing entities: {result.entities_missing}")
