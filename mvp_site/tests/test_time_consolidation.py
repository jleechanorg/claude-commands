...
class TestTimeConsolidation(unittest.TestCase):
    ...
    def test_legacy_time_migration(self):
        ...
        self.assertEqual(
            legacy_state.world_data["world_time"]["time_of_day"], 
            "Evening"
        )
    ...
    def test_already_consolidated_data(self):
        ...
        # Verify unchanged
        self.assertEqual(
            modern_state.world_data["world_time"]["time_of_day"], 
            "Midday"
        )
    ...
    def test_hour_to_time_of_day_mappings(self):
        ...
        for hour, expected in test_cases:
            with self.subTest(hour=hour):
                state = GameState(
                    world_data={
                        "world_time": {"hour": hour, "minute": 0, "second": 0}
                    }
                )
                self.assertEqual(
                    state.world_data["world_time"]["time_of_day"],
    ...
    def test_time_of_day_without_world_time(self):
        ...
        self.assertEqual(state.world_data["world_time"]["hour"], 9)  # Estimated hour for morning
