...
from gemini_service import _validate_and_enforce_planning_block, NarrativeResponse
...
class TestPlanningBlockValidationIntegration(unittest.TestCase):
        ...
        for response_text in test_cases:
            with self.subTest(response_text=response_text):
                ...
                try:
                    ...
                    self.fail(f"Character creation detection crashed with case variation: {e}")
    ...
    def test_planning_block_regeneration_logging(self, mock_parse, mock_get_text, mock_call_api, mock_logging):
        ...
        mock_logging.info.assert_any_call("🔍 PLANNING_BLOCK_REGENERATION: Sending prompt to API")
    ...
    @patch('gemini_service._get_text_from_response')
    @patch('gemini_service._parse_gemini_response')
    def test_planning_block_validation_success_logging(self, mock_parse, mock_get_text, mock_call_api, mock_logging):
        ...
        mock_parse.return_value = ("Valid planning block content", self.structured_response)
        ...
        success_log_found = any(
            "🔍 VALIDATION_SUCCESS: String planning block passed validation" in str(call)
    ...
    @patch('gemini_service._get_text_from_response')
    @patch('gemini_service._parse_gemini_response')
    def test_planning_block_validation_failure_logging(self, mock_parse, mock_get_text, mock_call_api, mock_logging):
        ...
        for expected_log in failure_logs:
            ...
            self.assertTrue(failure_log_found, f"Should log validation failure: {expected_log}")
    ...
    def test_planning_block_exception_logging(self, mock_call_api, mock_logging):
        ...
        for expected_log in exception_logs:
            ...
            self.assertTrue(exception_log_found, f"Should log exception: {expected_log}")

    @patch('gemini_service.logging_util')
    @patch('gemini_service._call_gemini_api')
    @patch('gemini_service._get_text_from_response')
    @patch('gemini_service._parse_gemini_response')
    def test_planning_block_source_logging(self, mock_parse, mock_get_text, mock_call_api, mock_logging):
        ...
        mock_structured.planning_block = {"thinking": "test", "choices": {"Continue": "test"}}
        ...
        source_log_found = any(
            "🔍 PLANNING_BLOCK_SOURCE: Using structured_response.planning_block" in str(call)
            for call in mock_logging.info.call_args_list
        )
    ...
    @patch('gemini_service._get_text_from_response')
    @patch('gemini_service._parse_gemini_response')
    def test_planning_block_parsing_logging(self, mock_parse, mock_get_text, mock_call_api, mock_logging):
        ...
        for expected_log in parsing_logs:
            ...
            self.assertTrue(parsing_log_found, f"Should log parsing step: {expected_log}")
