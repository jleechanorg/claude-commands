...
from main import KEY_SUCCESS, KEY_ERROR, KEY_MESSAGE, HEADER_AUTH
...
class TestAuthenticationMiddleware(unittest.TestCase):
    ...
    def test_auth_bypass_in_testing_mode(self):
        ...
        with patch('main.firestore_service') as mock_firestore_service:
            ...
            mock_firestore_service.get_campaigns_for_user.assert_called_once_with('custom-test-user')
    ...
    def test_valid_firebase_token(self):
        ...
        auth_headers = {
            HEADER_AUTH: 'Bearer valid-firebase-token'
        }
        ...
        with patch('main.auth.verify_id_token') as mock_verify_id_token, \
            ...
            mock_firestore_service.get_campaigns_for_user.assert_called_once_with('firebase-user-123')
    ...
    def test_invalid_firebase_token(self):
        ...
        mock_firebase_admin.auth.verify_id_token.side_effect = Exception("Invalid token")
        
        auth_headers = {
            HEADER_AUTH: 'Bearer invalid-firebase-token'
        }
    ...
    def test_malformed_auth_header(self):
        ...
        auth_headers = {
            HEADER_AUTH: 'malformed-header-without-bearer'
        }
    ...
    def test_auth_with_multiple_space_bearer_token(self):
        ...
        auth_headers = {
            HEADER_AUTH: 'Bearer  token-with-spaces  '
        }
    ...
    def test_auth_bypass_mixed_case(self):
        ...
        with patch('main.firestore_service') as mock_firestore_service:
            ...
            self.assertEqual(response.status_code, 200)
            mock_firestore_service.get_campaigns_for_user.assert_called_once_with('case-test-user')
        ...
        for header_value, expected_token in test_cases:
            with self.subTest(header=header_value):
                ...
                with patch('main.firestore_service') as mock_firestore_service:
                    ...
                    if expected_token:
                        self.assertEqual(response.status_code, 200)
                        mock_firebase_admin.auth.verify_id_token.assert_called_once_with(expected_token)
                    else:
