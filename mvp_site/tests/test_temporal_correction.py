...
async def test_temporal_correction_preserves_original_user_input():
    ...
    mock_game_state.world_data = {
        ...
        "current_location": "Avernus (Tiamat's Lair)",
    }
    ...
    backward_response.get_state_updates.return_value = {
        "world_data": {
            ...
            "current_location": "The High Hall (Throne Room)",
        }
    }
    ...
    with patch("mvp_site.world_logic.firestore_service") as mock_firestore:
        with patch("mvp_site.world_logic.llm_service") as mock_llm:
            with patch("mvp_site.world_logic.preventive_guards") as mock_guards:
                # Setup mocks
