...
def test_structured_fields_storage():
    ...
    campaign, story_entries = mock_firestore_service.get_campaign_by_id(user_id, campaign_id)
    ...
    if ai_entry.get('session_header') != structured_fields['session_header']:
        print(f"❌ session_header mismatch: {ai_entry.get('session_header')} != {structured_fields['session_header']}")
        success = False
    else:
        print("✓ session_header preserved")
    
    if ai_entry.get('planning_block') != structured_fields['planning_block']:
        print(f"❌ planning_block mismatch: {ai_entry.get('planning_block')} != {structured_fields['planning_block']}")
        success = False
    else:
        print("✓ planning_block preserved")
    
    if ai_entry.get('dice_rolls') != structured_fields['dice_rolls']:
        print(f"❌ dice_rolls mismatch: {ai_entry.get('dice_rolls')} != {structured_fields['dice_rolls']}")
        success = False
    else:
        print("✓ dice_rolls preserved")
    
    if ai_entry.get('resources') != structured_fields['resources']:
        print(f"❌ resources mismatch: {ai_entry.get('resources')} != {structured_fields['resources']}")
        success = False
    else:
        print("✓ resources preserved")
    
    if ai_entry.get('debug_info') != structured_fields['debug_info']:
        print(f"❌ debug_info mismatch: {ai_entry.get('debug_info')} != {structured_fields['debug_info']}")
        success = False
    else:
