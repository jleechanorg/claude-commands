...
class TestSimpleJsonBugCheck(unittest.TestCase):
    ...
    "debug_info": {
        ...
        try:
            ...
            has_json_artifacts = (
                '"narrative":' in result_text or 
                '"god_mode_response":' in result_text or
                '"entities_mentioned":' in result_text
            )
            print(f"Contains JSON artifacts: {has_json_artifacts}")
            
            # Test assertions
            self.assertNotIn('"narrative":', result_text, "Should not contain JSON keys")
            self.assertNotIn('"god_mode_response":', result_text, "Should not contain JSON keys")
            self.assertIn("CHARACTER CREATION", result_text, "Should contain actual content")
    ...
    "debug_info": {}
        ...
        try:
            ...
            self.assertNotIn('malformed": "missing closing brace', result_text,
