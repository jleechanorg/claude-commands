...
class TestDebugLoggingCoverage(unittest.TestCase):
    ...
    def test_log_api_response_safely_empty_response(self, mock_logging):
        ...
        mock_logging.warning.assert_called_with(
            "🔍 API_RESPONSE_DEBUG (test_context): Response is empty or None"
        )
    ...
    def test_log_api_response_safely_short_response(self, mock_logging):
        ...
        expected_calls = [
            call(
                f"🔍 API_RESPONSE_DEBUG (test_context): Length: {len(test_response)} chars"
            ),
    ...
    def test_log_api_response_safely_non_string_input(self, mock_logging):
        ...
        mock_logging.info.assert_any_call(
            f"🔍 API_RESPONSE_DEBUG (dict_context): Length: {len(response_str)} chars"
        )
        mock_logging.info.assert_any_call(
            f"🔍 API_RESPONSE_DEBUG (dict_context): Content: {response_str}"
        )
    ...
    def test_log_api_response_safely_default_params(self, mock_logging):
        ...
        mock_logging.info.assert_any_call(
            f"🔍 API_RESPONSE_DEBUG (): Length: {len(test_response)} chars"
        )
        mock_logging.info.assert_any_call(
            f"🔍 API_RESPONSE_DEBUG (): Content: {test_response}"
        )
    ...
    def test_malformed_json_debug_logging_short_text(self, mock_logging):
        ...
        mock_logging.debug.assert_any_call(
            f"🔍 MALFORMED_JSON_CONTENT: Length: {len(malformed_json)} chars"
        )

        # Since text is short, should log full content
        mock_logging.debug.assert_any_call(
            f"🔍 MALFORMED_JSON_CONTENT: {malformed_json}"
        )
    ...
    def test_malformed_json_debug_logging_long_text(self, mock_logging):
        ...
        with patch("robust_json_parser.try_parse_json", return_value=(None, False)):
            with patch("robust_json_parser.complete_truncated_json", return_value=None):
                with patch(
                    "robust_json_parser.extract_json_boundaries", return_value=None
                ):
                    with patch(
                        "robust_json_parser.RobustJSONParser._extract_fields",
                        return_value={"narrative": "test"},
                    ):
                        result, was_malformed = self.parser.parse(malformed_json)
        ...
        self.assertTrue(
            truncated_call_found, "Should log truncated content for long text"
        )
    ...
    def test_extracted_fields_logging(self, mock_logging):
        ...
        if result:  # Only if extraction succeeded
            mock_logging.info.assert_any_call(
                f"🔍 EXTRACTED_FIELDS: {list(result.keys())}"
            )
    ...
    def test_exception_handling_with_traceback_logging(
        self, mock_format_exc, mock_logging
    ):
        ...
        expected_logs = [
            ...
            "🔍 PLANNING_BLOCK_EXCEPTION: Traceback: Mocked traceback output",
        ]
    ...
    def test_case_insensitive_character_detection_logging(self, mock_logging):
        ...
        for test_case in test_cases:
            ...
            is_character_approval = (
                ...
                and "would you like to play as this character" in response_lower
            )

            self.assertTrue(
                is_character_approval,
                f"Should detect character creation in: {test_case}",
            )
