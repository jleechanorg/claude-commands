...
from firestore_service import (
    _handle_append_syntax, _handle_core_memories_safeguard,
    _handle_dict_merge, _handle_delete_token, _handle_string_to_dict_update,
    DELETE_TOKEN, update_state_with_changes
)
...
class TestFirestoreStateHelpers(unittest.TestCase):
    ...
    def test_handle_append_syntax_valid(self, mock_log, mock_append):
        ...
        mock_log.assert_called_with("update_state: Detected explicit append for 'items'.")
    ...
    def test_handle_append_syntax_core_memories_dedup(self, mock_log, mock_append):
        ...
        mock_append.assert_called_once_with(
            state['core_memories'], ['memory2', 'memory1'], 'core_memories', deduplicate=True
        )
    ...
    @patch('firestore_service._perform_append')
    @patch('logging_util.warning')
    def test_handle_core_memories_safeguard_creates_list(self, mock_warning, mock_append):
    ...
    def test_handle_delete_token_deletes_existing(self, mock_log):
        ...
        mock_log.assert_called_with("update_state: Deleting key 'key1' due to DELETE_TOKEN.")
    ...
    def test_handle_delete_token_missing_key(self, mock_log):
        ...
        mock_log.assert_called_with("update_state: Attempted to delete key 'missing' but it doesn't exist.")
