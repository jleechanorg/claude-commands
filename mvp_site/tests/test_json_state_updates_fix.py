    ...
    def setUp(self):
        ...
        self.state_updates = {
            "player_character_data": {
                "hp_current": 15,
                "combat_state": {"is_in_combat": True, "current_turn": "player"},
            },
            "npc_data": {"Goblin_1": {"hp_current": 8, "status": "wounded"}},
        }
        ...
        with patch("main.firestore_service") as mock_firestore:
            with patch("main.gemini_service") as mock_gemini:
                ...
                if "hp_current" in updated_state["player_character_data"]:
                    self.assertEqual(
                        updated_state["player_character_data"]["hp_current"], 15
                    )
                if "combat_state" in updated_state["player_character_data"]:
                    self.assertEqual(
                        updated_state["player_character_data"]["combat_state"][
                            "is_in_combat"
                        ],
                        True,
                    )
                if (
                    "npc_data" in updated_state
                    and "Goblin_1" in updated_state["npc_data"]
                ):
                    self.assertEqual(
                        updated_state["npc_data"]["Goblin_1"]["hp_current"], 8
                    )
                    self.assertEqual(
                        updated_state["npc_data"]["Goblin_1"]["status"], "wounded"
                    )
        ...
        with patch("main.firestore_service") as mock_firestore:
            with patch("main.gemini_service") as mock_gemini:
                ...
                if mock_firestore.update_campaign_game_state.called:
                    ...
                    self.assertEqual(
                        updated_state.get("player_character_data", {}).get(
                            "hp_current"
                        ),
                        25,
                    )
