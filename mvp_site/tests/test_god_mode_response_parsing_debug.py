...
class TestGodModeResponseParsingDebug(unittest.TestCase):
    ...
    "debug_info": {
        ...
        try:
            ...
            print(f"god_mode_response length: {len(parsed_json.get('god_mode_response', ''))}")
        except json.JSONDecodeError as e:
            self.fail(f"JSON parsing failed: {e}")
        
        # Debug: Test parse_structured_response
        try:
            narrative_text, structured_response = parse_structured_response(luke_scene_31)
            print(f"parse_structured_response succeeded")
            print(f"Returned narrative_text: '{narrative_text[:100]}...' (length: {len(narrative_text)})")
            ...
            self.assertNotIn('"god_mode_response":', narrative_text, "Should not contain JSON keys") 
    ...
    def test_narrative_response_creation_with_god_mode(self):
        ...
        try:
            ...
            print(f"NarrativeResponse creation succeeded")
            print(f"Has god_mode_response attr: {hasattr(response, 'god_mode_response')}")
        ...
        try:
            ...
            if god_mode_response:
                ...
                if narrative is None:
                    narrative = ''
                from narrative_response_schema import _combine_god_mode_and_narrative
                combined_response = _combine_god_mode_and_narrative(god_mode_response, narrative)
    ...
    def test_check_narrative_response_schema(self):
        ...
        print(f"NarrativeResponse fields: {getattr(NarrativeResponse, '__annotations__', 'No annotations')}")
