...
class TestStructuredFieldsUtils(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_structured_data = {
            ...
            'dice_rolls': ['Initiative: d20+2 = 15', 'Attack roll: d20+5 = 18', 'Damage: 1d8+3 = 7'],
        ...
        self.mock_structured_response.session_header = self.sample_structured_data['session_header']
        self.mock_structured_response.planning_block = self.sample_structured_data['planning_block']
        self.mock_structured_response.dice_rolls = self.sample_structured_data['dice_rolls']
        self.mock_structured_response.resources = self.sample_structured_data['resources']
        self.mock_structured_response.debug_info = self.sample_structured_data['debug_info']
    ...
    def test_extract_structured_fields_with_full_data(self):
        ...
        self.assertEqual(result[constants.FIELD_SESSION_HEADER], self.sample_structured_data['session_header'])
        self.assertEqual(result[constants.FIELD_PLANNING_BLOCK], self.sample_structured_data['planning_block'])
        self.assertEqual(result[constants.FIELD_DICE_ROLLS], self.sample_structured_data['dice_rolls'])
        self.assertEqual(result[constants.FIELD_RESOURCES], self.sample_structured_data['resources'])
        self.assertEqual(result[constants.FIELD_DEBUG_INFO], self.sample_structured_data['debug_info'])
    ...
    def test_extract_structured_fields_with_complex_debug_info(self):
        ...
        mock_structured_response.dice_rolls = ['Attack: d20+3 = 16', 'Damage: 2d6+2 = 8']
        ...
        self.assertEqual(result[constants.FIELD_DEBUG_INFO]['enemy_status']['goblin_1']['hp'], 8)
        self.assertEqual(result[constants.FIELD_DEBUG_INFO]['environmental_factors'], ['heavy_rain', 'difficult_terrain'])
...
8. Use a different approach (describe your action)"""
        ...
        mock_structured_response.dice_rolls = ['Perception: d20+4 = 18', 'Investigation: d20+2 = 14']
        mock_structured_response.resources = 'HP: 28/30, SP: 12/15, Torch time: 30 min'
        mock_structured_response.debug_info = {'location': 'temple_chamber', 'trap_disarmed': True}
        ...
        self.assertIn('Ancient Temple - Main Chamber', result[constants.FIELD_SESSION_HEADER])
