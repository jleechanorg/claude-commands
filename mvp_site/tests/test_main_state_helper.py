...
from main import _handle_legacy_migration, create_app
...
class TestLegacyMigration(unittest.TestCase):
    """Test legacy migration functionality."""

    def setUp(self):
        """Set up test environment."""
        self.user_id = 'test-user'
        self.campaign_id = 'test-campaign'
        self.story_context = [{'actor': 'gm', 'content': 'Welcome!'}]

    def test_legacy_migration_not_checked_status(self):
        """Test legacy migration when status is NOT_CHECKED."""
        with patch('main.logging_util') as mock_logging, \
             patch('main.MigrationStatus') as mock_migration_status:
            
            # Mock game state with NOT_CHECKED status
            mock_game_state = MagicMock()
            mock_game_state.migration_status = mock_migration_status.NOT_CHECKED
            
            # Mock the migration process - no perform_legacy_migration function exists
            # The migration is handled internally in _handle_legacy_migration
            result = _handle_legacy_migration(
                mock_game_state, 
                self.campaign_id, 
                self.story_context, 
                self.user_id
            ...
            # Should return the same game state since migration is handled internally
            self.assertEqual(result, mock_game_state)

    def test_legacy_migration_already_checked(self):
        """Test legacy migration when status is already checked."""
        with patch('main.logging_util') as mock_logging, \
             patch('main.MigrationStatus') as mock_migration_status:
            
            # Mock game state with COMPLETED status
            mock_game_state = MagicMock()
            mock_game_state.migration_status = mock_migration_status.COMPLETED
            ...
            # Should return original state without migration
            self.assertEqual(result, mock_game_state)

    def test_legacy_migration_in_progress_status(self):
        """Test legacy migration when status is IN_PROGRESS."""
        with patch('main.logging_util') as mock_logging, \
             patch('main.MigrationStatus') as mock_migration_status:
            
            # Mock game state with IN_PROGRESS status
            mock_game_state = MagicMock()
            mock_game_state.migration_status = mock_migration_status.IN_PROGRESS
