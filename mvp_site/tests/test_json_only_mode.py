...
class TestJSONOnlyMode(unittest.TestCase):
    ...
    def test_all_gemini_calls_must_use_json_mode(self):
        ...
        with patch('gemini_service.get_client') as mock_get_client:
            ...
            mock_client.models.generate_content = Mock(return_value=Mock(
    ...
    def test_main_py_no_fallback_parsing(self):
        ...
        mock_response.narrative_text = "[STATE_UPDATES_PROPOSED]{\"test\": true}[END_STATE_UPDATES_PROPOSED]"
    ...
    def test_generation_config_always_includes_json(self):
        ...
        with patch('gemini_service.get_client') as mock_get_client:
            mock_client = Mock()
            mock_get_client.return_value = mock_client
            mock_client.models.generate_content = Mock(return_value=Mock(text='{"narrative": "test"}'))
            
            # Test various API calls
            test_cases = [
                ("continue_story", lambda: gemini_service.continue_story([], "prompt", "story")),
                ("generate_opening_story", lambda: gemini_service.generate_opening_story("prompt")),
                ("get_god_response", lambda: gemini_service.get_god_response({}, "command"))
            ]
