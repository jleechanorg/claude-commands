...
class TestCombatCleanupComprehensive(unittest.TestCase):
    ...
    def test_automatic_cleanup_via_state_updates_hp_defeat(self):
        ...
        combatants_data = [
            ...
            {"name": "Orc Warrior", "initiative": 14, "type": "enemy", "hp_current": 30, "hp_max": 30},  # Healthy enemy
            {"name": "Goblin", "initiative": 12, "type": "enemy", "hp_current": 15, "hp_max": 15},      # Healthy enemy
        ]
        ...
        game_state.npc_data = {
            "Orc Warrior": {"name": "Fierce Orc", "type": "enemy", "description": "A brutal warrior"},
            "Goblin": {"name": "Sneaky Goblin", "type": "enemy", "description": "Quick and cunning"},
            "Merchant": {"name": "Bob", "type": "neutral", "description": "Friendly trader"}
        }
        
        # Verify initial state
        self.assertIn("Orc Warrior", game_state.combat_state['combatants'])
        self.assertIn("Goblin", game_state.combat_state['combatants'])
        self.assertEqual(game_state.combat_state['combatants']['Orc Warrior']['hp_current'], 30)
        
        print(f"Initial combatants: {list(game_state.combat_state['combatants'].keys())}")
        ...
        updated_state_dict = update_state_with_changes(initial_state_dict, ai_proposed_changes)
        final_state_dict = apply_automatic_combat_cleanup(updated_state_dict, ai_proposed_changes)
        
        # Step 4: VERIFY the cleanup worked
        print(f"After automatic cleanup - Combatants: {list(final_state_dict['combat_state']['combatants'].keys())}")
        print(f"After automatic cleanup - NPCs: {list(final_state_dict['npc_data'].keys())}")
        
        # Verify the fix is working
        
        # The defeated enemy should be completely removed from combat
        self.assertNotIn("Orc Warrior", final_state_dict['combat_state']['combatants'],
        ...
        initiative_names = [entry['name'] for entry in final_state_dict['combat_state']['initiative_order']]
        self.assertNotIn("Orc Warrior", initiative_names,
                        "Defeated enemy should be removed from initiative order")
        
        # The defeated enemy should be removed from NPC data
        self.assertNotIn("Orc Warrior", final_state_dict['npc_data'],
                        "Defeated enemy should be removed from NPC data")
        
        # Living entities should remain
        self.assertIn("Hero", final_state_dict['combat_state']['combatants'],
                     "Hero should remain in combat")
        self.assertIn("Goblin", final_state_dict['combat_state']['combatants'],
                     "Living enemy should remain in combat")
        self.assertIn("Merchant", final_state_dict['npc_data'],
    ...
    def test_combat_end_with_pre_defeated_enemies(self):
        ...
        game_state = GameState()
        combatants_data = [
            {"name": "Player", "initiative": 15, "type": "pc", "hp_current": 40, "hp_max": 50},
            {"name": "Dead Orc", "initiative": 12, "type": "enemy", "hp_current": 0, "hp_max": 25},  # Already dead
            {"name": "Living Wolf", "initiative": 10, "type": "companion", "hp_current": 20, "hp_max": 20}
        ]
        ...
        self.assertEqual(game_state.combat_state['combatants']['Dead Orc']['hp_current'], 0)
        
        print(f"Before combat end - Combatants: {list(game_state.combat_state['combatants'].keys())}")
        print(f"Dead Orc HP: {game_state.combat_state['combatants']['Dead Orc']['hp_current']}")
        ...
        print(f"After combat end - In combat: {final_state_dict['combat_state']['in_combat']}")
        print(f"After combat end - Combatants: {list(final_state_dict['combat_state']['combatants'].keys())}")
        
        # Verify expectations
        
        # Combat should be properly ended
        self.assertFalse(final_state_dict['combat_state']['in_combat'],
                        "Combat should be ended")
        
        # The pre-defeated enemy should be cleaned up when combat ends
        self.assertNotIn("Dead Orc", final_state_dict['combat_state']['combatants'],
                        "Pre-defeated enemies should be cleaned up when combat ends")
        
        self.assertNotIn("Dead Orc", final_state_dict['npc_data'],
                        "Pre-defeated enemies should be removed from NPC data")
        
        # Living entities should remain
        self.assertIn("Player", final_state_dict['combat_state']['combatants'],
                     "Player should remain")
        self.assertIn("Living Wolf", final_state_dict['combat_state']['combatants'],
    ...
    def test_multiple_enemies_defeated_same_turn(self):
        ...
        game_state = GameState()
        combatants_data = [
            {"name": "Wizard", "initiative": 20, "type": "pc", "hp_current": 35, "hp_max": 35},
            {"name": "Goblin A", "initiative": 15, "type": "enemy", "hp_current": 10, "hp_max": 10},
            {"name": "Goblin B", "initiative": 14, "type": "enemy", "hp_current": 8, "hp_max": 8},
            {"name": "Orc Chief", "initiative": 13, "type": "enemy", "hp_current": 25, "hp_max": 25}
        ]
        ...
        print(f"Before fireball - Combatants: {list(game_state.combat_state['combatants'].keys())}")
        
        # Step 2: Simulate AI casting fireball that kills multiple enemies
        ai_proposed_changes = {
            "combat_state": {
                "combatants": {
                    "Goblin A": {"hp_current": 0, "status": ["dead"]},  # Killed by fireball
                    "Goblin B": {"hp_current": 0, "status": ["dead"]},  # Killed by fireball
                    "Orc Chief": {"hp_current": 5, "status": ["wounded"]}  # Survived but wounded
                }
            }
        }
        ...
        print(f"After fireball - Combatants: {list(final_state_dict['combat_state']['combatants'].keys())}")
        print(f"After fireball - NPCs: {list(final_state_dict['npc_data'].keys())}")
        
        # Verify cleanup worked correctly
        
        # Both goblins should be removed
        self.assertNotIn("Goblin A", final_state_dict['combat_state']['combatants'],
                        "First defeated enemy should be removed")
        self.assertNotIn("Goblin B", final_state_dict['combat_state']['combatants'],
                        "Second defeated enemy should be removed")
        
        self.assertNotIn("Goblin A", final_state_dict['npc_data'],
                        "First defeated enemy should be removed from NPCs")
        self.assertNotIn("Goblin B", final_state_dict['npc_data'],
                        "Second defeated enemy should be removed from NPCs")
        
        # Survivors should remain
        self.assertIn("Wizard", final_state_dict['combat_state']['combatants'],
                     "PC should remain")
        self.assertIn("Orc Chief", final_state_dict['combat_state']['combatants'],
                     "Wounded but living enemy should remain")
        self.assertIn("Village Elder", final_state_dict['npc_data'],
                     "Non-combat NPC should remain")
        
        # Verify wounded enemy has correct HP
        self.assertEqual(final_state_dict['combat_state']['combatants']['Orc Chief']['hp_current'], 5,
    ...
    def test_cleanup_without_explicit_combat_state_changes(self):
        ...
        game_state = GameState()
        combatants_data = [
            {"name": "Ranger", "initiative": 16, "type": "pc", "hp_current": 30, "hp_max": 30},
            {"name": "Troll", "initiative": 8, "type": "enemy", "hp_current": 0, "hp_max": 45}  # Defeated previously
        ]
        ...
        self.assertEqual(game_state.combat_state['combatants']['Troll']['hp_current'], 0)
        
        print(f"Before update - Troll HP: {game_state.combat_state['combatants']['Troll']['hp_current']}")
        print(f"Before update - Combatants: {list(game_state.combat_state['combatants'].keys())}")
