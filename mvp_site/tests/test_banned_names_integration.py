    ...
    def test_load_banned_names_file_not_found(self):
        ...
        with patch('builtins.open', side_effect=FileNotFoundError):
            with patch('logging.warning') as mock_warning:
                ...
                self.assertIn("Banned names file not found", mock_warning.call_args[0][0])
    ...
    def test_world_content_includes_banned_names(self, mock_load_banned):
        ...
        with patch('builtins.open', side_effect=mock_open_file):
            with patch('os.path.join') as mock_join:
                ...
                self.assertIn("7. **Name Restrictions**: NEVER use any name from the banned names list", result)
    
    def test_banned_names_in_critical_section(self):
        """Test that banned names appear in the critical naming restrictions section"""
        with patch('world_loader.load_banned_names', return_value=self.sample_banned_names):
            with patch('builtins.open', mock_open(read_data="dummy content")):
                with patch('os.path.join') as mock_join:
                    ...
                    self.assertNotEqual(critical_start, -1, "CRITICAL NAMING RESTRICTIONS section not found")
            ...
            with patch('builtins.open', mock_open(read_data="dummy content")):
                with patch('os.path.join') as mock_join:
                    ...
                    for name in names_to_test:
                        self.assertIn(name, result, f"Banned name '{name}' not found in world content")
            ...
            with patch('builtins.open', mock_open(read_data="dummy content")):
                with patch('os.path.join') as mock_join:
                    ...
                    for item in enforcement_items:
                        self.assertIn(item, result, f"Enforcement item '{item}' not found")
...
class TestBannedNamesRealFile(unittest.TestCase):
    ...
    def test_real_banned_names_file_exists(self):
        ...
        if os.path.exists(self.banned_names_path):
            with open(self.banned_names_path, 'r') as f:
                content = f.read()
            ...
            key_names = ["Alaric", "Corvus", "Elara", "Valerius", "Aria", "Phoenix", "Raven"]
            ...
            self.assertGreaterEqual(len(name_lines), 50, "Expected at least 50 banned names")
