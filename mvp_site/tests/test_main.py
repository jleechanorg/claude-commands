...
class TestComplianceAPI(unittest.TestCase):
    ...
    @patch('claude_service.process_push_compliance')
    def test_track_push_compliance_endpoint(self, mock_process_push_compliance):
        """Test the /api/track-push-compliance endpoint."""
        # Mock the process_push_compliance function
        mock_process_push_compliance.return_value = {
            'push_compliance_rate': 0.85,
            'has_push_command': True,
            'push_verified': True,
            'push_command': 'git push origin HEAD:feature-branch',
        ...
        test_data = {
            'session_id': 'test-session-123',
            'command_text': 'git push origin HEAD:feature-branch && gh pr view',
            'verification_output': 'Pull request #123 is open'
        }
        
        response = self.client.post(
            '/api/track-push-compliance',
            headers=self.test_headers,
        ...
        self.assertTrue(data['success'])
        self.assertEqual(data['push_compliance_rate'], 0.85)
        self.assertTrue(data['has_push_command'])
        self.assertTrue(data['push_verified'])
        self.assertEqual(data['push_command'], 'git push origin HEAD:feature-branch')
        self.assertEqual(data['verification_command'], 'gh pr view')
        
        # Verify the function was called with correct parameters
        mock_process_push_compliance.assert_called_once_with(
            'test-session-123',
            'git push origin HEAD:feature-branch && gh pr view',
            'Pull request #123 is open'
        )

    def test_track_push_compliance_missing_session_id(self):
        """Test /api/track-push-compliance with missing session_id."""
        test_data = {
            'command_text': 'git push origin HEAD:feature-branch'
        }
    ...
    def test_push_compliance_status_endpoint(self, mock_get_push_compliance_status):
        """Test the /api/push-compliance-status/<session_id> endpoint."""
        # Mock the get_push_compliance_status function
        def mock_get_status(session_id):
            if session_id == 'high-push-compliance':
                return {
            ...
            elif session_id == 'low-push-compliance':
                return {
                    'compliance_rate': 0.60,
                    'is_compliant': False,
                    'total_pushes': 5,
        ...
        mock_get_push_compliance_status.side_effect = mock_get_status
        
        # Test high compliance session
        response = self.client.get(
            '/api/push-compliance-status/high-push-compliance',
            headers=self.test_headers
        )
        data = json.loads(response.data)
        self.assertEqual(data['compliance_rate'], 0.90)
        self.assertTrue(data['is_compliant'])
        self.assertEqual(data['total_pushes'], 10)
        self.assertEqual(data['verified_pushes'], 9)
        
        # Test low compliance session
        response = self.client.get(
            '/api/push-compliance-status/low-push-compliance',
            headers=self.test_headers
        )
        data = json.loads(response.data)
        self.assertEqual(data['compliance_rate'], 0.60)
        self.assertFalse(data['is_compliant'])
        self.assertEqual(data['total_pushes'], 5)
        self.assertEqual(data['verified_pushes'], 3)

    @patch('firestore_service.get_global_push_compliance_stats')
    def test_push_compliance_stats_endpoint(self, mock_get_global_stats):
        """Test the /api/push-compliance-stats endpoint."""
        # Mock the global stats function
        mock_get_global_stats.return_value = {
            'total_pushes': 50,
            'verified_pushes': 42,
        ...
        self.assertTrue(data['success'])
        stats = data['global_push_stats']
        self.assertEqual(stats['total_pushes'], 50)
        self.assertEqual(stats['verified_pushes'], 42)
        self.assertEqual(stats['compliance_rate'], 0.84)
        self.assertEqual(stats['unique_sessions'], 8)

    @patch('claude_service.get_push_compliance_status')
    @patch('claude_service.generate_push_compliance_alert')
    def test_push_compliance_alert_endpoint(self, mock_generate_alert, mock_get_status):
        """Test the /api/push-compliance-alert/<session_id> endpoint."""
        # Mock the functions
        def mock_status(session_id):
            if session_id == 'low-compliance-session':
                return {'compliance_rate': 0.65}
            else:
                return {'compliance_rate': 0.90}
        
        def mock_alert(compliance_rate):
            if compliance_rate < 0.80:
                return f"Push compliance is at {int(compliance_rate * 100)}%. Minimum required: 80%. Remember to verify push success with 'gh pr view' or 'git log origin/branch'"
            return None
        
        mock_get_status.side_effect = mock_status
        mock_generate_alert.side_effect = mock_alert
        
        # Test session with low compliance (should have alert)
        response = self.client.get(
            '/api/push-compliance-alert/low-compliance-session',
            headers=self.test_headers
        )
        data = json.loads(response.data)
        self.assertTrue(data['success'])
        self.assertIsNotNone(data['alert'])
        self.assertIn('65%', data['alert'])
        self.assertEqual(data['compliance_rate'], 0.65)
        
        # Test session with high compliance (should have no alert)
        response = self.client.get(
            '/api/push-compliance-alert/high-compliance-session',
            headers=self.test_headers
        )
        data = json.loads(response.data)
        self.assertTrue(data['success'])
