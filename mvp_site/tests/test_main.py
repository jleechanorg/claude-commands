...
class TestApiEndpoints(unittest.TestCase):
    ...
    def test_god_ask_state_with_delete_sentinel(self, mock_firestore_service):
        ...
        response_json_str = (
            response_data["response"].replace("```json\\n", "").replace("\\n```", "")
        )
    ...
    def test_god_mode_set_command(self, mock_update_state, mock_firestore_service):
        ...
        mock_update_state.return_value = {
            "player": {"name": "Hero", "level": 5, "hp": 50}
        }
    ...
    @patch("main.firestore_service")
    @patch("main.update_state_with_changes")
    def test_god_mode_update_state_command(
        self, mock_update_state, mock_firestore_service
    ):
    ...
    def test_normal_ai_interaction_flow(
        ...
        mock_gemini_response.debug_tags_present = {
            "dm_notes": False,
            "dice_rolls": False,
            "state_changes": False,
        }
        ...
        mock_gemini_response.get_narrative_text = MagicMock(
...
player.class = "Warrior\""""
    ...
    def test_truncate_game_state_for_logging_over_limit(self):
        ...
        self.assertEqual(
            len(lines), 5
        )  # Should be max_lines (including truncation message)
