...
def create_mock_gemini_response(
    ...
    mock_response.debug_tags_present = debug_tags_present or {
        "dm_notes": False,
        "dice_rolls": False,
        "state_changes": False,
    }
...
class TestDebugMode(unittest.TestCase):
    ...
    def test_debug_mode_toggle_commands(self, mock_firestore_service):
        ...
        updated_state = mock_firestore_service.update_campaign_game_state.call_args[0][
            2
        ]
    ...
    def test_debug_content_shown_when_enabled(self, mock_firestore_service):
        ...
        with patch("gemini_service.continue_story", return_value=mock_gemini_response):
            ...
            self.assertIn(
                "debug_info",
                data,
                "debug_info should be in response when debug mode enabled",
            )
            debug_info = data.get("debug_info", {})
            self.assertIn("dm_notes", debug_info, "dm_notes should be in debug_info")
            self.assertIn(
                "dice_rolls", debug_info, "dice_rolls should be in debug_info"
            )
            # Response should be clean narrative without embedded tags
            self.assertNotIn(
                "[DEBUG_START]",
                data.get("response", ""),
                "Response should not contain legacy debug tags",
            )
    ...
    def test_enhanced_dice_roll_display(self, mock_firestore_service):
        ...
        mock_response = (
            "You attack the orc! Your sword strikes true, dealing a devastating blow!"
        )
    ...
    def test_debug_content_hidden_when_disabled(self, mock_firestore_service):
        ...
        with patch("gemini_service.continue_story", return_value=mock_gemini_response):
            ...
            self.assertNotIn(
                "[DEBUG_START]",
                response_text,
                "Legacy debug tags should not be in response",
            )
            self.assertNotIn(
                "[DEBUG_END]",
                response_text,
            ...
            self.assertIn(
                "debug_info", data, "debug_info should always be included when present"
            )
