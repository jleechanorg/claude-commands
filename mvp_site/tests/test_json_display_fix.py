...
class TestJSONDisplayFix(unittest.TestCase):
    ...
    def test_narrative_extraction_with_invalid_field_fallback(self):
        ...
        self.assertIsInstance(result_narrative, str, "Should return narrative as string")
        
        # Critical assertion: should return the extracted narrative, not raw JSON
        self.assertIn("Scene #10", result_narrative, "Should contain the actual narrative text")
        self.assertIn("DM MODE", result_narrative, "Should contain the narrative content")
        self.assertNotIn('"narrative":', result_narrative, "Should NOT contain raw JSON structure")
        self.assertNotIn('"entities_mentioned":', result_narrative, "Should NOT contain raw JSON structure")
        
        # Response object should be valid fallback
        self.assertIsInstance(result_response, NarrativeResponse, "Should return NarrativeResponse object")
        self.assertEqual(result_response.narrative, result_narrative, "Response narrative should match returned narrative")
        self.assertEqual(result_response.entities_mentioned, ["Ser Caius of House Varrick", "Ser Bastion"])
        self.assertEqual(result_response.location_confirmed, "Starfall Command Barracks, Aeterna")
...
```'''
        ...
        result_narrative, result_response = parse_structured_response(markdown_json_response)
        ...
        self.assertNotIn("```json", result_narrative, "Should strip markdown formatting")
        self.assertNotIn('"narrative":', result_narrative, "Should extract narrative, not return raw JSON")
    ...
    def test_successful_json_parsing(self):
        ...
        self.assertIsInstance(result_response, NarrativeResponse)
        self.assertEqual(result_response.entities_mentioned, ["Sir Knight", "Ancient Dragon"])
    ...
    def test_regression_no_raw_json_in_narrative(self):
        ...
        for test_json in test_cases:
            with self.subTest(json_input=test_json[:50] + "..."):
                ...
                self.assertNotIn('"entities_mentioned":', result_narrative,
                               f"Narrative should not contain JSON structure: {result_narrative}")
                self.assertNotIn('"location_confirmed":', result_narrative,
                               f"Narrative should not contain JSON structure: {result_narrative}")
