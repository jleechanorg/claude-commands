...
class TestMissionHandler(unittest.TestCase):
    ...
    def test_process_mission_data(self):
        ...
        MissionHandler.process_mission_data(state, 'active_missions', 'quest1', 
                                          {'name': 'New Quest'})
    ...
    def test_handle_missions_dict_conversion(self):
        ...
        MissionHandler.handle_missions_dict_conversion(state, 'active_missions', missions_dict)
        self.assertEqual(len(state['active_missions']), 2)
        self.assertTrue(any(m['mission_id'] == 'quest1' for m in state['active_missions']))
        self.assertTrue(any(m['mission_id'] == 'quest2' for m in state['active_missions']))
    ...
    def test_handle_active_missions_conversion(self, mock_logging):
        ...
        MissionHandler.handle_active_missions_conversion(state, 'active_missions', missions_dict)
        self.assertEqual(len(state['active_missions']), 1)
        
        # Test with invalid value
        state = {}
        MissionHandler.handle_active_missions_conversion(state, 'active_missions', 'invalid')
        self.assertEqual(state['active_missions'], [])
