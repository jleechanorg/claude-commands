...
class TestDebugModeParser(unittest.TestCase):
    ...
    def test_only_works_in_god_mode(self):
        ...
        for cmd in test_cases:
            with self.subTest(cmd=cmd):
                ...
                self.assertIsNone(
                    result_char, f"Should not work in character mode: {cmd}"
                )
    ...
    def test_is_debug_toggle_command(self):
        ...
        self.assertTrue(
            DebugModeParser.is_debug_toggle_command("enable debug mode", "god")
        )
        self.assertTrue(DebugModeParser.is_debug_toggle_command("debug off", "god"))

        # Should return False for non-debug commands
        self.assertFalse(
            DebugModeParser.is_debug_toggle_command("attack goblin", "god")
        )
        self.assertFalse(
            DebugModeParser.is_debug_toggle_command("debug the issue", "god")
        )

        # Should return False in character mode
        self.assertFalse(
            DebugModeParser.is_debug_toggle_command("enable debug mode", "character")
        )
