...
class NumericFieldConverter:
    ...
    @classmethod
    def convert_dict_with_fields(cls, data: Dict[str, Any], numeric_fields: Set[str]) -> Dict[str, Any]:
        """
        ...
        for key, value in data.items():
            if isinstance(value, dict):
                ...
                result[key] = [
                    cls.convert_dict_with_fields(item, numeric_fields) if isinstance(item, dict) 
                    else cls.try_convert_to_int(item) if key in numeric_fields
                    else item
                    for item in value
                ]
            else:
    ...
    def convert_all_possible_ints(cls, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            if isinstance(value, dict):
                ...
                # Process lists
                result[key] = [
                    cls.convert_all_possible_ints(item) if isinstance(item, dict) 
