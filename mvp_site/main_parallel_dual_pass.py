...
from mvp_site.llm_service import generate_content
...
def add_parallel_dual_pass_routes(app, get_campaign_info):
    ...
    def enhance_entities(campaign_id):
        ...
        try:
            ...
            logging_util.info(
                f"PARALLEL_DUAL_PASS: Starting background entity enhancement for {len(missing_entities)} entities"
            )
            ...
            if final_narrative:
                logging_util.info(
                    f"PARALLEL_DUAL_PASS: Successfully enhanced narrative with {len(missing_entities)} entities"
                )
                return jsonify(
                    {
            ...
            logging_util.error(
                f"PARALLEL_DUAL_PASS: Error in enhance_entities: {str(e)}"
            )
