...
def create_app():
    ...
    def handle_interaction(user_id, campaign_id):
        try:
            ...
            logging_util.error(f"Critical error in handle_interaction for campaign {campaign_id}: {e}")
            logging_util.error(f"User input: {user_input if 'user_input' in locals() else 'N/A'}")
