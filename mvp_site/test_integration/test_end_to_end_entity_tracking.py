...
class TestEndToEndEntityTracking(unittest.TestCase):
    ...
    def test_full_flow_with_503_fallback_and_entity_retry(self, mock_get_client):
        ...
        responses = [
            ...
            Mock(
                text='{"narrative": "Sariel enters the library quietly.", "entity_tracking": {"mentioned_entities": ["Sariel"]}}'
            ),
            Mock(
                text='{"narrative": "Sariel enters the library where her brother Cassian is reading.", "entity_tracking": {"mentioned_entities": ["Sariel", "Cassian"]}}'
            ),
        ]
    ...
    def test_entity_manifest_creation_and_validation(self):
        ...
        manifest = create_from_game_state(
            self.game_state.to_dict(), session_number=1, turn_number=5
        )
