...
class TestStructuredResponseIntegration(unittest.TestCase):
    ...
    def test_god_mode_response_generation(self, mock_firestore, mock_genai_client):
        ...
        mock_response.text = json.dumps({
            ...
            "state_updates": {
                "combat_state": {"round_number": 1, "initiative_order": ["Hero", "orc_1"]}
            },
            "debug_info": {
                "dice_rolls": ["Initiative: Hero 18, Orc 12"],
                "resources": "Player HP: 45/45, Spell Slots: 4/4/3/3/1",
                "dm_notes": ["Orc positioned for flanking", "Player has tactical advantage with ranged options"],
                "state_rationale": "Combat initiated, positions set"
            }
        })
        
        mock_client_instance.models.generate_content.return_value = mock_response
        mock_client_instance.models.count_tokens.return_value = MagicMock(total_tokens=150)
        ...
        response = self.client.post(
            ...
            headers=self.test_headers,
            json={'input': 'god mode: An orc appears! What should I do?', 'mode': 'god'}
        )
        ...
        god_choices = [choice_id for choice_id in choices.keys() if choice_id.startswith('god:')]
        self.assertGreaterEqual(len(god_choices), 2, "Should have at least 2 God mode choices")
        ...
        for choice_id, choice_data in choices.items():
            if choice_id.startswith('god:'):
                ...
                self.assertRegex(choice_data['description'], r'(\+\d+|\d+d\d+|AC|damage|advantage)', 
    ...
    @patch('gemini_service.genai.Client')
    @patch('firestore_service.firestore')
    def test_meta_dm_advice_god_mode_generation(self, mock_firestore, mock_genai_client):
        ...
        mock_response.text = json.dumps({
            ...
            "entities_mentioned": ["Lyra", "Crystal_of_Eternal_Light", "Ancient_Prophecy"],
            ...
            "debug_info": {
                "plot_structure": ["Setup complete", "Rising action: personal stakes", "Next: complication"],
                "character_development": "Lyra: Identity crisis -> Self-acceptance arc",
                "dm_tools": ["Dramatic irony", "Foreshadowing", "Personal stakes integration"],
                "narrative_timing": "Perfect moment for backstory integration - player investment is high"
            }
        })
        
        mock_client_instance.models.generate_content.return_value = mock_response
        mock_client_instance.models.count_tokens.return_value = MagicMock(total_tokens=200)
        ...
        choices = planning_block['choices']
        god_choices = [choice_id for choice_id in choices.keys() if choice_id.startswith('god:')]
        self.assertGreaterEqual(len(god_choices), 3, "Should have at least 3 God mode DM advice choices")
        
        # Verify DM advice choices contain narrative guidance
        dm_keywords = ['backstory', 'narrative', 'character', 'plot', 'reveal', 'development', 'arc']
        for choice_id, choice_data in choices.items():
            if choice_id.startswith('god:'):
                self.assertIn('text', choice_data)
                self.assertIn('description', choice_data)
                ...
                keyword_found = any(keyword in description_lower for keyword in dm_keywords)
    ...
    def test_real_god_mode_dm_advice_integration(self):
        ...
        self.assertEqual(response.status_code, 200, f"God mode request failed: {response.data}")
        data = response.json
        ...
        dm_advice_indicators = [
            'plot', 'hook', 'intrigue', 'player', 'agency', 'political', 'transition',
            'story', 'narrative', 'conflict', 'character', 'motivation'
        ]
        god_response_lower = god_response.lower()
        advice_matches = [indicator for indicator in dm_advice_indicators 
                         if indicator in god_response_lower]
        self.assertGreaterEqual(len(advice_matches), 3, 
        ...
        planning_block = data['planning_block']
        self.assertIn('thinking', planning_block, "Missing thinking field in planning block")
        self.assertIn('choices', planning_block, "Missing choices field in planning block")
        ...
        god_choices = [choice_id for choice_id in choices.keys() if choice_id.startswith('god:')]
        self.assertGreaterEqual(len(god_choices), 2, f"Should have God mode choices with god: prefix. Got: {list(choices.keys())}")
        
        # Verify choice structure and DM-focused content
        for choice_id, choice_data in choices.items():
            if choice_id.startswith('god:'):
                self.assertIn('text', choice_data, f"Choice {choice_id} missing text")
                self.assertIn('description', choice_data, f"Choice {choice_id} missing description")
                ...
                choice_text_lower = (choice_data['text'] + ' ' + choice_data['description']).lower()
                dm_guidance_terms = ['introduce', 'develop', 'create', 'guide', 'present', 'establish', 
                                   'handle', 'discuss', 'explain', 'transition', 'political', 'conflict',
                                   'player', 'agency', 'intrigue', 'social', 'plot', 'hook']
                has_dm_guidance = any(term in choice_text_lower for term in dm_guidance_terms)
                self.assertTrue(has_dm_guidance, 
