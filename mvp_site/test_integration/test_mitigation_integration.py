...
from entity_validator import EntityValidator, EntityRetryManager
...
class TestMitigationIntegration(unittest.TestCase):
    ...
    def test_validator_with_retry_manager(self):
        ...
        result, attempts = self.retry_manager.validate_with_retry(
            narrative_missing, self.expected_entities, 
            self.location, retry_callback
        )
    ...
    def test_dual_pass_with_validator(self):
        ...
        result = self.dual_pass.generate_with_dual_pass(
            "Continue the story", self.expected_entities,
            self.location, generation_callback
        )
    ...
    def test_cassian_problem_mitigation(self):
        ...
        cassian_instruction = self.instruction_gen.create_entity_specific_instruction("Cassian", player_input)
        ...
        cassian_suggestions = [s for s in validation.retry_suggestions if "Cassian" in s]
