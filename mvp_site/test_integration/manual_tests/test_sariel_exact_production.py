...
def run_sariel_exact_production():
    ...
    print("\nResponse preview:")
    print(
        continue_response[:500] + "..."
        if len(continue_response) > 500
        else continue_response
    )
    ...
    if cassian_in_response:
        print(
            f"✅ Cassian in response: {cassian_in_response} - CASSIAN PROBLEM SOLVED!"
        )
    else:
        print(
            f"❌ Cassian in response: {cassian_in_response} - CASSIAN PROBLEM DETECTED!"
        )
        print(
            "   The user referenced Cassian but the AI didn't include him in the response"
        )
    ...
    print(
        f"2. Cassian problem test: {'✅ PASSED' if cassian_in_response else '❌ FAILED'}"
    )
