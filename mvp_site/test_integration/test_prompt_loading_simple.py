...
class TestPromptLoadingLogic(unittest.TestCase):
    ...
    def test_user_selectable_prompts_constant(self):
        ...
        self.assertEqual(
            ...
            ['narrative', 'mechanics', 'calibration']
        )
    ...
    def test_calibration_filtering_in_continue_story(self):
        ...
        selected_prompts_filtered = [p_type for p_type in selected_prompts 
    ...
    def test_calibration_filtering_with_future_prompts(self):
        ...
        selected_prompts = ['narrative', 'mechanics', 'calibration', 'future_prompt_1', 'future_prompt_2']
        selected_prompts_filtered = [p_type for p_type in selected_prompts 
                                    if p_type != constants.PROMPT_TYPE_CALIBRATION]
        
        # Should include everything except calibration
        self.assertEqual(selected_prompts_filtered, ['narrative', 'mechanics', 'future_prompt_1', 'future_prompt_2'])
        self.assertNotIn('calibration', selected_prompts_filtered)
    ...
    def test_calibration_included_in_get_initial_story(self):
        ...
        prompts_to_load = [constants.PROMPT_TYPE_NARRATIVE, 
    ...
    def test_prompt_loading_order_get_initial_story(self, mock_load):
        ...
        system_instruction_parts.append(gemini_service._load_instruction_file(constants.PROMPT_TYPE_MASTER_DIRECTIVE))
        system_instruction_parts.append(gemini_service._load_instruction_file(constants.PROMPT_TYPE_GAME_STATE))
        system_instruction_parts.append(gemini_service._load_instruction_file(constants.PROMPT_TYPE_ENTITY_SCHEMA))
        
        if constants.PROMPT_TYPE_NARRATIVE in selected_prompts:
            system_instruction_parts.append(gemini_service._load_instruction_file(constants.PROMPT_TYPE_CHARACTER_TEMPLATE))
        
        if constants.PROMPT_TYPE_MECHANICS in selected_prompts:
            system_instruction_parts.append(gemini_service._load_instruction_file(constants.PROMPT_TYPE_CHARACTER_SHEET))
        
        for p_type in [constants.PROMPT_TYPE_NARRATIVE, constants.PROMPT_TYPE_MECHANICS, constants.PROMPT_TYPE_CALIBRATION]:
            if p_type in selected_prompts:
                system_instruction_parts.append(gemini_service._load_instruction_file(p_type))
        
        system_instruction_parts.append(gemini_service._load_instruction_file(constants.PROMPT_TYPE_DESTINY))
    ...
    def test_prompt_loading_order_continue_story(self, mock_load):
        ...
        calibration_calls = [c for c in mock_load.call_args_list if c == call('calibration')]
        self.assertEqual(len(calibration_calls), 0, "Calibration should not be loaded in continue_story")
