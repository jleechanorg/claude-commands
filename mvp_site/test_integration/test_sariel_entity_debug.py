...
logging_util.basicConfig(
    ...
    handlers=[logging_util.StreamHandler(sys.stdout)],
)
...
class TestSarielEntityDebug(unittest.TestCase):
    ...
    def test_debug_sariel_missing(self):
        ...
        if state_response.status_code == 200:
            ...
            print(
                f"Player character data: {game_state.get('player_character_data', {})}"
            )
        ...
        print(
            f"Expected entities: {first_prompt['context'].get('expected_entities', [])}"
        )

        interaction_data = {"input": first_prompt["input"], "mode": "character"}
