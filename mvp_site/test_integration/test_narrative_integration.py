...
class TestNarrativeIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if not DEPS_AVAILABLE:
            self.fail(
                "CRITICAL FAILURE: Integration test dependencies missing (Flask, etc.). Install requirements.txt to fix."
            )
    ...
    def _get_game_state(self):
        ...
        if not state_json or not state_json.strip():
            print("WARNING: Empty state_json received")
            return {}
        try:
        ...
        try:
            ...
            for i in range(max_interactions):
                ...
                if self._detect_npc_approach(response_text):
                    ...
                    print(
                        f"NPC approach detected at interaction {i + 1}, scene {current_scene}"
                    )
            ...
            if len(npc_approaches) > 0:
                ...
                print(
                    f"⚠️ No NPC approaches detected in {interactions_tested} interactions (may be within normal variance)"
                )
    ...
    def test_dynamic_encounter_frequency(self):
        ...
        try:
            ...
            for i, prompt in enumerate(populated_prompts):
                interaction_data = {"input": prompt, "mode": "character"}
            ...
            print("✅ Dynamic encounter frequency test completed:")
            print(
                f"   Populated area encounters: {len(populated_encounters)}/{len(populated_prompts)} ({populated_rate:.1%})"
            )
            print(
                f"   Wilderness area encounters: {len(wilderness_encounters)}/{len(wilderness_prompts)} ({wilderness_rate:.1%})"
            )
            ...
            if populated_encounters or wilderness_encounters:
                print("   Encounter patterns detected across environments")
                print("   System is responding to environmental context")
            else:
                print(
                    "   No specific encounters detected (may be within normal variance)"
                )
