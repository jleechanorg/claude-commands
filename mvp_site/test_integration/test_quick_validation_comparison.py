...
class TestQuickValidationComparison(unittest.TestCase):
    ...
    def test_cassian_problem_comparison(self):
        ...
        for i in range(5):
            ...
            results['simple_cassian_tests'].append({
                'run': i+1,
                'cassian_mentioned': cassian_mentioned
            })
            logger.info(f"Result: Cassian {'✓ mentioned' if cassian_mentioned else '✗ missing'}")
            time.sleep(1)
    ...
    def _test_cassian_problem(self, run_id: str) -> bool:
        ...
        try:
            ...
            cassian_prompt = self.sariel_data['prompts'][2]  # "ask for forgiveness. tell cassian..."
            
            interaction_data = {
                'input': cassian_prompt['input'],
                'mode': 'character'
            }
    ...
    def _display_summary(self, results: dict):
        ...
        # Simple validation
        simple_success = sum(1 for r in results['simple_cassian_tests'] if r['cassian_mentioned'])
        logger.info(f"\nSimple Validation: {simple_success}/5 ({simple_success*20}%)")
        
        # Pydantic validation
        pydantic_success = sum(1 for r in results['pydantic_cassian_tests'] if r['cassian_mentioned'])
        logger.info(f"Pydantic Validation: {pydantic_success}/5 ({pydantic_success*20}%)")
        
        # Analysis
        logger.info("\nAnalysis:")
        if simple_success == 0 and pydantic_success == 0:
            logger.info("❌ The Cassian Problem remains unsolved with current implementation")
            logger.info("   Entity tracking modules are not fully integrated")
        elif simple_success > pydantic_success:
            logger.info(f"✓ Simple validation performed better ({simple_success*20}% vs {pydantic_success*20}%)")
        elif pydantic_success > simple_success:
            logger.info(f"✓ Pydantic validation performed better ({pydantic_success*20}% vs {simple_success*20}%)")
        else:
