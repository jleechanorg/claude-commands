...
class TestStructuredFieldsIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.structured_fields = {
            ...
            constants.FIELD_DICE_ROLLS: ['Attack Roll: 1d20+8 = 22', 'Damage: 2d6+5 = 13'],
    ...
    def test_structured_fields_full_flow(self, mock_firestore, mock_genai_client):
        ...
        mock_db.collection.return_value.document.return_value.get.return_value = MagicMock(exists=True, to_dict=lambda: mock_user_doc)
        ...
        mock_response.text = json.dumps({
                ...
                'session_header': self.structured_fields[constants.FIELD_SESSION_HEADER],
                'planning_block': self.structured_fields[constants.FIELD_PLANNING_BLOCK],
        ...
        self.assertEqual(response_data[constants.FIELD_DICE_ROLLS], self.structured_fields[constants.FIELD_DICE_ROLLS])
        self.assertEqual(response_data[constants.FIELD_RESOURCES], self.structured_fields[constants.FIELD_RESOURCES])
        self.assertEqual(response_data[constants.FIELD_DEBUG_INFO], self.structured_fields[constants.FIELD_DEBUG_INFO])
        ...
        self.assertEqual(gemini_story_data[constants.FIELD_SESSION_HEADER], self.structured_fields[constants.FIELD_SESSION_HEADER])
        self.assertEqual(gemini_story_data[constants.FIELD_PLANNING_BLOCK], self.structured_fields[constants.FIELD_PLANNING_BLOCK])
        self.assertEqual(gemini_story_data[constants.FIELD_DICE_ROLLS], self.structured_fields[constants.FIELD_DICE_ROLLS])
        self.assertEqual(gemini_story_data[constants.FIELD_RESOURCES], self.structured_fields[constants.FIELD_RESOURCES])
        self.assertEqual(gemini_story_data[constants.FIELD_DEBUG_INFO], self.structured_fields[constants.FIELD_DEBUG_INFO])
    ...
    def test_structured_fields_partial_flow(self, mock_firestore, mock_genai_client):
        ...
        self.assertEqual(response_data[constants.FIELD_SESSION_HEADER], 'Session 2: Exploration')
        self.assertEqual(response_data[constants.FIELD_DICE_ROLLS], ['Perception: 1d20+3 = 16'])
