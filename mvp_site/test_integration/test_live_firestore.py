...
class TestLiveFirestoreFetching(unittest.TestCase):
    ...
    def test_sequence_id_generation_and_sorting(self):
        ...
        firestore_service.add_story_entry(
            self.user_id, self.campaign_id, "user", "First new entry."
        )
        _campaign, story = firestore_service.get_campaign_by_id(
            self.user_id, self.campaign_id
        )
        ...
        print("--- Adding Turn 2 ---")
        firestore_service.add_story_entry(
            self.user_id, self.campaign_id, "gemini", "Second new entry."
        )
        _campaign, story = firestore_service.get_campaign_by_id(
            self.user_id, self.campaign_id
        )
        ...
        print("--- Adding Turn 3 ---")
        firestore_service.add_story_entry(
            self.user_id, self.campaign_id, "user", "Third new entry."
        )
        _campaign, story = firestore_service.get_campaign_by_id(
            self.user_id, self.campaign_id
        )
