...
os.environ['CI'] = 'true'  # Force CI mode to disable real services
...
from test_integration.test_integration import BaseCampaignIntegrationTest, create_mock_gemini_response, run_god_command
...
class TestInteractionIntegration(BaseCampaignIntegrationTest):
    ...
    def setUpClass(cls):
        ...
        cls.update_game_state_patcher = patch('firestore_service.update_campaign_game_state')
    ...
    def test_ai_state_update_is_merged_correctly(self, mock_gemini_api):
        ...
        mock_gemini_api.return_value = create_mock_gemini_response(
            ...
            planning_block={
                "options": [
                    {"action": "ContinueExploring", "description": "Continue exploring with your new strength"},
                    {"action": "RestAndRecover", "description": "Rest and recover from your adventure"},
                    {"action": "CheckInventory", "description": "Check your new gold and equipment"}
                ]
            },
            state_updates={
                "player_character_data": {
    ...
    def test_comprehensive_mock_infrastructure(self):
        ...
        expected_fields = ['title', 'created_at', 'id']  # Use actual field names from mock data
        ...
        print(f"Final test data structure has {len(final_data)} root keys with complex nesting preserved")
    ...
    def test_google_auth_never_called_in_mock_tests(self, mock_gemini_api):
        ...
        try:
            ...
            print("✅ All Firebase operations successfully mocked - no authentication required")
            print("✅ CI authentication error should be resolved")
            
        except Exception as e:
            error_message = str(e)
            if any(keyword in error_message.lower() for keyword in ['credentials', 'authentication', 'auth', 'default service account']):
                self.fail(f"Firebase authentication still being attempted despite comprehensive mocking: {error_message}")
            else:
        ...
        for func_name in firestore_functions:
            if hasattr(firestore_service, func_name):
                ...
                try:
                    ...
                    error_message = str(e)
                    if any(keyword in error_message.lower() for keyword in ['credentials', 'authentication', 'auth', 'default service account']):
                        self.fail(f"Function {func_name} still attempting authentication: {error_message}")
    ...
    def test_story_progression_smoke_test(self, mock_gemini_api):
        ...
        mock_gemini_api.return_value = create_mock_gemini_response(
            ...
            planning_block={
                "options": [
                    {"action": "ExamineArtifact", "description": "Examine the crystalline orb closely"},
                    {"action": "TakeArtifact", "description": "Take the artifact"},
                    {"action": "LeaveArtifact", "description": "Leave the artifact and continue exploring"}
                ]
            }
        )
