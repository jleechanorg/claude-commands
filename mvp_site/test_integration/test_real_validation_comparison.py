...
class TestRealValidationComparison(unittest.TestCase):
    ...
    def _run_single_campaign_test(self, run_id: str) -> Dict[str, Any]:
        ...
        result = {
            'run_id': run_id,
            'validation_type': 'pydantic' if os.environ.get('USE_PYDANTIC') == 'true' else 'simple',
        ...
        try:
            ...
            if response.status_code != 201:
                result['errors'].append(f"Campaign creation failed: {response.status_code}")
                return result
            ...
            for i, prompt_data in enumerate(self.sariel_data['prompts'][1:6]):
                ...
                if response.status_code == 200:
                    ...
                    logger.info(f"Interaction {i+1}: {found_count}/{expected_count} entities tracked")
                    
                else:
                    interaction_result['error'] = f"Request failed: {response.status_code}"
                    logger.error(f"Interaction {i+1} failed: {response.status_code}")
                
                result['interactions'].append(interaction_result)
                result['total_entities_tracked'] += len(interaction_result['entities_found'])
    ...
    def _display_summary(self, results: Dict[str, Any]):
        ...
        logger.info(f"  Cassian Problem solved: {simple_cassian_solved}/10 ({simple_cassian_solved*10}%)")
        ...
        pydantic_cassian_solved = sum(1 for r in pydantic_runs if r['cassian_mentioned'])
        pydantic_total_entities = sum(r['total_entities_tracked'] for r in pydantic_runs)
        pydantic_successful_runs = sum(1 for r in pydantic_runs if r['campaign_created'])
        
        logger.info("\nPydantic Validation Results:")
        logger.info(f"  Successful runs: {pydantic_successful_runs}/10")
        logger.info(f"  Cassian Problem solved: {pydantic_cassian_solved}/10 ({pydantic_cassian_solved*10}%)")
        ...
        # Comparison
        logger.info("\nComparison:")
        logger.info(f"  Cassian Problem: Simple {simple_cassian_solved*10}% vs Pydantic {pydantic_cassian_solved*10}%")
        logger.info(f"  Entity Tracking: Simple {simple_total_entities} vs Pydantic {pydantic_total_entities}")
