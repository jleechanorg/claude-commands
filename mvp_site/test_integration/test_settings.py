...
class TestConstants:
    ...
    FORMATS = {
        ...
        'docx': {'mime': 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'content': b'DOCX content here'},
    ...
    def get_sample_campaign(campaign_id: str = None) -> Dict[str, Any]:
        ...
        return {
            ...
            'story': [
                ...
                {'content': 'You find ancient runes carved into the bark.', 'actor': 'gemini'}
            ],
            'player': {
                'name': 'Hero',
                'level': 5,
...
class BaseAPITest(BaseTestCase):
    ...
    def get_campaign(self, campaign_id: str, expected_status: int = 200):
        ...
        response = self.client.get(f'/api/campaigns/{campaign_id}', headers=self.test_headers)
        if expected_status:
            self.assertEqual(response.status_code, expected_status)
        return response
    ...
    def export_campaign(self, campaign_id: str, format_type: str, expected_status: int = 200):
...
class ErrorTestingMixin:
    ...
    def test_error_conditions(self, endpoint: str, method: str, error_cases: List[tuple]):
        """Test multiple error conditions for an endpoint."""
        for case_name, request_data, mock_side_effect, expected_status, expected_error in error_cases:
            with self.subTest(case=case_name):
                ...
                if method.upper() == 'GET':
                    ...
                    response = self.client.post(endpoint, headers=self.test_headers, json=request_data)
                elif method.upper() == 'PATCH':
                    response = self.client.patch(endpoint, headers=self.test_headers, json=request_data)
