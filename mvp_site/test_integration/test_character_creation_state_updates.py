...
def test_character_creation_state_updates():
    ...
    try:
        ...
        has_state_updates = (
            "[STATE_UPDATES_PROPOSED]" in response
            or "STATE_UPDATES_PROPOSED" in response
        )
        has_end_marker = (
            "[END_STATE_UPDATES_PROPOSED]" in response
            or "END_STATE_UPDATES_PROPOSED" in response
        )
        ...
        if has_state_updates and has_end_marker:
            ...
            if state_match:
                ...
                print(
                    state_content[:200] + "..."
                    if len(state_content) > 200
                    else state_content
                )

                # Check if it tracks character creation
                if (
                    "character_creation" in state_content
                    or "custom_campaign_state" in state_content
                ):
