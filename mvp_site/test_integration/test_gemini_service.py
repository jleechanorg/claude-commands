...
class TestInitialStoryPromptAssembly(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        temp_path_map = {
            key: os.path.join(cls.temp_dir, path)
            for key, path in gemini_service.PATH_MAP.items()
        }
    ...
    def _create_realistic_mock_content(self, instruction_type):
        ...
        return content_map.get(
            instruction_type, f"# {instruction_type.upper()}\nGeneric content..."
        )

    def _assert_instructions_loaded_in_order(
        self, actual_system_instruction, expected_instruction_types
    ):
    ...
    def test_all_checkboxes_scenario(self, mock_load_instruction_file, mock_get_client):
        ...
        self._assert_instructions_loaded_in_order(
            actual_system_instruction, expected_instruction_order
        )

    @patch("gemini_service.get_client")
    @patch("gemini_service._load_instruction_file")
    def test_get_initial_story_no_selected_prompts(
        self, mock_load_instruction_file, mock_get_client
    ):
    ...
    def test_only_narrative_scenario(self, mock_load_instruction_file, mock_get_client):
        ...
        # --- Assert ---
        actual_system_instruction = (
            mock_get_client.return_value.models.generate_content.call_args.kwargs[
                "config"
    ...
    @patch("gemini_service.get_client")
    @patch("gemini_service._load_instruction_file")
    def test_only_calibration_scenario(
        self, mock_load_instruction_file, mock_get_client
    ):
    ...
    def test_use_default_world_adds_prefix_to_prompt(
        self, mock_add_world, mock_load_instruction_file, mock_get_client
    ):
        ...
        expected_modified_prompt = (
            "Use default setting Assiah. Create a character who is a brave knight"
        )

        # --- Act ---
        gemini_service.get_initial_story(
            original_prompt, selected_prompts=["narrative"], use_default_world=True
        )
        ...
        actual_call_args = (
            mock_get_client.return_value.models.generate_content.call_args
        )
...
class TestStaticPromptParts(unittest.TestCase):
    def test_core_memories_formatting_with_memories(self):
        ...
        mock_game_state.custom_campaign_state = {
            "core_memories": ["Defeated the goblin king.", "Found the ancient amulet."]
        }
        ...
        # --- Act ---
        _, core_memories_summary, _ = gemini_service._get_static_prompt_parts(
            mock_game_state, []
        )
...
class TestContextTruncation(unittest.TestCase):
    ...
    def test_context_just_under_limit_no_truncation(self, mock_get_stats):
        ...
        story_context = [{"text": "a" * 100}, {"text": "b" * 100}, {"text": "c" * 100}]
        char_budget = 301
...
class TestResourceTrackingInDebugOutput(unittest.TestCase):
    ...
    def test_debug_mode_includes_resource_tracking_instructions(
        self, mock_get_static, mock_truncate, mock_load_instruction, mock_api_call
    ):
        ...
        gemini_service.continue_story(
            "test input", "character", [], test_game_state, ["narrative"]
        )
    ...
    @patch("gemini_service._truncate_context")
    @patch("gemini_service._get_static_prompt_parts")
    def test_debug_mode_false_still_includes_debug_instructions(
        self, mock_get_static, mock_truncate, mock_load_instruction, mock_api_call
    ):
    ...
    @patch("gemini_service._truncate_context")
    @patch("gemini_service._get_static_prompt_parts")
    def test_resource_tracking_format_comprehensive(
        self, mock_get_static, mock_truncate, mock_load_instruction, mock_api_call
    ):
...
class TestUserInputCountAndModelSelection(unittest.TestCase):
    ...
    @patch("gemini_service._truncate_context")
    @patch("gemini_service._get_static_prompt_parts")
    def test_pro_model_used_for_first_five_inputs(
        self, mock_get_static, mock_truncate, mock_load_instruction, mock_api_call
    ):
        ...
        test_cases = [
            ...
            (
                [
                    {"actor": "user", "text": "input1"},
                    {"actor": "user", "text": "input2"},
                ],
                3,
                gemini_service.DEFAULT_MODEL,
            ),  # 3rd input
            (
                [{"actor": "user", "text": f"input{i}"} for i in range(1, 4)],
                4,
                gemini_service.DEFAULT_MODEL,
            ),  # 4th input
            (
                [{"actor": "user", "text": f"input{i}"} for i in range(1, 5)],
                5,
                gemini_service.DEFAULT_MODEL,
            ),  # 5th input
            (
                [{"actor": "user", "text": f"input{i}"} for i in range(1, 6)],
                6,
                gemini_service.DEFAULT_MODEL,
            ),  # 6th input
        ]
    ...
    @patch("gemini_service._truncate_context")
    @patch("gemini_service._get_static_prompt_parts")
    def test_testing_mode_always_uses_test_model(
        self, mock_get_static, mock_truncate, mock_load_instruction, mock_api_call
    ):
        ...
        # Act
        gemini_service.continue_story(
            "test input", "character", story_context, test_game_state, ["narrative"]
        )
    ...
    @patch("gemini_service._truncate_context")
    @patch("gemini_service._get_static_prompt_parts")
    def test_user_input_count_calculation_ignores_ai_entries(
        self, mock_get_static, mock_truncate, mock_load_instruction, mock_api_call
    ):
    ...
    @patch("gemini_service._truncate_context")
    @patch("gemini_service._get_static_prompt_parts")
    def test_user_input_count_handles_empty_context(
        self, mock_get_static, mock_truncate, mock_load_instruction, mock_api_call
    ):
