...
"""
    ...
    def _print_summary(self, results: dict):
        ...
        simple_cassian_success = sum(
            1 for r in results["simple_results"] if r["cassian_mentioned"]
        )
        simple_test_success = sum(
            1 for r in results["simple_results"] if r["cassian_problem_tested"]
        )
        logger.info("\nSimple Validation:")
        ...
        pydantic_cassian_success = sum(
            1 for r in results["pydantic_results"] if r["cassian_mentioned"]
        )
        pydantic_test_success = sum(
            1 for r in results["pydantic_results"] if r["cassian_problem_tested"]
        )
        logger.info("\nPydantic Validation:")
