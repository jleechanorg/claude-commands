    ...
    def create_entity_preload_text(self, game_state: Dict[str, Any], 
        ...
        if manifest.player_characters:
            ...
            for pc in manifest.player_characters:
                ...
                pc_name = pc.display_name if hasattr(pc, 'display_name') else getattr(pc, 'name', 'Unknown')
        ...
        if manifest.npcs:
            ...
            for npc in manifest.npcs:
                ...
                status_text = f" ({', '.join(status_info)})" if status_info else ""
                npc_name = npc.display_name if hasattr(npc, 'display_name') else getattr(npc, 'name', 'Unknown')
        ...
        entity_names = []
        if manifest.player_characters:
            entity_names.extend([pc.display_name if hasattr(pc, 'display_name') else getattr(pc, 'name', 'Unknown') for pc in manifest.player_characters])
        if manifest.npcs:
            entity_names.extend([npc.display_name if hasattr(npc, 'display_name') else getattr(npc, 'name', 'Unknown') for npc in manifest.npcs])
