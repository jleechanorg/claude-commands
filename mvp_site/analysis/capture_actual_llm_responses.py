...
def capture_sariel_llm_responses():
    ...
    try:
        ...
        prompts_path = os.path.join(os.path.dirname(__file__), 'mvp_site', 'tests', 'data', 'sariel_campaign_prompts.json')
        ...
        for i, prompt_data in enumerate(interaction_prompts):
            ...
            if prompt_data['metadata'].get('is_cassian_problem'):
                ...
                print(f"🚨 CASSIAN PROBLEM CHECK: {'✅ RESOLVED' if cassian_mentioned else '❌ STILL FAILING'}")
        ...
        output_path = os.path.join('mvp_site', 'tests', 'data', 'sariel_actual_llm_responses.json')
        ...
        summary_lines = [
            ...
            f"**Capture Method**: Working integration test pattern",
        ...
        for response in captured_responses:
            ...
            if response['metadata']['is_cassian_problem']:
                ...
                summary_lines.insert(-2, f"🚨 **THE CASSIAN PROBLEM**: {'RESOLVED' if cassian_mentioned else 'STILL FAILING'}")
                summary_lines.insert(-2, "")

        summary_path = os.path.join('mvp_site', 'tests', 'data', 'sariel_actual_responses_summary.md')
