...
**II. The Dynamic Aptitude System**
...
*   **C. The Yearly Progression Model:** A character's Aptitude scores change fractionally each year based on their age and Aptitude Potential coefficient, as follows:
    *   **1. Growth Years (Ages 16-25):**
        *   All Aptitudes: `+ (Potential Coefficient / 10)` points per year.
    *   **2. Plateau Years (Ages 26-35):**
        *   Physical (Physique, Coordination, Health): `+ ((Potential Coefficient - 3) * 0.1)`
        *   Intelligence & Wisdom: `+ (Potential Coefficient * 0.05)`
    *   **3. Prime & Early Decline (Ages 36-54):**
        *   Physical: `((3 - Potential Coefficient) * -0.2)`
        *   Intelligence: `+ ((Potential Coefficient - 4) * 0.1)`
        *   Wisdom: `+ ((Potential Coefficient - 2) * 0.05)`
    *   **4. Late Decline (Ages 55+):**
        *   Physical: `((2 - Potential Coefficient) * -0.2)`
        *   Intelligence: `((3 - Potential Coefficient) * -0.2)`
        *   Wisdom: `+ ((Potential Coefficient - 2) * 0.05)`

*   **D. Special Growth: The "Lesson Learned" Check:** Wisdom can also be actively developed outside of normal progression by successfully making an Intelligence check to learn from significant life failures. The Challenge Number (CN) for this check is between 12-18, determined by the GM (AI) based on the severity and complexity of the failure from which the character is attempting to learn. Success grants a minor, immediate increase to Wisdom (e.g., +0.1 to +0.5 points). This check can only be attempted for a truly significant, character-defining failure, at the GM's discretion, and generally no more than once per Class Level.
