...
* **2.1 Core Resolution:** All actions where the outcome is uncertain are resolved with a Resolution Check: d20 + relevant modifier vs. a Challenge Number (CN).
* **2.2 Aptitude System:**
    * A character has five core Aptitudes: Physique, Coordination, Health, Intelligence, and Wisdom.
    * **Aptitude Score:** The current number representing a character's ability, used for checks.
    * **Aptitude Potential:** A hidden stat that represents a character's innate talent and capacity for growth.
    * **Hidden Stats:** All NPC stats are hidden by default.
* **2.3 Personality Traits (The Big Five):**
    * These are five static traits rated 1-5 used as direct modifiers for social checks: Openness, Conscientiousness, Extraversion, and Agreeableness.
    * **Neuroticism:** Informs character behavior and specific mechanics, but is not used for active checks.
* **2.4 Relationship & Rapport System:**
    * **Rapport (Trust & History):** A score from -5 (Rivals) to +10 (Inseparable) between any two main characters. It modifies the CN for social checks between them.
    * **Chemistry (Attraction & Romance):** A score from 0 (None) to +10 (Soulmates). It provides a bonus to checks related to romance and flirting.
    * **NPC Rapport:** A simplified tier system (0-3) models bonds with non-player characters, providing bonuses to social checks and influencing Internal Conflict checks.
    * **Protector's Conviction:** When a character makes a Deception check specifically to protect an individual with whom they have a positive Rapport score, they may add their Rapport score with that person as a bonus to their roll.
...
* **2.6 Internal Conflict System:** Major character-defining decisions are resolved with a contested check (d20 + Trait A vs d20 + Trait B), with bonuses applied based on a character's Core Motivation.
