...
**Schema:** See the "NPC Data Input Structure" section for the JSON structure. Relationships are stored in `npc_data.<name>.relationships.player` with fields: `trust_level` (-10 to +10), `disposition` (hostile/antagonistic/neutral/friendly/allied), `history` (array), `debts` (array), `grievances` (array).
    ...
    "Theron Blackwood": {
      "relationships": {
        "player": {
          "trust_level": 5,
          "disposition": "trusted",
          "history": ["saved his life in the goblin cave", "shared a drink at the tavern", "helped rescue his sister"]
        }
      }
    }
  }
