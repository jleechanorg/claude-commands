...
class TestFunctionValidationFlow(unittest.TestCase):
    ...
    def test_complete_interaction_with_hp_discrepancy_detection(self, 
                                                               mock_add_story, 
                                                               mock_update_state,
                                                               mock_get_state,
                                                               mock_get_campaign,
        ...
        proposed_changes = gemini_service.parse_llm_response_for_state_changes(ai_response)
        
        if proposed_changes:
            # Apply changes to temporary state for validation
            temp_state_dict = test_state.to_dict()
            updated_temp_state = firestore_service.update_state_with_changes(temp_state_dict, proposed_changes)
            temp_game_state = GameState.from_dict(updated_temp_state)
            
            # Validate the response against updated state
            post_update_discrepancies = temp_game_state.validate_checkpoint_consistency(ai_response)
            
            # Should detect HP/consciousness discrepancy
            self.assertGreater(len(post_update_discrepancies), 0, 
                             "Should detect HP/consciousness discrepancy")
            
            # Check that discrepancy mentions unconscious and HP
            found_hp_discrepancy = any("unconscious" in d.lower() and "hp" in d.lower() 
                                     for d in post_update_discrepancies)
            self.assertTrue(found_hp_discrepancy, 
                          "Should detect specific unconscious/HP mismatch")
        
        # 3. Verify logging occurred (check our captured log messages)
        validation_logs = [msg for msg in self.log_messages if "VALIDATION" in msg or "discrepancy" in msg.lower()]
    ...
    def test_mission_completion_detection(self, mock_get_client):
        ...
        self.assertGreater(len(discrepancies), 0, "Should detect mission completion discrepancies")
        ...
        self.assertIn("dragon", discrepancy_text, "Should detect dragon mission completion")
        self.assertIn("treasure", discrepancy_text, "Should detect treasure mission completion")
    ...
    def test_mock_libraries_behavior(self):
        ...
        campaign, story = self.mock_firestore.get_campaign_by_id(self.test_user_id, self.test_campaign_id)
    ...
    def test_state_update_integration(self):
        ...
        original_memories = len(fresh_state_data["custom_campaign_state"]["core_memories"])
        new_memories = len(updated_state.custom_campaign_state["core_memories"])
        
        # The append should add 1 new memory
        self.assertEqual(new_memories, original_memories + 1, "Should have exactly 1 more memory after append")
