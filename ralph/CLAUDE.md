...
- If remote CI verification is blocked (for example `gh` cannot reach `api.github.com`), capture the exact failing command/error and run local workflow-equivalent checks as evidence in `progress.txt`.
