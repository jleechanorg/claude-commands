...
# Usage: ./ralph.sh or ./ralph/ralph.sh [--tool amp|claude] [max_iterations]

set -e

cleanup() {
  echo ""
  echo "Ralph interrupted. Progress saved to ${PROGRESS_FILE:-progress.txt}"
}
...
for i in $(seq 1 $MAX_ITERATIONS); do
  ...
  # Check for completion signal (normalize whitespace so AI wrapping/newlines don't break detection)
