...
import sys
mvp_site_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'mvp_site')
...
if not firebase_admin._apps:
    cred_path = os.path.expanduser("~/.firebase/worldarchitect-ai-firebase-adminsdk.json")
...
def analyze_narrative_for_desync(narrative: str, game_state: dict) -> List[Dict]:
    ...
    if 'npc_data' in game_state:
        for npc_id, npc_data in game_state['npc_data'].items():
            if isinstance(npc_data, dict):
                ...
                if npc_data.get('location') == game_state.get('world_data', {}).get('current_location'):
                    expected_entities.add(name)
    ...
    if current_location:
        ...
        location_mentioned = any(word in narrative_lower for word in location_words if len(word) > 3)
...
def analyze_campaign(campaign_id: str, campaign_data: dict, user_id: str, limit: int = 50) -> Dict:
    ...
    game_state_ref = db.collection('users').document(user_id).collection('campaigns').document(campaign_id).collection('game_state')
    
    # Actually, let's get the campaign doc which contains story
    campaign_doc = db.collection('users').document(user_id).collection('campaigns').document(campaign_id).get()
    ...
    if analysis['total_turns'] > 0:
        analysis['desync_rate'] = len(analysis['desync_incidents']) / analysis['total_turns']
...
def select_campaigns_for_testing():
    ...
    for campaign_data in real_campaigns:
        ...
        if any(a['campaign_id'] == campaign_data['campaign_id'] for a in analyzed_campaigns):
            continue
        ...
        for incident in campaign['desync_incidents'][:3]:  # First 3 examples
            for issue in incident['issues']:
                if issue['type'] == 'missing_entity':
                    desync_examples.append(f"- Turn {incident['turn']}: Missing {issue['entity']} from narrative")
                elif issue['type'] == 'missing_combat_participant':
                    desync_examples.append(f"- Turn {incident['turn']}: Combat participant {issue['entity']} not mentioned")
                elif issue['type'] == 'location_mismatch':
                    desync_examples.append(f"- Turn {incident['turn']}: Location '{issue['expected_location']}' not referenced")
