...
def initialize_firebase():
    ...
    if not firebase_admin._apps:
        try:
            ...
            project_root = os.path.dirname(worktree_root)  # Go up one more level from worktree_tweaks/
            firebase_key_path = os.path.join(project_root, "serviceAccountKey.json")
            
            if os.path.exists(firebase_key_path):
                cred = credentials.Certificate(firebase_key_path)
                firebase_admin.initialize_app(cred)
                print(f"✅ Firebase initialized with service account key from {firebase_key_path}")
            else:
...
def analyze_with_collection_groups(db):
    ...
    for story_doc in story_group.stream():
        ...
        if len(path_parts) >= 6 and path_parts[0] == 'users' and path_parts[2] == 'campaigns' and path_parts[4] == 'story':
            user_id = path_parts[1]
    ...
    for user_id, data in user_data.items():
        ...
        if data['campaigns']:
            data['most_active_campaign'] = max(data['campaigns'], key=lambda x: x['entry_count'])
            data['avg_entries_per_campaign'] = data['total_entries'] / data['total_campaigns'] if data['total_campaigns'] > 0 else 0
...
def generate_report(user_analytics):
    ...
    sorted_users = sorted(user_analytics, 
    ...
    if len(user_analytics) > 0:
        print(f"Average campaigns per user: {total_campaigns / len(user_analytics):.2f}")
    ...
    print("-" * 40)
    print(f"{'Rank':<4} {'User ID':<30} {'Campaigns':<10} {'Entries':<8} {'Avg/Campaign':<12} {'Most Active Campaign'}")
    print("-" * 100)
    ...
    for i, user in enumerate(sorted_users[:10], 1):
        ...
        print(f"{i:<4} {user_id_display:<30} {user['total_campaigns']:<10} "
    ...
    for i, user in enumerate(sorted_users[:3], 1):
        ...
        if user['most_active_campaign']:
            ...
            print(f"   Most Active Campaign: {mac['title']} ({mac['entry_count']} entries)")
        
        # Show top 3 campaigns for this user
        top_campaigns = sorted(user['campaigns'], key=lambda x: x['entry_count'], reverse=True)[:3]
...
def export_to_csv(user_analytics, filename="analysis/firebase_collection_group_analytics.csv"):
    ...
    with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['rank', 'user_id', 'total_campaigns', 'total_entries', 
                      'avg_entries_per_campaign', 'most_active_campaign_title', 
