...
def analyze_prs(since_date):
    ...
    cmd = f'gh pr list --state merged --limit 1000 --json number,title,createdAt,mergedAt'
...
def main():
    ...
    print(f"\nCommit types:")
    print(f"  - Fix commits: {commits['fix_commits']} ({commits['fix_commits']/commits['total']*100:.1f}%)")
    print(f"  - Feature commits: {commits['feature_commits']} ({commits['feature_commits']/commits['total']*100:.1f}%)")
    print(f"  - Other commits: {commits['other_commits']} ({commits['other_commits']/commits['total']*100:.1f}%)")
    print()
