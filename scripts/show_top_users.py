...
def initialize_firebase():
    ...
    if not firebase_admin._apps:
        ...
        if os.path.exists(firebase_key_path):
            ...
            raise Exception(f"❌ Could not find Firebase key")
    ...
    for campaign in campaigns:
        ...
        for story in stories[:3]:  # First 3 entries
            ...
            snippet = {
                ...
                'text': story_data.get('text', '')[:100] + '...' if len(story_data.get('text', '')) > 100 else story_data.get('text', '')
            }
...
def show_top_users():
    ...
    user_analytics.sort(key=lambda x: x['total_campaigns'] + x['total_entries'], reverse=True)
    ...
    for i, user in enumerate(user_analytics[:10], 1):
        print(f"\n#{i} USER: {user['user_id']}")
        print(f"   📊 {user['total_campaigns']} campaigns, {user['total_entries']} total entries")
