    ...
    campaigns = [
        {
            ...
            'desync_incidents': [
                ...
                },
                {
                    'turn': 78,
                    'issues': [
                        {'type': 'missing_combat_participant', 'entity': 'Shadow Guardian'},
                        {'type': 'location_mismatch', 'expected_location': 'Temple of Shadows'}
                    ],
            ...
            'desync_incidents': [
                {
                    ...
                    'issues': [
                        ...
                        {'type': 'missing_combat_participant', 'entity': 'Kraken Tentacle 3'}
                    ],
            ...
            'desync_incidents': [
                {
                    'turn': 89,
                    'issues': [
                        {'type': 'location_mismatch', 'expected_location': 'Astral Plane'},
        ...
        for incident in campaign['desync_incidents'][:2]:
            for issue in incident['issues']:
                if issue['type'] == 'missing_entity':
                    examples.append(f"  - Turn {incident['turn']}: Missing '{issue['entity']}' from narrative")
                elif issue['type'] == 'missing_combat_participant':
                    examples.append(f"  - Turn {incident['turn']}: Combat participant '{issue['entity']}' not mentioned")
                elif issue['type'] == 'location_mismatch':
                    examples.append(f"  - Turn {incident['turn']}: Location '{issue['expected_location']}' not referenced")
        
        issues_summary = ', '.join(f"{k.replace('_', ' ')}: {v}" for k, v in campaign['common_issues'].items())
