...
class MockGeminiAPI:
    ...
    async def generate_narrative_async(self, prompt: str, approach: str = "baseline") -> GeminiResponse:
        ...
        return GeminiResponse(
            text=response_text,
            tokens_used=tokens,
            generation_time_ms=delay * 1000
        )
    
    def generate_narrative(self, prompt: str, approach: str = "baseline") -> GeminiResponse:
...
Narrative: {included[0]} surveyed the scene{f' while {included[1]} stood nearby' if len(included) > 1 else ''}."""
        ...
        elif approach == "validation_hints":
            ...
            if entities:
                return f"{entities[0]} looked around the area, ready for whatever came next."
    ...
    def _extract_entities_from_prompt(self, prompt: str) -> List[str]:
        ...
        if "Present:" in prompt:
            ...
            # Filter common words
            entities = [w for w in words if w not in 
                       ["The", "Location", "Character", "Narrative", "Generate", 
                        "Continue", "Write", "Present", "REQUIREMENTS"]]
