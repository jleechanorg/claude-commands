...
def add_mock_to_line(lines, line_num, mock_response_var):
    ...
    mock_creation = [
        f"        # Create mock GeminiResponse",
        f"        mock_gemini_response = MagicMock()",
        f"        mock_gemini_response.narrative_text = {mock_response_var}",
        f"        mock_gemini_response.debug_tags_present = {{'dm_notes': True, 'dice_rolls': True, 'state_changes': False}}",
        f"        mock_gemini_response.state_updates = {{}}",
        f"        "
    ]
...
# Read the file
file_path = '/home/jleechan/projects/worldarchitect.ai/mvp_site/tests/test_debug_mode.py'
...
for i, line in enumerate(lines):
    ...
    if "with patch('gemini_service.continue_story', return_value=mock_gemini_response):" in line:
