...
def test_write_and_read(db):
    ...
    story_entries = [
        {'actor': 'user', 'text': 'Test story entry 1', 'timestamp': datetime.now(timezone.utc)},
        {'actor': 'gemini', 'text': 'Test story entry 2', 'timestamp': datetime.now(timezone.utc)},
        {'actor': 'user', 'text': 'Test story entry 3', 'timestamp': datetime.now(timezone.utc)}
    ]
...
def verify_data_exists(db, test_user_id, campaign_id):
    ...
    for i, story in enumerate(stories):
        ...
        print(f"     Entry {i+1}: {story_data['actor']} - {story_data['text'][:30]}...")
