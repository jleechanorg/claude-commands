...
sys.path.insert(0, os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'prototype'))
...
class ValidationOnlyApproach:
        ...
        if isinstance(npc_data, dict):
            for npc_name, npc_info in npc_data.items():
                ...
                if isinstance(npc_info, dict):
                    ...
                    if npc_info.get('present', False) or npc_info.get('recently_active', False):
    ...
    def validate_narrative(self, narrative: str, entities: Dict[str, Set[str]]) -> Dict:
        ...
        validation_result['desync_rate'] = len(missing_entities) / len(entities['active_entities']) if entities['active_entities'] else 0
    ...
    def test_campaign_scenario(self, campaign_id: str, scenario_name: str, 
                             game_state: Dict, narrative: str) -> Dict:
        ...
        if result['desync_detected']:
            print(f"  ❌ Desync detected! Missing {len(result['entities_missing'])} entities:")
    ...
    def generate_report(self, output_path: str = None):
        ...
        avg_validation_time = sum(r['validation_time_ms'] for r in self.results) / total_tests
        ...
        report = {
            ...
            'summary': {
                ...
                'entity_detection_rate': (total_entities - total_missing) / total_entities if total_entities > 0 else 0,
        ...
        print(f"Tests with Desync: {total_desyncs} ({total_desyncs/total_tests*100:.1f}%)")
        print(f"Entity Detection Rate: {report['summary']['entity_detection_rate']*100:.1f}%")
        ...
        for campaign_id, stats in report['by_campaign'].items():
            ...
            print(f"  {campaign_id}: {stats['scenarios_with_desync']}/{stats['total_scenarios']} desyncs ({desync_rate*100:.1f}%)")
...
if __name__ == "__main__":
    ...
    sariel_state = {
        ...
        "world_data": {
            "current_location": "Cressida's Chambers"
        }
    }
