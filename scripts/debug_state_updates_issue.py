...
def parse_llm_response_for_state_changes(llm_text_response: str) -> dict:
    ...
    for json_string in reversed(matches):
        ...
        try:
            ...
            print(f"✗ Found STATE_UPDATES_PROPOSED but invalid JSON: {json_string[:50]}...")
...
class MockGeminiResponse:
    narrative_text: str
    structured_response: Optional[MockStructuredResponse]
...
What next?"""
    ...
    print("   ❌ This is the bug! State updates are in structured response, not markdown!\n")
    ...
    gemini_response = MockGeminiResponse(
        narrative_text=new_format_response,
